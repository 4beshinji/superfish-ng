...
from superfish_ng.material_hphi_tracking_jobs import execute_material_hphi_tracking,read_material_hphi_tracking,_prepare,execute_prepared_material_hphi_tracking
...
from test_material_hphi_tuning_saved import request as tune_request
...
from superfish_ng.material_hphi import solve_material_hphi


def pair():
    data=tune_request();project=HphiProject.from_dict(data['project'])
    law=MaterialHphiShapeLaw.from_dict(data['shape_law'])
    a,b=[build_material_hphi_tune_trial(project,law,value) for value in (1.,1.05)]
    q=material_hphi_trial_comparison(a,b,previous_mode_ids=['TEM'],previous_mode_count=1,current_mode_count=1)
    return tuple(solve_material_hphi(t.project.case) for t in (a,b)),q
...
class MaterialHphiTrackingJobTests(unittest.TestCase):
    def test_portable_owned_projects_native_and_rehashed_result_rejection(self):
        with tempfile.TemporaryDirectory() as t,ExitStack() as cleanup:
            ...
            run=root/'tracking';report=execute_material_hphi_tracking(a,b,q,run);self.assertEqual(report['status'],'PASS')
            ...
            self.assertEqual(read_material_hphi_tracking(run),report);self.assertEqual(read_job(run)['status'],'complete')
            ...
            with self.assertRaises(FileExistsError):execute_material_hphi_tracking(run/'previous',run/'current',q,run)
            ...
            with self.assertRaises(ValueError):read_material_hphi_tracking(run)
    ...
    def test_worker_cancel_restart_and_forged_kind_cannot_change_routing(self):
        with tempfile.TemporaryDirectory() as t,ExitStack() as cleanup:
            ...
            cancelled=manager.start_material_hphi_tracking(a,b,q);self.assertEqual(manager.cancel(cancelled)['status'],'cancelled')
            identifier=manager.start_material_hphi_tracking(a,b,q);self.assertEqual(manager.processes[identifier].wait(timeout=180),0)
            directory=manager.directory(identifier);report=read_material_hphi_tracking(directory);self.assertEqual(report['status'],'PASS')
    ...
    def test_queued_input_change_and_linked_native_are_rejected(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            with self.assertRaisesRegex(ValueError,'queued hphi tracking inputs changed'):execute_prepared_material_hphi_tracking(run)
            self.assertEqual(json.loads((run/'job.json').read_text())['status'],'failed')
            with self.assertRaises(ValueError):execute_prepared_material_hphi_tracking(run)
            linked=root/'linked-source';shutil.copytree(b,linked);(linked/'fields.npz').unlink();(linked/'fields.npz').symlink_to(b/'fields.npz')
            with self.assertRaises(ValueError):execute_material_hphi_tracking(a,linked,q,root/'must-not-exist')
            self.assertFalse((root/'must-not-exist').exists())
            with self.assertRaises(ValueError):execute_material_hphi_tracking(a,b,replace(q,controls=replace(q.controls,max_dofs=1)),root/'budget-fail')
            self.assertFalse((root/'budget-fail').exists())
            with self.assertRaises(ValueError):execute_material_hphi_tracking(a,b,replace(q,max_sample_points=1),root/'samples-fail')
            ...
            with self.assertRaisesRegex(ValueError,'original material Hphi'):
                execute_material_hphi_tracking(root/'straight',b,q,root/'physics-fail')
            self.assertFalse((root/'physics-fail').exists())

    def test_public_cli_execute_and_replay_keep_unverified_guard(self):
        import io
        from contextlib import redirect_stdout
        from superfish_ng.cli import main
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);a,b,q=sources(root)
            q=replace(q,controls=replace(q.controls,relative_cluster_gap=.9));q.save(root/'request.json')
            out=io.StringIO()
            with redirect_stdout(out):
                self.assertEqual(main(['execute-material-hphi-tracking',str(a),str(b),str(root/'request.json'),'--out',str(root/'run')]),0)
            executed=json.loads(out.getvalue());self.assertEqual(executed['status'],'UNVERIFIED')
            self.assertEqual(executed['current_mode_ids'],[None])
            out=io.StringIO()
            with redirect_stdout(out):self.assertEqual(main(['replay-material-hphi-tracking',str(root/'run')]),0)
            self.assertEqual(json.loads(out.getvalue()),executed)
