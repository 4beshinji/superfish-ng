...
def partition(axis=False,holes=1,scale=1.,uniform=None):
    ...
    return OffAxisMagneticPartition(mesh,materials,regions)


class OffAxisMagneticMaterialTests(unittest.TestCase):
    def test_explicit_coverage_measures_and_interfaces_without_rf_parameters(self):
        for axis in (False,):
            for holes in (0,1,2):
                p=partition(axis,holes);self.assertEqual(p.to_dict(),OffAxisMagneticPartition.from_dict(p.to_dict()).to_dict())
    ...
    def test_no_implicit_vacuum_and_reject_unsupported_physics(self):
        ...
        for change in changes:
            ...
            with self.assertRaises(ValueError):OffAxisMagneticPartition.from_dict(data)
        with self.assertRaises(ValueError):partition(axis=True)
