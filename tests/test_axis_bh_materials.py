...
import numpy as np
from scripts.validate_axis_bh_forms import partition
from scripts.curved_meridional_reference import fixture
from superfish_ng.axis_bh_materials import AxisBHPartition


class AxisBHMaterialTests(unittest.TestCase):
    def test_original_ownership_tables_volumes_axis_contact_and_immutable_state(self):
        for holes in (0,1,2):
            p=partition(holes);self.assertEqual(AxisBHPartition.from_dict(p.to_dict()).to_dict(),p.to_dict())
            ...
            for i,r in enumerate(p.regions):self.assertEqual(p.region_axis_contact[i],bool(np.any(axis[list(r.cell_indices)])))
            ...
            for name in ('cell_material_indices','cell_region_indices','region_axis_contact','region_volume_m3','interface_material_definition_changes'):self.assertFalse(getattr(p,name).flags.writeable)
    ...
    def test_reject_missing_azimuthal_model_unsupported_geometry_material_and_cell_types(self):
        ...
        for change in changes:
            ...
            with self.assertRaises(ValueError):AxisBHPartition.from_dict(data)
        with self.assertRaises(ValueError):AxisBHPartition(fixture(False,1,shear=0.)[0]['base_mesh'],p.materials,p.regions)
