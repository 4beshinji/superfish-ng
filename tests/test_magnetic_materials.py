...
def partition(concave=False,scale=1.,rotation=None,shift=(0.,0.)):
    ...
    return PlanarMagneticPartition(mesh,[LinearMagneticMaterial('lower',2.),LinearMagneticMaterial('upper',5.)],
        [MagneticRegion('bottom','lower',np.flatnonzero(~labels).tolist()),MagneticRegion('top','upper',np.flatnonzero(labels).tolist())])


class PlanarMagneticMaterialTests(unittest.TestCase):
    def test_areas_interfaces_and_neutral_geometry_across_coordinate_origin(self):
        for concave in (False,True):
            p=partition(concave);self.assertEqual(p.to_dict(),PlanarMagneticPartition.from_dict(p.to_dict()).to_dict())
            np.testing.assert_allclose(p.region_area_m2,np.array([2.,1. if concave else 2.])/32**2,rtol=1e-13,atol=0.)
            self.assertNotIn('boundary',p.to_dict()['geometry']);self.assertNotIn('epsilon_r',p.to_dict()['materials'][0]);self.assertFalse(hasattr(p,'region_volume_m3'))
            ...
            for name in ('mu_r','reluctivity_m_per_h','cell_region_indices','region_area_m2','interface_edges'):self.assertFalse(getattr(p,name).flags.writeable)
    ...
    def test_strict_partition_without_implicit_boundary_material_or_thickness(self):
        ...
        changes=[lambda d:d.update(coordinates='axisymmetric_rz'),lambda d:d.update(thickness_m=1.),lambda d:d.update(schema_version=True),
            ...
            lambda d:d['materials'][0].update(epsilon_r=1.),lambda d:d['materials'][0].update(mu_r=-1.),lambda d:d['materials'][0].update(mu_r=1e-323),
        ...
        for bad in (True,0.,float('inf'),float('nan'),1+1j,10**1000):
            with self.assertRaises(ValueError):LinearMagneticMaterial('bad',bad)
        for change in changes:
            data=copy.deepcopy(raw);change(data)
            with self.assertRaises(ValueError):PlanarMagneticPartition.from_dict(data)
