...
class CurvedHphiTuningJobTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_real_worker_pause_resume_and_manager_restart(self):
        first = self.manager.start_curved_hphi_tune(self.request, max_new_trials=1)
        state = self.wait(first)
        self.assertEqual(state["status"], "complete")
        self.assertEqual(state["kind"], "curved_hphi_tune")
        self.assertEqual(state["tuning_status"], "PAUSED")
        self.assertEqual(state["computed_trials"], 1)
        checkpoint = read_curved_hphi_tune(self.root / first / "execution/checkpoint-001.json")
        second = self.manager.start_curved_hphi_tune(
            self.request, checkpoint=checkpoint, max_new_trials=1
        )
        state = self.wait(second)
        self.assertEqual(state["tuning_status"], "PAUSED")
        self.assertEqual(state["computed_trials"], 2)
        result = read_curved_hphi_tune(self.root / second / "curved-hphi-tune-results.json")
        ...
        self.manager = JobManager(self.root)
        self.addCleanup(self.manager.close)
        self.assertEqual(read_curved_hphi_tune(self.root / second / "curved-hphi-tune-results.json"), result)
        self.assertEqual(self.manager.status(second, verify=True)["tuning_status"], "PAUSED")

        from superfish_ng.curved_hphi_tuning_jobs import INPUT, _owned_input
    ...
    def test_invalid_submission_allocates_no_job(self):
        ...
        with self.assertRaises(ValueError):
            self.manager.start_curved_hphi_tune(invalid)
        self.assertEqual(self.manager.list(), [])

    def test_whole_worker_move_and_completion_manifest_tampering(self):
        identifier=self.manager.start_curved_hphi_tune(self.request,max_new_trials=1)
    ...
    def test_real_worker_cancel_preserves_checkpoint_for_new_job(self):
        ...
        first = self.manager.start_curved_hphi_tune(self.request, max_new_trials=1)
        state = self.wait(first)
        frequency = read_curved_hphi_tune(
        ...
        identifier = self.manager.start_curved_hphi_tune(running_request)
        ...
        checkpoint = read_curved_hphi_tune(checkpoint_path)
        self.assertEqual(checkpoint["status"], "PAUSED")
        resumed = self.manager.start_curved_hphi_tune(
    ...
    def test_prepared_worker_failure_keeps_checkpoint_without_manifest(self):
        ...
        from superfish_ng.curved_hphi_tuning_jobs import (
            INPUT,
            execute_prepared_curved_hphi_tune,
        )
        ...
        (directory / "job.json").write_text(
            json.dumps(dict(status="queued", kind="curved_hphi_tune", input_sha256={INPUT: ""})),
            encoding="utf-8",
        )
        from superfish_ng.jobs import _digest
        ...
        def fail(request_data, output, **kwargs):
            ...
            execute_curved_hphi_tune(request_data, output, max_new_trials=1)
            raise RuntimeError("injected Hphi worker failure")

        with patch("superfish_ng.curved_hphi_tuning_jobs.execute_curved_hphi_tune", side_effect=fail):
            with self.assertRaisesRegex(RuntimeError, "injected Hphi worker failure"):
                execute_prepared_curved_hphi_tune(directory)
        self.assertEqual(read_job(directory)["status"], "failed")
        self.assertEqual(
            read_curved_hphi_tune(directory / "execution/checkpoint-001.json")["status"],
            "PAUSED",
        )
