...
from superfish_ng.curved_hphi_saved import read_curved_hphi_run,curved_hphi_result


def request():
    p=HphiProject(coax(.125,shear=1/64,n=2));g=p.case.geometry
    return dict(format='superfish_ng_curved_hphi_tune',schema_version=1,project=p.to_dict(),parameter='deformation',
        shape_law=CurvedHphiShapeLaw(1.,g.points_rz_m.tolist(),'transport_on_axis').to_dict(),bounds=[1.,1.125],
        target_hz=1.1e9,frequency_tolerance_hz=1e6,mesh_frequency_tolerance_hz=1e6,parameter_tolerance=1e-6,
        max_trials=8,initial_ids=['TEM'],mode_id='TEM',controls=HphiTrackingControls().to_dict(),
        refinement_levels=1,max_triangles=10000,max_dofs=10000,max_sample_points=2000000,identity_recovery=None)


class CurvedHphiTuneSavedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        from superfish_ng.curved_hphi_tuning_saved import execute_curved_hphi_tune
        cls.tmp=tempfile.TemporaryDirectory();cls.root=Path(cls.tmp.name);cls.request=request()
        cls.first=execute_curved_hphi_tune(cls.request,cls.root/'first',max_new_trials=1)
        cls.second=execute_curved_hphi_tune(cls.request,cls.root/'second',checkpoint=cls.first,max_new_trials=1)

    @classmethod
    def tearDownClass(cls):cls.tmp.cleanup()

    def test_owned_portable_replay_and_resume_preserve_full_native(self):
        from superfish_ng.curved_hphi_tuning_saved import read_curved_hphi_tune
        ...
        with patch('superfish_ng.curved_hphi_tuning_saved.solve_curved_hphi',side_effect=AssertionError('replay solved')):
            self.assertEqual(read_curved_hphi_tune(self.root/'second/checkpoint-002.json'),self.second)
        a,b=[read_curved_hphi_run(Path(c['trial_runs'][0])/'solution') for c in (self.first,self.second)]
        ...
        self.assertEqual(curved_hphi_result(a),curved_hphi_result(b))
    ...
    def test_moving_source_and_whole_owned_output_needs_no_original(self):
        ...
        try:
            replay=read_curved_hphi_tune(relocated/'checkpoint-002.json')
    ...
    def test_tampering_paths_and_request_mismatch_rejected(self):
        from superfish_ng.curved_hphi_tuning_saved import read_curved_hphi_tune,replay_curved_hphi_tune,execute_curved_hphi_tune
        fields=Path(self.second['trial_runs'][0])/'solution/fields.npz';original=fields.read_bytes()
        try:
            fields.write_bytes(original+b'changed')
            with self.assertRaises(ValueError):read_curved_hphi_tune(self.root/'second/checkpoint-002.json')
        ...
        for name,value in (('schema_version',True),('can_resume',1),('format','superfish_ng_hphi_tune_checkpoint'),('extra',0)):
            ...
            with self.assertRaises(ValueError):replay_curved_hphi_tune(bad)
        ...
        with self.assertRaisesRegex(ValueError,'request differs'):
            execute_curved_hphi_tune(changed,self.root/'rejected',checkpoint=self.first)
        self.assertFalse((self.root/'rejected').exists())
        with self.assertRaises(FileExistsError):execute_curved_hphi_tune(self.request,self.root/'first')

    def test_failed_resume_preserves_verified_prefix(self):
        from superfish_ng.curved_hphi_tuning_saved import execute_curved_hphi_tune,read_curved_hphi_tune
        target=self.root/'failed-resume'
        with patch('superfish_ng.curved_hphi_tuning_saved.solve_curved_hphi',side_effect=RuntimeError('injected solve failure')):
            with self.assertRaisesRegex(RuntimeError,'injected solve failure'):
                execute_curved_hphi_tune(self.request,target,checkpoint=self.first,max_new_trials=1)
        prefix=read_curved_hphi_tune(target/'checkpoint-001.json')
    ...
    def test_traversal_and_symbolic_links_rejected_before_native_read(self):
        from superfish_ng.curved_hphi_tuning_saved import replay_curved_hphi_tune,read_curved_hphi_tune
        ...
        for path in ('../first/trial-001',str(self.root/'second/trial-001')):
            ...
            with self.assertRaises(ValueError):replay_curved_hphi_tune(bad,base_directory=self.root/'first')
        link=self.root/'checkpoint-link.json';link.symlink_to(self.root/'first/checkpoint-001.json')
        with self.assertRaises(ValueError):read_curved_hphi_tune(link)
        ...
        try:
            with self.assertRaises(ValueError):read_curved_hphi_tune(self.root/'first/checkpoint-001.json')
