...
class ProjectTests(unittest.TestCase):
    def test_repeated_polygon_area_and_end_lengths(self):
        ...
        cell = {
            "type": "profile",
            "points_zr_m": [[0, 0.04], [0.05, 0.08], [0.1, 0.04]],
        }
        end = {"type": "profile", "points_zr_m": [[0, 0.04], [0.02, 0.04]]}
        geometry = assemble_geometry(
            [
        ...
        self.assertAlmostEqual(case.length, 0.34)
        self.assertAlmostEqual(area, 0.0196)
    ...
    def test_unknown_duplicate_and_mismatch_rejected(self):
        case = {
            "schema_version": 1,
            "geometry": {"type": "pillbox", "radius_m": 0.06, "length_m": 0.1},
        }
        with self.assertRaisesRegex(ValueError, "duplicate"):
            load_document('{"schema_version":1,"schema_version":2}')
        with self.assertRaises(ValueError):
            Project.from_dict(
                {"project_version": 1, "case": case, "material": "dielectric"}
            )
        sections = [{"geometry": case["geometry"], "count": 2}]
        with self.assertRaisesRegex(ValueError, "geometry"):
            Project.from_dict(
                {"project_version": 1, "case": case, "sections": sections}
            )
        with self.assertRaises(ValueError):
    ...
    def test_arc_radius_and_indices_survive_assembly(self):
        ...
        self.assertTrue(all(a["radius_m"] == 0.01 for a in repeated["arcs"]))
    ...
    def test_save_refuses_overwrite_and_reflection_is_explicit(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            with self.assertRaises(FileExistsError):
                p.save(path)
        raw = copy.deepcopy(p.to_dict())
        raw["reflect_full"] = True
