...
2026-09-08、接点座標区間API追加後（直前基準 `0e217f2`）。最新の受入範囲と履歴は [実装状況](docs/IMPLEMENTATION_STATUS.md)。
...
| 検証 | 標準336件中334合格・2 skip。Pillbox/Bessel場、球形独立参照、楕円/双曲線の幾何・FEM細分、RF、保存、GUI等。数値・ブラウザー受入は個別記録を参照 |
...
弧端認証・全要件照合が残ります。保存・Case/CLI/GUIは接続済みです。
支持曲線の接点座標を有理数区間で囲むAPIを追加しました。有限弧端の所属認証は別の残件です。適応誤差推定は未対応です。
