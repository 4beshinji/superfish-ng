...
2026-09-08、接線構築GUI追加後（直前基準 `4b45bdf`）。最新の受入範囲と履歴は [実装状況](docs/IMPLEMENTATION_STATUS.md)。
...
| 検証 | 標準328件中326合格・2 skip。Pillbox/Bessel場、球形独立参照、楕円/双曲線の幾何・FEM細分、RF、保存、GUI等。数値・ブラウザー受入は個別記録を参照 |
...
弧端認証・全要件照合が残ります。保存・Case/CLI/GUIは接続済みです。適応誤差推定は未対応です。
