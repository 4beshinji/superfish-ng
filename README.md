...
2026-09-18 JST：H08の[同軸寸法と直線一般形状の写像契約](docs/HPHI_TUNING.md)を`hphi_geometry_mapping.py`へ追加した。
同軸の内外半径/長さの非相似写像と、軸/穴を保持する向き保存アフィン写像を有理行列式で検査し、
写像後の前メッシュを既存の厳密同領域overlayで現メッシュと比較する（最近点対応は推測しない）。
新7件・関連34件PASS、14.091秒、終了0。解析面積/体積と独立積分、一様尺度のH02一致、被覆/穴/軸の拒否を確認。次はH09。

2026-09-15 JST：[D01の元要件と明示後続要件](docs/D01_ACCEPTANCE.md)を監査し、親D01.S/I/Vを受入済みに更新した。
個別ID回復の全利用先接続と、異なる初期比較接続・境界分割への対応を含む21実行報告、検査内容、現在のソースを照合。
親33=10受入/16進行/6他未受入/1範囲外。D02/D03と全計画goalは継続中。以下の旧段階の未完表記は履歴である。


2026-09-15 JST：[明示参照座標による曲線比較版5](docs/CURVED_REFERENCE_PARTITION.md)を実装・限定受入。
...
2026-09-14：[逐次Studyの個別ID回復と再開](docs/TRACKED_STUDY_IDENTITY_RECOVERY.md)を要求版2・CLI/JobManager/GUIへ接続。
...
2026-09-14 JST：[非アフィン曲線寸法による周波数調整](docs/CURVED_HARMONIC_TUNING.md)を追加。
曲線寸法を多項式で連動させ、固定分割を保った実FEM・モード追跡・最終細分・保存再開へ接続しました。
関連42テスト、独立18実FEM、Chrome新版12/旧版7項目が合格。親D02全体と全計画goalは継続中です。

2026-09-14 JST：[固定局所細分履歴付きRF探索](docs/RF_OPTIMIZATION_HISTORY.md)を追加。
元の分割を保持して形状を変え、履歴の後に細分して制約・表面量を確認できます。
関連29テストと独立24実FEMの球形解析五量・境界積分・Maxwell尺度則が合格。全計画goalは継続中です。
新旧GUIの入力・中止再開・表面評価・元場表示も分割検証で確認しました。

2026-09-14 JST：[A01のcavsim2d比較と採用判断](docs/A01_BACKEND_COMPARISON.md)を完了。
固定版・両側3水準・2形状・3回の36実FEMで、周波数/RF/場と誤差対自由度/時間を照合しました。
製品は自作NGを維持し、候補は限定した独立参照用途とします。親33課題は9受入/17進行/6他未受入/1範囲外、全計画goalは継続中です。

2026-09-14 JST：[曲線親子追跡の共通履歴再構築](docs/NESTED_RECONSTRUCTION_REUSE.md)を共有化。
入力の厳密検証を保ち、3形状で転送・追跡・RF指標の完全一致とRF指標計算の約1.50〜1.53倍の改善を確認しました。
固定二次領域の質量保存・円筒解析RF・尺度則も合格。一般適応効率と全計画goalは継続中です。

2026-09-14 JST：[異なる細分履歴の共通比較分割](docs/CURVED_COMPARISON_OVERLAY.md)を版4で追加。
対応する初期P2分割から別々に細分した比較メッシュを交差分割し、元のFEM場を保存追跡・CLI/GUIで比較できます。
新7件の分割検証・既存56件、独立積分/4実FEMの尺度則、Chrome新版/従来版各10項目が合格。全計画goalは継続中。

2026-09-14 JST：[異なる曲線細分履歴への選択領域移送](docs/CURVED_SELECTION_TRANSFER.md)をAPI/CLI/GUIへ追加。
対応する初期分割から別々に細分したメッシュ間で領域の交差を求め、覆う要素または内包される要素を選べます。
新8/既存50件の分割検証、独立物理積分・4 FEMの尺度則、Chrome新18/旧20項目と保存場一致を確認。全計画goalは継続中。
...
2026-09-14 JST：[二次領域を保つ初期メッシュ置換](docs/CURVED_PROJECT_REMESH.md)をAPI/CLIへ追加。
置換メッシュと新しい全履歴を明示し、旧要素番号を流用せず分割を固定します。
新8件の分割検証・既存38件・専用8 FEMが合格。全二次境界/独立幾何・尺度則・円筒解析RFと保存追跡を確認。
Studyのメッシュ切替/二分/再開・GUIへの接続と自動対応は残り、全計画goalを継続中。
...
解析モード順位交差と二分点の実メッシュを確認。初期再メッシュ/自動対応・N04全体と全計画goalは継続中。

2026-09-14 JST：[調和変位による履歴付き曲線変形](docs/CURVED_HARMONIC_DEFORMATION.md)を追加。
曲線番号と曲線上の位置を対応させ、全体がアフィンでない形状変更でも元メッシュと固定分割を保持できます。
新8テストの分割検証と既存33件、専用4 FEM/CLI・独立幾何/尺度則・保存追跡再生が合格。
一般形状Study/専用GUIへの接続、自動対応とN04全体は残り、全計画goalを継続中。

2026-09-14 JST：[元メッシュと履歴を保持する曲線アフィンStudy](docs/CURVED_AFFINE_STUDY.md)を追加。
宣言した変形則から各条件を独立に作成し、保存追跡・逐次再開・区間二分・GUIへ接続しました。
関連62テストと入力補修後2件、専用9 FEMとworker再開5 FEM、Chrome10項目と表示3項目が合格。
一般形状/初期再メッシュの掃引、自動対応、N04全体は残り、全計画goalを継続中。以下は各段階の履歴。
...
2026-09-14 JST：[大規模曲線メッシュの図上選択](docs/LARGE_CURVED_MESH_SELECTION.md)を追加。
実計算の二次曲線と要素番号を保った250,000要素までの表示・拡大/移動・選択に対応。
26,624要素の実メッシュと選択後のnative再構築、250,000要素の描画試験、従来SVGから実FEMへの受渡しが合格。
N04全体の適応効率・物理精度やC00.V/G03/V02は継続中。以下の過去記録より各課題の最新文書を優先する。

2026-09-14 JST：[G03の現行要件照合](docs/G03_CURRENT_AUDIT.md)で構築10までの到達点と旧入力/物理精度の残件を整理した。
...
2026-09-14 JST：[非円円錐曲線同士の全元点対からのフィレット構築](docs/NONCIRCULAR_CONIC_FILLET.md)を限定受入。
構築版10で全有限元点対/順序、同一中心・共有端点・カスプ・零長と出力誤差を検査し、Case/CLI/GUIへ接続しました。
関連59テスト、独立32条件61元点対、解析積分と実FEM尺度則、Chrome110チェック42取得が合格。
旧構築1〜9/保存診断1〜13を保持。物理ピーク収束・旧曲線入力対応・G03全体と全計画は継続中。以下は各段階の履歴です。

2026-09-14 JST：[同じ円錐曲線の等距離オフセットと反対枝](docs/EQUAL_DISTANCE_CONIC_BRANCHES.md)を限定受入。
診断13で恒等零射影の共通支持/等距離を分類し、反対の双曲線枝の0/1/2接点と同じ枝の反対法線側の離隔を確定。
関連112+追加1テスト、独立576条件94元点対/凸性1,080検査、Chrome100チェック42取得が合格。旧診断1〜12/構築1〜9を保持。
次は一般非円の全元点対からのフィレット構築。G03全体と全計画は継続中。以下は各段階の履歴です。


2026-09-14 JST：[両非零の円錐曲線オフセット交点](docs/GENERAL_CONIC_OFFSET_INTERSECTIONS.md)を限定受入。
診断12で有限射影から全相手実点・向き付き距離・有限弧・カスプと中心重複を分類する。
関連118+追加2テスト、独立7条件18元点対、Chrome104チェック38取得が合格。旧診断1〜11/構築1〜9を保持。
恒等零の一部と新しい一般非円フィレット構築、G03全体と全計画は継続中。以下は各段階の履歴です。


2026-09-14 JST：[両非零オフセットの全元候補射影](docs/GENERAL_CONIC_OFFSET_PROJECTION.md)を専用範囲で受入。
一般の楕円/双曲線対から次数56以下の必要条件を作り、整数Sturm・元平方根符号・有限元弧で候補を列挙する。
関連35テスト、独立336点の行列式/6条件32候補が合格。相手の実元点・符号・有限弧は次段階であり、一般交点の完成ではない。
公開診断11と構築/GUIの経路は維持。G03全体と全計画は継続中。以下は各段階の履歴です。

2026-09-14 JST：[主軸表現が異なる同一円錐曲線のオフセット](docs/REPARAMETERIZED_CONIC_OFFSETS.md)を限定受入。
診断版11で軸交換・半回転後の有限重なり/共有端点/反射元点対を扱い、旧保存診断1〜10/構築1〜9を保持。
関連103テスト、独立1,984条件435反射元点対、実Chrome90チェック34取得が合格。
異なる支持・距離の両非零オフセットと新フィレット構築、G03全体と全計画は継続中。以下は各段階の履歴です。

2026-09-14 JST：[元の非円円錐曲線と法線オフセットの全交点](docs/CONIC_IMPLICIT_OFFSET.md)を限定受入。
一方の距離が0の楕円/双曲線の組合せを診断版10へ追加。関連97テスト、独立80条件200元点対が合格。
旧保存診断1〜9/構築1〜9を保持し、既存GUIの96チェック38取得も確認。
両非零オフセットの一般処理、新候補構築、G03全体と全計画は継続中。以下は各段階の履歴です。

2026-09-14 JST：[円同士の全元点対からのフィレット構築](docs/CIRCULAR_FILLET.md)を限定受入。
構築版9で接触・共有端点・潰れ円・負支持半径を扱い、旧構築1〜8/診断1〜9を保持。
関連62テストの最終合格、独立336条件264元点対と実FEM尺度則、Chrome186チェック86取得を確認。
非円弧同士の一般交点、物理ピーク収束、G03全体と全計画は継続中。以下は各段階の履歴です。

2026-09-14 JST：[円・非円円錐曲線の全元点対からのフィレット構築](docs/CIRCLE_CONIC_FILLET.md)を限定受入。
構築版8で端点・同一中心・カスプ・厳密零長を扱い、旧構築1〜7/診断1〜9を保持。
関連63テスト（62+追加1）、独立44条件100元点対、実FEM尺度則、Chrome154チェック64取得が合格。
円同士の全根からの構築・非円弧同士の一般交点、G03全体と全計画は継続中。
以下の日付付き記録は各段階の履歴であり、最新状態は先頭の記録を優先する。

2026-09-14 JST：[円と非円円錐曲線オフセットの全交点](docs/CIRCLE_CONIC_CROSSINGS.md)を限定受入。
新規診断版9で有限弧端・接触・カスプ・潰れ円を分類し、旧診断1〜8/構築1〜7を保持。
関連87件の最終合格、独立53条件、Chrome110チェック44取得と全22保存バイト一致を確認。
円/非円フィレットへの接続・非円弧同士の一般交点と全計画は継続中。

2026-09-14 JST：[全根列挙からの直線・円錐曲線フィレット構築](docs/ALGEBRAIC_CONIC_FILLET.md)を限定受入。
構築版7で端点・カスプ・同一中心の元点対を候補へ接続し、旧構築1〜6/診断1〜8を保持。
関連71テスト（70+追加1）・独立非円36/円24条件・FEM尺度則・Chrome138チェック54取得が合格。
曲線同士の一般交点、G03全体と全計画は継続中。

2026-09-14 JST：[直線と非円円錐曲線オフセットの全交点](docs/LINE_NONCIRCULAR_CROSSINGS.md)を限定受入。
関連83項目・独立115条件・実Chrome140チェック64取得が合格。カスプ・同一中心を含む全根を分類する。
新規診断版8、旧版1〜7を保持。新候補構築・曲線同士の一般交点と全計画は継続中。

2026-09-14 JST：[直線と非円楕円・双曲線オフセットの接触](docs/LINE_NONCIRCULAR_OFFSET_CONTACTS.md)を限定受入。
関連65テスト・独立1,944条件・実Chrome118チェック64取得が合格。
新規診断版7、旧版1〜6を保持。確認済み接触と全域分類を区別し、G03と全計画は継続中。

2026-09-14 JST：[一般二進回転・直線長を保持する円オフセット診断](docs/ALGEBRAIC_CIRCULAR_OFFSETS.md)を限定受入。
関連58テスト・独立1,944条件・実Chrome62チェック32取得が合格。
新規診断版6、旧保存版1〜5を保持。G03と全計画は継続中。
...
2026-09-13：[O02原要件の照合](docs/O02_ACCEPTANCE.md)を専用範囲で完了。
...
2026-09-13：[静的StudyのGUI入力・実行・各条件表示](docs/STATIC_FIELD_STUDY_GUI.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[静的ProjectのGUI入力・実求解・元場表示](docs/STATIC_FIELD_GUI.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[静的Projectの実worker・保存・CLI](docs/STATIC_FIELD_JOBS.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[検証済み磁気後処理量のGUI受渡し](docs/S05_GUI_HANDOFF.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[軸を含まない磁気力報告の保存・CLI](docs/OFF_AXIS_MAGNETIC_FORCE_NATIVE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[軸を含まない軸対称磁場の軸方向力](docs/OFF_AXIS_MAGNETIC_FORCE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[B-H・反跳材料の力・仮想仕事報告保存・CLI](docs/PLANAR_MAGNETIC_FORCE_MATERIAL_NATIVE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[B-H・反跳材料の平面仮想仕事](docs/PLANAR_MAGNETIC_VIRTUAL_WORK_MATERIALS.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[B-H・反跳材料解からの平面Maxwell応力](docs/PLANAR_MAGNETIC_FORCE_MATERIALS.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[平面磁場の力・トルク報告保存・CLI](docs/PLANAR_MAGNETIC_FORCE_NATIVE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[平面磁場のMaxwell応力と仮想仕事](docs/PLANAR_MAGNETIC_FORCE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[B-H/反跳モデルの多極報告保存・CLI](docs/PLANAR_MAGNETIC_MULTIPOLE_MATERIAL_NATIVE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[反跳/B-Hモデルの線形無源領域での多極抽出](docs/PLANAR_MAGNETIC_MULTIPOLE_MATERIALS.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[多極抽出結果の保存・元native照合・CLI](docs/PLANAR_MAGNETIC_MULTIPOLE_NATIVE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[元の平面FEM磁場からの多極抽出](docs/PLANAR_MAGNETIC_MULTIPOLE_EXTRACTION.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[平面磁場の有限多極表現と座標変換](docs/PLANAR_MAGNETIC_MULTIPOLES.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[S04原要件の受入照合](docs/S04_ACCEPTANCE.md)を完了。
...
2026-09-13：[軸非接続P1非線形磁場の保存・失敗再現・CLI](docs/OFF_AXIS_BH_NATIVE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[軸非接続P1の境界付き非線形磁静場](docs/OFF_AXIS_BH_SOLVE.md)を主ツリーへ統合・限定受入。
...
2026-09-13：[軸非接続P1の非線形B-H弱形式](docs/OFF_AXIS_BH_FORMS.md)を主ツリーへ統合・限定受入。
