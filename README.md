...
[TEの対称面鏡映](docs/TE_REFLECTION_PLAN.md)をAPI・CLI・専用native版4・Project/Job・GUIへ接続。元の半領域と係数を保存し、全領域の場を再構成して検証する。番号は対称性で選ばれた部分スペクトルの順序。直線P1/P2・曲面P2の独立解析/反射不変量と保存操作を確認し、標準790件（788合格、2skip）と既存TM/TE数値回帰が合格。鏡映結果の追跡/収束比較は未対応。
...
- TEの一般形状の追跡・鏡映結果の追跡/収束比較、m>0の双極・四重極モード、同軸TEM、静電場、静磁場、非線形材料は未実装。
