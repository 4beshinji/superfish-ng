...
2026-09-08、版3固定直線・弧接続統合後（直前基準 `0b9ef0a`）。最新の受入範囲と履歴は [実装状況](docs/IMPLEMENTATION_STATUS.md)。
...
| 検証 | 標準356件中354合格・2 skip。Pillbox/Bessel場、球形独立参照、楕円/双曲線の幾何・FEM細分、RF、保存、GUI等。数値・ブラウザー受入は個別記録を参照 |
...
フィレット・全要件照合が残ります。固定直線と弧の接続も版3で追加し、保存・Case/CLI/GUIへ接続済みです。
