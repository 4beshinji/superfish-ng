...
| 分野 | 0.1.0 の内容 |
|---|---|
| 形状 | pillbox、正の外半径を持つ折れ線 `R(z)`。各断面は軸から壁まで真空 |
| メッシュ | 形状に沿った2D三角形、一次要素、軸とPEC境界のタグ |
| 物理 | 真空、回転対称、m=0、Hφのみの磁場を持つTM系、全外壁PEC |
...
| 検証 | 23テスト、pillboxの周波数収束、6モード、Bessel場、RF量、スケーリング |
...
- `R(z)>0` が必要です。穴・内導体・垂直段差・折返す輪郭・任意CAD・RFQを扱えません。
- 両端は金属板で閉じています。例題の細い首は開放ビームポートではありません。
