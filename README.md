...
[境界分割が異なる多角形の厳密アフィン共通分割API](docs/PLANAR_EXACT_AFFINE_OVERLAY.md)を追加。
写像と逆写像を有理数で評価し、独立した境界/内部メッシュの各元要素を厳密に被覆する。
標準964件（962合格・2skip）、独立16条件32FEMと保存再読込、既存9モード19量の回帰を確認。
P02の幾何基盤であり、専用追跡要求・保存・CLI・GUIへの接続は後続工程。
...
- TEの一般形状の追跡・鏡映結果の追跡、m>0の双極・四重極モード、同軸TEM、静電場、静磁場、非線形材料は未実装。
