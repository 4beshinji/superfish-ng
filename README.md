...
2026-09-08、有限弧所属/fraction区間API追加後（直前基準 `6cf9c1d`）。最新の受入範囲と履歴は [実装状況](docs/IMPLEMENTATION_STATUS.md)。
...
| 検証 | 標準344件中342合格・2 skip。Pillbox/Bessel場、球形独立参照、楕円/双曲線の幾何・FEM細分、RF、保存、GUI等。数値・ブラウザー受入は個別記録を参照 |
...
G03全体は部分対応です。有限弧の接線数値判定と明示選択G1接続APIを追加しましたが、
区間に基づく切詰め・全要件照合が残ります。保存・Case/CLI/GUIは接続済みです。
支持曲線の接点座標を有理数区間で囲むAPIを追加しました。有限弧所属/fractionの区間APIも追加し、区間に基づく切詰め・構築統合が残ります。適応誤差推定は未対応です。
