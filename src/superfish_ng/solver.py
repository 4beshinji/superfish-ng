...
def solve(case):
    mesh = make_mesh(case)
    k, m = assemble(mesh)
    if case.modes >= len(mesh.points)-1:
        raise ValueError("modes must be smaller than node count minus one")
    d = 1/np.sqrt(m.diagonal())
    scale = diags(d)
    a, b = (scale @ mat @ scale for mat in (k, m))
    ...
    lam, u = lam[order], d[:, None]*vectors[:, order]
