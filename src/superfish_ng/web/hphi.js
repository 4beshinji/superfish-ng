...
async function refresh(){
 const jobs=(await api('jobs')).filter(job=>['hphi_solve','hphi_study','hphi_convergence','hphi_tracking','hphi_tracking_history','hphi_tune','curved_hphi_tune','material_hphi_tune','curved_hphi_tracking','material_hphi_tracking','curved_hphi_tracking_history','material_hphi_tracking_history'].includes(job.kind)),signature=JSON.stringify(jobs);
 ...
 const label=document.createElement('strong');label.textContent=`${job.status}${['hphi_tune','curved_hphi_tune','material_hphi_tune'].includes(job.kind)?' / 周波数調整':job.kind==='hphi_study'?' / 独立掃引':job.kind==='hphi_convergence'?' / 細分差診断':['hphi_tracking_history','curved_hphi_tracking_history','material_hphi_tracking_history'].includes(job.kind)?' / 追跡履歴':['hphi_tracking','curved_hphi_tracking','material_hphi_tracking'].includes(job.kind)?' / 部分空間対応':''} / ${job.id}`;row.append(label);
 ...
 button.onclick=run(async()=>{if(active){await api('cancel',{id:job.id});await refresh();}else if(['hphi_tune','curved_hphi_tune','material_hphi_tune'].includes(job.kind))await openHphiTune(job.id);else if(job.kind==='hphi_study')await openStudy(job.id);else if(job.kind==='hphi_convergence')await openConvergence(job.id);else if(['hphi_tracking_history','curved_hphi_tracking_history','material_hphi_tracking_history'].includes(job.kind))await openHphiHistory(job.id);else if(['hphi_tracking','curved_hphi_tracking','material_hphi_tracking'].includes(job.kind))await openTracking(job.id);else await openResult(job.id);});row.append(button);
 if(['hphi_tune','curved_hphi_tune','material_hphi_tune'].includes(job.kind)&&!active){const checkpoints=document.createElement('button');checkpoints.textContent='保存地点を選ぶ';checkpoints.dataset.hphiTuneCheckpoints=job.id;checkpoints.onclick=run(async()=>{const data=await api('hphi-tune-checkpoints',{id:job.id});let list=row.querySelector('.hphi-tune-checkpoints');if(!list){list=document.createElement('div');list.className='hphi-tune-checkpoints';row.append(list);}list.replaceChildren();if(!data.indices.length)list.textContent='完了した保存地点はありません。';for(const index of data.indices){const open=document.createElement('button');open.textContent=`試行 ${index} までを再検証`;open.dataset.hphiTuneCheckpoint=index;open.onclick=run(async()=>showHphiTune(await api('hphi-open-tune-checkpoint',{id:job.id,index}),`${job.id} / 試行 ${index}`));list.append(open);}});row.append(checkpoints);}
 $('jobs').append(row);}
}
...
function loadTracking(q){
 ...
 const a=q.previous_resolution?.current_partition.mesh||q.previous_comparison_geometry?.base_mesh||q.previous_comparison_mesh,b=q.current_resolution?.current_partition.mesh||q.current_comparison_geometry?.base_mesh||q.current_comparison_mesh;
...
function historyCandidates(jobs){
 historyPairJobs=jobs.filter(j=>['hphi_tracking','curved_hphi_tracking','material_hphi_tracking'].includes(j.kind)&&j.status==='complete');const signature=JSON.stringify(historyPairJobs.map(j=>j.id));
...
function loadHphiTune(request){
 ...
 const mesh=request.project.case.format==='superfish_ng_coaxial_case'?request.project.case.mesh:request.project.case.geometry?.base_mesh||request.project.case.partition?.mesh||request.project.case.mesh||request.project.case.fem;
