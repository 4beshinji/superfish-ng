...
async function refresh(){
 const jobs=(await api('jobs')).filter(job=>['hphi_solve','hphi_study','hphi_convergence','hphi_tracking','hphi_tracking_history','hphi_tune','curved_hphi_tune','material_hphi_tune','curved_hphi_tracking','curved_hphi_tracking_history'].includes(job.kind)),signature=JSON.stringify(jobs);
 ...
 const label=document.createElement('strong');label.textContent=`${job.status}${['hphi_tune','curved_hphi_tune','material_hphi_tune'].includes(job.kind)?' / 周波数調整':job.kind==='hphi_study'?' / 独立掃引':job.kind==='hphi_convergence'?' / 細分差診断':['hphi_tracking_history','curved_hphi_tracking_history'].includes(job.kind)?' / 追跡履歴':['hphi_tracking','curved_hphi_tracking'].includes(job.kind)?' / 部分空間対応':''} / ${job.id}`;row.append(label);
