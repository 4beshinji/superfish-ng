...
function documentFromForm(){
 ...
 const fem=isCylinder(c)?c.mesh:c.fem;fem.element_order=numeric('order');fem.quadrature_order=numeric('quadrature');
...
function showQuantities(){
 ...
 $('na').textContent='R/Q（加速器定義・回路定義）・加速電圧：N/A。軸は真空領域外で、加速経路を宣言していません。';
 ...
 if(q.wall_h2_integral_a2_by_segment){let offset=0;const lines=[];for(const [component,contour] of [result.case.mesh.outer_rz_m,...result.case.mesh.holes_rz_m].entries()){for(let edge=0;edge<contour.length;edge++)lines.push(`${component?'穴 '+component:'外周'} / 線分 ${edge+1}: ${(q.surface_resistance_ohm*q.wall_h2_integral_a2_by_segment[offset++]/2).toPrecision(6)}`);}$('segment-losses').textContent=lines.join('\n');}
}
