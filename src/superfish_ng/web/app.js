...
async function refreshJobs() {
  ...
  for (const j of jobs) {
    ...
    if (j.kind === "planar_solve") title.textContent += " / 平面RF";
    row.append(title);
    ...
    action.onclick = async () => {
      try {
        ...
        else if (j.kind === "planar_solve") location.href = `/planar.html?job=${encodeURIComponent(j.id)}`;
...
function trackingJobs(jobs) {
  const completed = jobs.filter(j => j.status === "complete" && !["study","tracked_study","adaptive_study","tune","adaptive_refinement","planar_solve"].includes(j.kind));
