...
async function refreshJobs() {
  ...
  for (const j of jobs) {
    ...
    if (["hphi_solve","hphi_study","hphi_convergence","hphi_tracking"].includes(j.kind)) title.textContent += " / Hφ RF";
...
function trackingJobs(jobs) {
  const completed = jobs.filter(j => j.status === "complete" && !["study","tracked_study","adaptive_study","tune","adaptive_refinement","hphi_solve","hphi_study","hphi_convergence","hphi_tracking","planar_solve","planar_study","planar_convergence","planar_tracking","planar_tracking_history"].includes(j.kind));
