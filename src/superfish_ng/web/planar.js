...
for(const id of ['convergence-levels','convergence-ranks','convergence-budget','convergence-thresholds'])$(id).addEventListener('input',()=>{$('dirty').textContent='未保存の細分要求';});
 ...
 function refreshTrackingSources(jobs){
  trackingJobs=jobs;const kind=$('tracking-mapping').value,polygon=kind!=='normalized_rectangle',similarity=kind==='polygon_similarity',remesh=kind==='polygon_same_domain',composed=kind==='polygon_similarity_remesh',affine=kind==='polygon_affine_remesh';$('tracking-polygon-options').hidden=!polygon||remesh;$('tracking-remesh-options').hidden=!(remesh||composed||affine);$('tracking-origin-note').hidden=similarity||composed||affine;$('tracking-similarity-options').hidden=!(similarity||composed||affine);$('tracking-angle-row').hidden=affine;$('tracking-affine-options').hidden=!affine;$('tracking-refinement-options').hidden=composed||affine;$('tracking-remesh-note').textContent=remesh?'同じ多角形の元要素どうしを比較します。節点番号や分割は異なって構いません。形状の変更は受け付けません。':'宣言変換で前の境界節点列を厳密に写し、内部の節点・対角線・接続は独立に選べます。境界節点の列は変換後の値と完全一致させる必要があります。';
 ...
 function trackingFromForm(){
  const kind=$('tracking-mapping').value,polygon=kind!=='normalized_rectangle',similarity=kind==='polygon_similarity',remesh=kind==='polygon_same_domain',composed=kind==='polygon_similarity_remesh',affine=kind==='polygon_affine_remesh';
  ...
  return {format:'superfish_ng_planar_tracking_request',tracking_version:affine?6:composed?5:remesh?4:similarity?3:polygon?2:1,mapping,previous_mode_count:numeric('tracking-previous-count'),current_mode_count:numeric('tracking-current-count'),previous_mode_ids:JSON.parse($('tracking-ids').value),previous_identity_groups:JSON.parse($('tracking-groups').value),controls:JSON.parse($('tracking-controls').value)};
 }
 function loadTracking(request){
  const polygon=[2,3,4,5,6].includes(request.tracking_version);$('tracking-mapping').value=polygon?request.mapping.name:'normalized_rectangle';
  ...
  if([4,5,6].includes(request.tracking_version))$('tracking-candidate-tests').value=request.mapping.max_candidate_tests;
  ...
  if(request.tracking_version===6){$('tracking-linear-a').value=request.mapping.linear_xy[0][0];$('tracking-linear-b').value=request.mapping.linear_xy[0][1];$('tracking-linear-c').value=request.mapping.linear_xy[1][0];$('tracking-linear-d').value=request.mapping.linear_xy[1][1];$('tracking-translation-x').value=request.mapping.translation_xy_m[0];$('tracking-translation-y').value=request.mapping.translation_xy_m[1];$('tracking-inverse').checked=request.mapping.inverse;}
