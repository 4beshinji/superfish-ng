...
"""Axis-connected recoil magnetostatics with affine H and named potentials."""
...
from .axis_recoil_fem import axis_recoil_forms
...
class AxisRecoilCase:
    ...
    def __post_init__(self):
        if type(self.partition) is not AxisRecoilPartition:raise ValueError('explicit axis magnetic partition required')
        partition=AxisRecoilPartition.from_dict(self.partition.to_dict());object.__setattr__(self,'partition',partition)
    ...
    def to_dict(self):
        return dict(format='superfish_ng_axis_recoil_case',schema_version=1,physics='linear_recoil_magnetostatic',
    ...
    def from_dict(cls,data):
        ...
        if (data['format']!='superfish_ng_axis_recoil_case' or type(data['schema_version']) is not int
            or data['schema_version']!=1 or data['physics']!='linear_recoil_magnetostatic'):
            raise ValueError('expected superfish_ng_axis_recoil_case version 1, linear_recoil_magnetostatic physics')
        if type(data['boundaries']) is not list:raise ValueError('axis magnetostatic boundaries must be a JSON list')
        return cls(AxisRecoilPartition.from_dict(data['partition']),data['current_density_phi_a_per_m2'],
...
@dataclass(eq=False)
class AxisRecoilSolution:
    case: AxisRecoilCase
    space: object
    stiffness: object
    current_load_a_m2: np.ndarray
    remanent_load_a_m2: np.ndarray
    ...
    def fields_in_cells(self,cell_indices,barycentric):
        ...
        magnetic=np.column_stack((-radius*gradient[:,1],2*regular+radius*gradient[:,0]));intensity=np.einsum('qij,qj->qi',self.case.partition.reluctivity_tensor_m_per_h[cells],magnetic-self.case.partition.remanent_b_t[cells])
    ...
    def probe_at(self,points_rz_m):
        ...
        return dict(points_rz_m=points.tolist(),cell_indices=cells.tolist(),barycentric=bary.tolist(),
            ...
            convention='a=Aphi/r[T], Aphi=r*a[Wb/m], Br=-r*d_z(a)[T], Bz=2*a+r*d_r(a)[T], H=nu(original cell)*(B-Brem)[A/m]',
...
def solve_axis_recoil(case):
    if type(case) is not AxisRecoilCase:raise ValueError('explicit AxisRecoilCase required')
    case=AxisRecoilCase.from_dict(case.to_dict())
    space,k,volume,remanent,report=axis_recoil_forms(case.partition,dict(case.current_density_phi_a_per_m2),case.element_order,quadrature_order=case.quadrature_order)
    ...
    for array in (coefficients,volume,remanent,boundary_load,free,*fixed_boundaries.values()):array.setflags(write=False)
    solution=AxisRecoilSolution(case,space,k,volume,remanent,boundary_load,coefficients,free,MappingProxyType(fixed_boundaries),report,residual)
    axis_recoil_quantities(solution)
    return solution


def axis_recoil_quantities(solution):
    """Full-3D constitutive potentials with B=0 and H=0 reference states."""
    if type(solution) is not AxisRecoilSolution:raise ValueError('AxisRecoilSolution required')
    ...
    constant_test=constant_scale=0.
    ...
    reaction=s.stiffness@s.aphi_over_r_t-s.current_load_a_m2-s.remanent_load_a_m2-s.boundary_load_a_m2
    ...
    source_work=float(s.aphi_over_r_t@s.current_load_a_m2);remanent_work=float(s.aphi_over_r_t@s.remanent_load_a_m2)
    boundary_work=float(s.aphi_over_r_t@s.boundary_load_a_m2);fixed_work=sum(b.value*fixed_reaction[b.id] for b in case.boundaries if b.kind=='fixed_aphi_over_r')
    ...
    # The constant-a test has B_test=(0,2), so the left side is integral 2Hz dV.
    # Remanence is already included in H; do not subtract its load a second time.
    moment_balance=constant_test-float(s.current_load_a_m2.sum())-float(s.boundary_load_a_m2.sum())-sum(fixed_reaction.values())
    moment_scale=constant_scale+float(abs(s.current_load_a_m2).sum())+float(abs(s.boundary_load_a_m2).sum())+sum(abs(v) for v in fixed_reaction.values())
    moment_scale=max(moment_scale,float(np.linalg.norm(s.remanent_load_a_m2,1)))
    ...
    if not np.isfinite([u,coupled,w0,ws,constant,work_error,potential_error,coupling_error,moment_error,divergence_error,current]).all() or min(u,ws,constant)<0 or max(work_error,potential_error,coupling_error,moment_error,divergence_error)>1e-9:
        raise ValueError('axis recoil work/constitutive/current-test identity or divergence-free flux is unresolved')
    return dict(b_quadratic_j=u,remanence_coupling_j=coupled,constitutive_potential_b0_j=w0,constitutive_potential_h0_j=ws,
        ...
        total_source_current_a=current,fixed_boundary_reaction_a_m2=fixed_reaction,fixed_boundary_original_reaction_a_m2=fixed_original,
        ...
        source_work_j=source_work,remanent_load_work_j=remanent_work,boundary_load_work_j=boundary_work,fixed_boundary_work_j=fixed_work,
        ...
        potential_convention='W0=integral(.5*B.nu.B-B.nu.Brem) references B=0 and can be negative; Ws=integral(.5*(B-Brem).nu.(B-Brem)) references H=0 and is nonnegative; full 3D dV=2*pi*r dr dz, unit J; both B derivatives are H',
        ...
        interpretation='finite axis-connected linear recoil domain; regular a has no constant gauge kernel; fixed-reference constitutive potentials are not absolute magnet internal energy or irreversible demagnetization; discrete identities do not bound original field error; no winding inductance or force is inferred')
