...
class PlanarTrackingRequest:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.mapping, (PolygonScaleMapping, PolygonSimilarityMapping, PolygonRemeshMapping, PolygonSimilarityRemeshMapping, PolygonAffineRemeshMapping)) and self.mapping != 'normalized_rectangle':
            raise ValueError('planar tracking requires normalized_rectangle or a declared PolygonScaleMapping or PolygonSimilarityMapping or PolygonRemeshMapping or PolygonSimilarityRemeshMapping or PolygonAffineRemeshMapping')
    ...
    def to_dict(self):
        return dict(format='superfish_ng_planar_tracking_request', tracking_version=6 if isinstance(self.mapping, PolygonAffineRemeshMapping) else 5 if isinstance(self.mapping, PolygonSimilarityRemeshMapping) else 4 if isinstance(self.mapping, PolygonRemeshMapping) else 3 if isinstance(self.mapping, PolygonSimilarityMapping) else 2 if isinstance(self.mapping, PolygonScaleMapping) else 1,
                    mapping=self.mapping.to_dict() if isinstance(self.mapping, (PolygonScaleMapping, PolygonSimilarityMapping, PolygonRemeshMapping, PolygonSimilarityRemeshMapping, PolygonAffineRemeshMapping)) else self.mapping, previous_mode_count=self.previous_mode_count,
    ...
    def from_dict(cls, data):
        ...
        if (data['format'] != 'superfish_ng_planar_tracking_request'
                or type(data['tracking_version']) is not int or data['tracking_version'] not in (1, 2, 3, 4, 5, 6)):
            raise ValueError('expected superfish_ng_planar_tracking_request tracking_version 1, 2, 3, 4, 5 or 6')
        mapping = (PolygonAffineRemeshMapping.from_dict(data['mapping']) if data['tracking_version']==6
...
def track_planar_modes(previous, current, request):
    ...
    polygon = isinstance(request.mapping, (PolygonScaleMapping, PolygonSimilarityMapping, PolygonRemeshMapping,
                                           PolygonSimilarityRemeshMapping, PolygonAffineRemeshMapping))
    if polygon:
    ...
    if polygon:
        overlay = (polygon_affine_remesh_overlay if affine else polygon_similarity_remesh_overlay if composed else polygon_remesh_overlay if remesh else polygon_similarity_overlay if similarity else polygon_scale_overlay)(previous.case.mesh, current.case.mesh, request.mapping,
        ...
        overlay = rectangle_tracking_overlay((previous.case.nx, previous.case.ny), (current.case.nx, current.case.ny),
                                             max_overlay_triangles=controls.max_overlay_triangles)
    rotation = (request.mapping.current_to_previous_linear if affine
    ...
    resolutions = [(similarity_spectral_resolution if similarity or remesh or composed or affine else polygon_spectral_resolution if polygon else rectangle_spectral_resolution)(s, max_refined_triangles=controls.max_refined_triangles)
    ...
    result = track_sampled_mode_subspaces(*features, np.ones(len(features[0])),
        ...
        comparison_description=('all physical peak E components under the declared invertible proper or orientation-reversing affine map with an independently meshed interior on the exact transformed boundary; current transverse vectors transported by the adjugate of the effective linear map; exact intersections of the two element unions' if affine else 'all physical peak E components under the declared proper similarity with an independently meshed interior on the exact transformed boundary; current vectors rotated into the previous frame; exact intersections of the two element unions' if composed else 'all physical peak E components under the declared proper similarity; current vectors in the previous frame; previous original physical xy area' if similarity else 'all physical peak E components composed with the declared origin polygon scale; verified nested original elements; previous physical xy area' if polygon else 'all physical peak E components pulled back by x=a*rho, y=b*eta; exact rectangle triangle intersections; reference area d_rho d_eta'),
    ...
    result.update(format='superfish_ng_planar_tracking_result', result_version=6 if affine else 5 if composed else 4 if remesh else 3 if similarity else 2 if polygon else 1, request=request.to_dict(),
    ...
    if polygon and not remesh and not composed and not affine:
