...
from .curved_hphi_tuning_jobs import _owned_input,verify_curved_hphi_tune_job,_load
from .curved_hphi_tuning_saved import _snapshot_trial, read_curved_hphi_tune, replay_curved_hphi_tune
...
def _checkpoint_directory(manager, identifier):
    ...
    if state.get("kind") != "curved_hphi_tune" or state.get("status") not in (
        "complete",
        "cancelled",
        "interrupted",
        "failed",
...
def _submitted_data(directory):
    request_path = directory / "curved-hphi-tune-request.json"
...
def _saved_checkpoint(manager, identifier, index):
    ...
    if state.get("input_sha256") is not None and state["input_sha256"] != {
        "curved-hphi-tune-request.json": submitted_hash
    }:
    ...
    result = read_curved_hphi_tune(path)
...
def curved_hphi_tuning_response(manager, action, data):
    ...
    if action in ("hphi-normalize-tune", "hphi-start-tune"):
        ...
        validate_curved_hphi_tune(request)
        if action == "hphi-normalize-tune":
            return parse_json(_canonical(request))
        return {
            "id": manager.start_curved_hphi_tune(
    ...
    if action == "hphi-open-tune-checkpoint":
        ...
        if state.get("status") != "complete" or state.get("kind") != "curved_hphi_tune":
            raise ValueError(
                "select a completed Hphi tune job; partial checkpoints can be opened separately"
            )
        result = verify_curved_hphi_tune_job(directory,state,_load(directory / "manifest.json"))
    else:
        document = data["document"]
        if isinstance(document, str):
            document = parse_json(document)
        result = replay_curved_hphi_tune(document)
