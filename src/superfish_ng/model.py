...
def capabilities():
    ...
    return {'capabilities_version': 1,
            'planar_cutoff': {'case_format': 'superfish_ng_planar_case', 'schema_version': 1, 'schema_versions': [1,2],
                ...
                'commands': ['solve-planar','replay-planar','probe-planar','execute-planar-project','plot-planar','execute-planar-study','replay-planar-study','execute-planar-convergence','replay-planar-convergence'],
                ...
                'job_kind': 'planar_solve', 'job_operations': ['start_planar','start_planar_study','start_planar_convergence','import_planar_result','cancel','status','list'],
                'limits': 'vacuum single PEC boundary; straight P1/P2; no holes, curved geometry, materials, propagation, planar mode tracking or surface-peak accuracy guarantee'}, 'case_schema_versions': [1, 2, 3],
