...
def _from_replayed_construction(construction, *, schema_version=13):
    ...
    if construction['schema_version'] not in (5,6,7,8,9):return None
...
def diagnose_construction(document):
    ...
    if result is None:raise ValueError('offset diagnosis requires a saved version 5, 6, 7, 8 or 9 fillet construction')
    return result
...
def replay_construction_diagnosis(document):
    ...
    if expected is None:raise ValueError('offset diagnosis requires a saved version 5, 6, 7, 8 or 9 fillet construction')
