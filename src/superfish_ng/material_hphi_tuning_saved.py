...
"""Owned original material tuning checkpoints, physical replay and fresh-output resume."""
from copy import deepcopy
from pathlib import Path
from .config import integer,keys
from .project import parse_json
from .hphi_project import HphiProject
from .material_hphi import solve_material_hphi
from .material_hphi_saved import save_material_hphi_run,read_material_hphi_run
from .material_hphi_shape_tuning import MaterialHphiShapeLaw
from .material_hphi_tuning import validate_material_hphi_tune,material_hphi_tune_decision,_trial,_assess_trial
...
SCOPE='original_material_straight_hphi_tuning'


def _checkpoint(request,runs,sources,trials):
    decision=material_hphi_tune_decision(request,trials)
    return dict(format='superfish_ng_material_hphi_tune_checkpoint',schema_version=1,
...
def _stable(runs,sources):
    if [_snapshot_trial(run) for run in runs]!=sources:
        raise ValueError('material tune owned sources changed during verification or execution')


def _request_copy(base,request):
    if base is not None:
        if not (Path(base)/'request.json').is_file():raise ValueError('material tune owner requires request.json')
...
def _recompute(request,runs):
    ...
    request=deepcopy(request);project=validate_material_hphi_tune(request)
    law=MaterialHphiShapeLaw.from_dict(request['shape_law'])
    runs=_resolve_trial_runs(list(runs))
    if len(runs)>request['max_trials']+1:raise ValueError('material tune paths exceed search plus final trial budget')
    ...
    for run in runs:
        ...
        if owned.to_dict()!=record.project.to_dict():raise ValueError('material tune owned Project differs from declared trial')
        solution=read_material_hphi_run(Path(run)/'solution')
...
def _verified(document,base_directory=None):
    ...
    keys(document,CHECKPOINT_KEYS,CHECKPOINT_KEYS,'material Hphi tune checkpoint')
    if (document['format']!='superfish_ng_material_hphi_tune_checkpoint' or type(document['schema_version']) is not int
            or document['schema_version']!=1 or document['scope']!=SCOPE):
        raise ValueError('expected material Hphi tune checkpoint schema_version 1')
    if type(document['can_resume']) is not bool or type(document['trials']) is not list or type(document['trial_sources_sha256']) is not list:
        raise ValueError('material tune checkpoint requires boolean can_resume and list trials/sources')
    ...
    if _canonical(document)!=_canonical(expected):raise ValueError('material tune checkpoint differs from owned native replay')
    _request_copy(base,document['request']);_stable(runs,expected['trial_sources_sha256'])
    return expected,records,solutions


def replay_material_hphi_tune(document,*,base_directory=None):
...
def _read(path):
    ...
    if path!=path.resolve() or path.is_symlink() or not path.is_file():
        raise ValueError('material tune checkpoint must be a regular file without symbolic-link components')
...
def read_material_hphi_tune(path):return _read(path)[0]
...
def execute_material_hphi_tune(request,directory,*,max_new_trials=None,checkpoint=None):
    ...
    """
    request=deepcopy(request);project=validate_material_hphi_tune(request)
    request=parse_json(_canonical(request));law=MaterialHphiShapeLaw.from_dict(request['shape_law'])
    ...
    if checkpoint is None:previous=_checkpoint(request,[],[],[])
        ...
        if _canonical(previous['request'])!=_canonical(request):raise ValueError('material tune resume request differs from checkpoint')
        if previous['status']!='PAUSED' or not previous['can_resume']:raise ValueError('only verified PAUSED material tuning can resume')
    ...
    while previous['status']=='PAUSED' and (max_new_trials is None or count<max_new_trials):
        ...
        try:
            ...
            solution=solve_material_hphi(record.project.case)
            save_material_hphi_run(record.project.case,solution,run/'solution')
            source=_snapshot_trial(run);owned=HphiProject.load(run/'project.json')
            if owned.to_dict()!=record.project.to_dict():raise ValueError('material tune Project changed during execution')
            restored=read_material_hphi_run(run/'solution')
            ...
            if implementation!=_implementation_hashes():raise RuntimeError('implementation changed during material tune execution')
            result=_checkpoint(request,new_runs,new_sources,trials+[trial]);_write_checkpoint(directory,result)
        except Exception as error:
            _write_json(directory/f'failure-{index+1:03d}.json',dict(document_type='material_hphi_tune_failure',request=request,
