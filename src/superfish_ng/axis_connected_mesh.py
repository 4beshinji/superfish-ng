...
"""Connected straight (r,z) triangulations touching one axis interval, with PEC holes.
...
def _rz(value, name):
    ...
    if np.any(result[:,0] < 0):
        raise ValueError(f'{name} requires nonnegative radius; the vacuum must connect to one explicit axis interval')
    return result
...
class AxisConnectedMesh:
    ...
    axis_edges: np.ndarray = field(init=False)
    axis_nodes: np.ndarray = field(init=False)
    axis_interval_m: tuple = field(init=False)
    ...
    def __post_init__(self):
        ...
        axis_vertices = outer[:,0] == 0
        axis_segments = axis_vertices & np.roll(axis_vertices,-1)
        if np.count_nonzero(axis_vertices)<2 or np.count_nonzero(axis_segments)!=np.count_nonzero(axis_vertices)-1:
            raise ValueError('axis-connected outer contour requires one continuous nonzero axis interval')
        if np.any((np.roll(outer[:,1],-1)-outer[:,1])[axis_segments]>=0):
            raise ValueError('counterclockwise outer contour must follow its axis boundary toward decreasing z')
        if any(np.any(h[:,0]<=0) for h in holes):
            raise ValueError('PEC holes must have strictly positive radius and cannot touch the axis')
        ...
        axis_by_segment = np.concatenate((axis_segments,*[np.zeros(len(h),dtype=bool) for h in holes]))
        if (not np.isfinite(surface).all() or np.any(surface < 0)
                or not np.array_equal(surface==0,axis_by_segment)):
            raise ValueError('meridional surface area is outside positive SI arithmetic')
        axis_edges=np.flatnonzero(np.all(points[edges,0]==0,axis=1))
        axis_nodes=np.unique(edges[axis_edges]);axis_nodes=axis_nodes[np.argsort(points[axis_nodes,1])]
        boundary_tags=np.full(len(edges),'pec',dtype='U20');boundary_tags[axis_edges]='axis'
        values = dict(boundary_tags=boundary_tags,axis_edges=axis_edges,axis_nodes=axis_nodes,
                      axis_interval_m=(float(points[axis_nodes[0],1]),float(points[axis_nodes[-1],1])),outer_rz_m=outer,holes_rz_m=holes,points_rz_m=points,triangles=cells,
    ...
    def to_dict(self):
        return dict(format='superfish_ng_axis_connected_mesh',schema_version=1,coordinates='r,z in metres',boundary='axis_and_pec',
    ...
    def from_dict(cls,data):
        ...
        if (data['format'] != 'superfish_ng_axis_connected_mesh' or type(data['schema_version']) is not int
            or data['schema_version'] != 1 or data['coordinates'] != 'r,z in metres' or data['boundary'] != 'axis_and_pec'):
            raise ValueError('expected meridional mesh schema 1, r,z in metres and axis_and_pec')
