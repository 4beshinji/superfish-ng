...
def track_curved_piecewise_remesh_modes(previous,current,previous_ids,*,mapping,sample_order,comparison_meshes,**controls):
    ...
    if any(not isinstance(s,(CurvedSolution,TESolution)) or s.case.geometry_order!=2 for s in solutions):
        raise ValueError('curved piecewise_remesh requires two native curved solutions')
    ...
    reflected=bool(sector is not None and sector['reflected'])
    if not all(te) and any(s.reflection_source_case is not None for s in solutions):
        raise ValueError('TM reflected piecewise comparison is unsupported')
    copies=2 if reflected else 1
    field='Ephi_V_per_m' if all(te) else 'Hphi_A_per_m'
    spaces=[];boundaries=[];projects=[];comparison_cases=[];edge_checks=[]
    for solution,document in zip(solutions,comparison_meshes):
        ...
        limit=MAX_SAMPLES//(copies*sample_order**2)
        ...
        source_case=solution.reflection_source_case if reflected else solution.case
        budget=source_case.contour_mesh
        case=replace(source_case,curved_refinement_levels=levels,curved_refinement_steps=steps,
        ...
        boundary_case,boundary_space=case,space
        if reflected:
            ...
            extension=reflect_curved_space(case,space,coefficient_parity=parity)
            boundary_case,boundary_space=reflected_case(case,extension),extension.space
        boundaries.append(compare_quadratic_space_boundaries(boundary_case,boundary_space,solution.case,solution.space))
        edge_checks.append(dict(boundary_space.edge_check))
        spaces.append(space)
    ...
    if comparison_meshes[0]['schema_version']==5:
        ...
        overlay=build_curved_reference_partition(*projects,reference_vertices=[m['reference_vertices'] for m in comparison_meshes],
            max_pair_tests=comparison_meshes[0]['max_pair_tests'],max_triangles=limit)
        ...
        correspondence=infer_curved_comparison_correspondence(comparison_cases,spaces,
    ...
    cells=len(overlay.report['triangles']) if overlay is not None else len(first.geometry.cell_nodes);count=copies*cells*sample_order**2
    ...
    weights=np.tile([w for _,w in rule],copies*cells)
    ...
    for side,(solution,space) in enumerate(zip(solutions,spaces)):
        ...
        det=np.concatenate([item['determinant_m2'] for item in data])
        if reflected:
            source_case=comparison_cases[side]
            plane=0. if source_case.z_min!='pec' else source_case.length
            shift=source_case.length if plane==0. else 0.
            mirror=points.copy();mirror[:,1]=2*plane-points[:,1]+shift
            points=points.copy();points[:,1]+=shift
            points=np.concatenate((points,mirror));det=np.tile(det,2)
        r=points[:,0]
    ...
    report['physical_mapping']=dict(name=mapping,comparison_geometry_order=2,sample_order=sample_order,sample_count=count,
        comparison_triangle_count=copies*cells,solver_triangle_counts=[len(s.space.geometry.cell_nodes) for s in solutions],
        ...
        comparison_edge_checks=edge_checks,field=field,
    ...
    if reflected:
        report['physical_mapping']['reflection_mapping']=dict(declaration_domain='source half-domain',evaluation_domain='both lobes of native reflected fields',source_comparison_triangle_count=cells,construction='source chart and its mirror; positive absolute Jacobian measure; original half shifted for z_min symmetry')
    if automatic:report['physical_mapping']['source_numbering_correspondence' if reflected else 'numbering_correspondence']=correspondence
    if overlay is not None:report['physical_mapping']['source_common_reference_partition' if reflected else 'common_reference_partition']=overlay.report
    return report
