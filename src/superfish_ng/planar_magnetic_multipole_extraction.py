...
def extract_planar_magnetic_multipoles(solution,frame,maximum_order=8,angular_samples=128):
    if type(solution) is not PlanarMagnetostaticSolution:raise ValueError('multipole extraction currently requires a linear scalar PlanarMagnetostaticSolution')
    ...
    case=solution.case.to_dict();fresh=solve_planar_magnetostatic(solution.case)
    for name in ('az_relative_to_reference_wb_per_m','az_wb_per_m'):
        if not np.array_equal(getattr(solution,name),getattr(fresh,name)):raise ValueError('multipole source coefficients disagree with actual linear FEM replay')
    ...
    series=PlanarMagneticMultipoleSeries(frame,coefficients[0].real,coefficients[0].imag,'Original verified linear planar FEM; Case SHA256 '+source_hash+'; relative Az float64 SHA256 '+field_hash)
    return dict(format='superfish_ng_planar_magnetic_multipole_extraction',schema_version=1,series=series.to_dict(),source_case_sha256=source_hash,source_relative_az_sha256=field_hash,
