...
"""Explicit positive magnetic materials on straight axis-connected domains."""
...
class AxisMagneticPartition:
    ...
    def __post_init__(self):
        if type(self.mesh) is not AxisConnectedMesh:
            raise ValueError('axis magnetic materials require a straight AxisConnectedMesh; off-axis gauge and curved geometry are unsupported')
        for name,cls in (('materials',LinearMagneticMaterial),('regions',MagneticRegion)):
            ...
            if not isinstance(values,(list,tuple)) or not values or any(type(v) is not cls for v in values):
                raise ValueError('axis magnetic '+name+' requires explicit '+cls.__name__+' values')
        ...
        except ValueError as exc:raise ValueError(str(exc).replace('RF material','axis magnetic material')) from exc
    ...
    def to_dict(self):
        return dict(format='superfish_ng_axis_magnetic_partition',schema_version=1,coordinates='axisymmetric_rz',
    ...
    def from_dict(cls,data):
        names=['format','schema_version','coordinates','geometry','materials','regions'];keys(data,names,names,'axis magnetic partition')
        if (data['format']!='superfish_ng_axis_magnetic_partition' or type(data['schema_version']) is not int
            or data['schema_version']!=1 or data['coordinates']!='axisymmetric_rz'):
            raise ValueError('expected superfish_ng_axis_magnetic_partition version 1, axisymmetric_rz coordinates')
        geometry=data['geometry'];names=['type','outer_rz_m','holes_rz_m','points_rz_m','triangles'];keys(geometry,names,names,'axis magnetic geometry')
        if geometry['type']!='explicit_straight_axis_connected':
            raise ValueError('axis magnetic partition requires explicit straight axis-connected geometry')
        if type(data['materials']) is not list or type(data['regions']) is not list:raise ValueError('axis magnetic materials and regions must be JSON lists')
        mesh=AxisConnectedMesh(**{name:geometry[name] for name in names[1:]})
