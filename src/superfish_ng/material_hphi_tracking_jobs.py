...
def _check_sources(document, directory):
    ...
    if document['format'] != 'superfish_ng_material_hphi_tracking_sources' or type(document['sources_version']) is not int or document['sources_version'] != 1:
...
def is_material_hphi_tracking(directory, state, manifest):
    ...
    for name in ('tracking.json', 'tracking-results.json'):
        if (directory/name).is_file():
            ...
            if isinstance(data, dict) and data.get('format') in ('superfish_ng_material_hphi_tracking_request', 'superfish_ng_material_hphi_tracking_result'):
                return True
    return False


def verify_material_hphi_tracking(directory, state, manifest):
    ...
    if state.get('status') != 'complete' or state.get('kind') != KIND or manifest.get('kind') != KIND:
        raise ValueError('hphi tracking state and manifest require complete kind=material_hphi_tracking')
    ...
    request = MaterialHphiTrackingRequest.load(directory/'tracking.json')
    solutions = [read_hphi_run(directory/side/'solution') for side in ('previous', 'current')]
    expected = track_material_hphi_modes(*solutions, request)
...
def read_material_hphi_tracking(directory):
    directory = Path(directory)
    return verify_material_hphi_tracking(directory, _load(directory/'job.json'), _load(directory/'manifest.json'))


def _prepare(previous, current, request, directory):
    if not isinstance(request, MaterialHphiTrackingRequest): raise ValueError('expected MaterialHphiTrackingRequest')
    request = MaterialHphiTrackingRequest.from_dict(request.to_dict())
    sources = [_source(path) for path in (previous, current)]
    from .material_hphi import MaterialHphiCase
    from .material_hphi_comparison import material_hphi_overlay
    from .hphi_spectral_resolution import _maximum_edge
    import numpy as np
    controls=request.controls
    for side,source,count in zip(('previous','current'),sources,(request.previous_mode_count,request.current_mode_count)):
        case=source[1].case
        if type(case) is not MaterialHphiCase:raise ValueError('material tracking requires original material Hphi native sources')
        if count>=case.modes:raise ValueError('tracked band requires a computed upper guard mode')
        if case.modes>controls.max_gram_modes:raise ValueError('material tracking exceeds max_gram_modes')
        resolution=getattr(request,side+'_resolution');old=case.partition
        if resolution.previous_partition.to_dict()!=old.to_dict() or getattr(request.comparison,side+'_partition').to_dict()!=old.to_dict():
            raise ValueError('material tracking declaration differs from its original native partition')
        fine=resolution.current_partition;order=getattr(request,side+'_comparison_order')
        if (order<case.element_order or len(fine.mesh.triangles)<=len(old.mesh.triangles)
                or _maximum_edge(fine.mesh)>=_maximum_edge(old.mesh)):
            raise ValueError('material comparison requires at least original order, more cells and smaller maximum edge')
        for partition,p in ((old,case.element_order),(fine,order)):
            mesh=partition.mesh;dofs=len(mesh.points_rz_m)
            if p==2:dofs+=len(np.unique(np.sort(mesh.triangles[:,[[0,1],[1,2],[2,0]]].reshape(-1,2),axis=1),axis=0))
            if dofs>controls.max_dofs:raise ValueError('material tracking exceeds max_dofs')
    for comparison in (request.previous_resolution,request.current_resolution,request.comparison):
        overlay=material_hphi_overlay(comparison,max_candidate_tests=controls.max_candidate_tests,
            max_overlay_triangles=controls.max_overlay_triangles,max_interface_tests=request.max_interface_tests,
            max_interface_pieces=request.max_interface_pieces)
        if len(overlay.overlay.determinants)*(controls.quadrature_order+10)**2>request.max_sample_points:
            raise ValueError('material tracking exceeds max_sample_points')
    source_document = dict(format='superfish_ng_material_hphi_tracking_sources', sources_version=1,
...
def execute_prepared_material_hphi_tracking(directory):
    ...
    if directory.is_symlink() or state.get('status') != 'queued' or state.get('kind') != KIND:
        raise ValueError('hphi tracking worker requires a fresh queued material_hphi_tracking job')
    ...
    try:
        ...
        _check_sources(_load(directory/'sources.json'), directory)
        result = track_material_hphi_modes(*(read_hphi_run(directory/side/'solution') for side in ('previous', 'current')),
                                    MaterialHphiTrackingRequest.load(directory/'tracking.json'))
        ...
        result = read_material_hphi_tracking(directory)
        if inputs != _input_hashes(directory) or implementation != _implementation_hashes():
...
def execute_material_hphi_tracking(previous, current, request, directory):
    directory = Path(directory); _prepare(previous, current, request, directory)
    return execute_prepared_material_hphi_tracking(directory)


def start_material_hphi_tracking(manager, previous, current, request):
    with manager.lock:
        ...
        try:
            with (directory/'log.txt').open('x') as log:
                manager.processes[identifier] = subprocess.Popen(
                    [sys.executable, '-m', 'superfish_ng.material_hphi_tracking_jobs', str(directory)],
...
if __name__ == '__main__':
    if len(sys.argv) != 2: raise SystemExit('usage: python -m superfish_ng.material_hphi_tracking_jobs PREPARED_DIRECTORY')
    execute_prepared_material_hphi_tracking(sys.argv[1])
