...
from .curved_hphi_tracking_history import _current_groups, _native_part, _verify_link
from .curved_hphi_identity_recovery import CurvedHphiIdentityRecoveryRequest, recover_curved_hphi_modes


def verify_recovered_curved_hphi_history_steps(directories, request):
    ...
    for index,path in enumerate(directories):
        ...
        if index in placements:
            recovery=CurvedHphiIdentityRecoveryRequest.from_dict(placements[index])
            ...
            native_request=CurvedHphiTrackingRequest.load(path/'tracking.json')
            event=recover_curved_hphi_modes(read_hphi_run(anchor['location']),
    ...
    return dict(format='superfish_ng_curved_hphi_tracking_history_result',history_version=2,
