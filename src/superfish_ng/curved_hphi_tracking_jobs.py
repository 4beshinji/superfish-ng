...
def _check_sources(document, directory):
    ...
    if document['format'] != 'superfish_ng_curved_hphi_tracking_sources' or type(document['sources_version']) is not int or document['sources_version'] != 1:
...
def is_curved_hphi_tracking(directory, state, manifest):
    ...
    for name in ('tracking.json', 'tracking-results.json'):
        if (directory/name).is_file():
            ...
            if isinstance(data, dict) and data.get('format') in ('superfish_ng_curved_hphi_tracking_request', 'superfish_ng_curved_hphi_tracking_result'):
                return True
    return False


def verify_curved_hphi_tracking(directory, state, manifest):
    ...
    if state.get('status') != 'complete' or state.get('kind') != KIND or manifest.get('kind') != KIND:
        raise ValueError('hphi tracking state and manifest require complete kind=curved_hphi_tracking')
    ...
    request = CurvedHphiTrackingRequest.load(directory/'tracking.json')
    solutions = [read_hphi_run(directory/side/'solution') for side in ('previous', 'current')]
    expected = track_curved_hphi_modes(*solutions, request)
...
def read_curved_hphi_tracking(directory):
    directory = Path(directory)
    return verify_curved_hphi_tracking(directory, _load(directory/'job.json'), _load(directory/'manifest.json'))


def _prepare(previous, current, request, directory):
    if not isinstance(request, CurvedHphiTrackingRequest): raise ValueError('expected CurvedHphiTrackingRequest')
    request = CurvedHphiTrackingRequest.from_dict(request.to_dict())
    sources = [_source(path) for path in (previous, current)]
    from .curved_hphi import CurvedHphiCase
    from .curved_hphi_comparison import CurvedHphiComparisonDomain,build_curved_hphi_comparison,CurvedHphiComparisonBudgetExceeded
    from .curved_hphi_spectral_resolution import _maximum_cell_diameter
    ...
    for side,source,count in zip(('previous','current'),sources,(request.previous_mode_count,request.current_mode_count)):
        case=source[1].case
        if type(case) is not CurvedHphiCase:raise ValueError('curved tracking requires original curved Hphi native sources')
        if count>=case.modes:raise ValueError('tracked band requires a computed upper guard mode')
        if case.modes>controls.max_gram_modes:raise ValueError('curved tracking exceeds max_gram_modes')
        old=case.geometry;fine=getattr(request,side+'_comparison_geometry');order=getattr(request,side+'_comparison_order')
        if (order<case.element_order or len(fine.cell_nodes)<=len(old.cell_nodes)
                or _maximum_cell_diameter(fine)>=_maximum_cell_diameter(old)):
            raise ValueError('curved comparison requires at least original order, more cells and smaller whole-P2 diameter')
        for geometry,p in ((old,case.element_order),(fine,order)):
            dofs=len(geometry.points_rz_m) if p==2 else len(geometry.base_mesh.points_rz_m)
            if dofs>controls.max_dofs:raise CurvedHphiComparisonBudgetExceeded('curved tracking exceeds max_dofs')
        reference=getattr(request.domain,side)
        domain=CurvedHphiComparisonDomain(reference,reference,'same_vacuum',restriction_policy=request.domain.restriction_policy)
        geometries.append((old,fine,domain,getattr(request,side+'_cells'),getattr(request,side+'_comparison_cells')))
    geometries.append((sources[0][1].case.geometry,sources[1][1].case.geometry,request.domain,request.previous_cells,request.current_cells))
    for old,fine,domain,old_cells,fine_cells in geometries:
        overlay=build_curved_hphi_comparison(old,fine,domain,previous_cells=old_cells,current_cells=fine_cells,
            max_pair_tests=controls.max_candidate_tests,max_triangles=controls.max_overlay_triangles)
        if len(overlay.triangles)*len(list(triangle_quadrature(controls.quadrature_order+10)))>request.max_sample_points:
            raise CurvedHphiComparisonBudgetExceeded('curved tracking exceeds max_sample_points')
    source_document = dict(format='superfish_ng_curved_hphi_tracking_sources', sources_version=1,
...
def execute_prepared_curved_hphi_tracking(directory):
    ...
    if directory.is_symlink() or state.get('status') != 'queued' or state.get('kind') != KIND:
        raise ValueError('hphi tracking worker requires a fresh queued curved_hphi_tracking job')
    ...
    try:
        ...
        _check_sources(_load(directory/'sources.json'), directory)
        result = track_curved_hphi_modes(*(read_hphi_run(directory/side/'solution') for side in ('previous', 'current')),
                                    CurvedHphiTrackingRequest.load(directory/'tracking.json'))
        ...
        result = read_curved_hphi_tracking(directory)
        if inputs != _input_hashes(directory) or implementation != _implementation_hashes():
...
def execute_curved_hphi_tracking(previous, current, request, directory):
    directory = Path(directory); _prepare(previous, current, request, directory)
    return execute_prepared_curved_hphi_tracking(directory)


def start_curved_hphi_tracking(manager, previous, current, request):
    with manager.lock:
        ...
        try:
            with (directory/'log.txt').open('x') as log:
                manager.processes[identifier] = subprocess.Popen(
                    [sys.executable, '-m', 'superfish_ng.curved_hphi_tracking_jobs', str(directory)],
...
if __name__ == '__main__':
    if len(sys.argv) != 2: raise SystemExit('usage: python -m superfish_ng.curved_hphi_tracking_jobs PREPARED_DIRECTORY')
    execute_prepared_curved_hphi_tracking(sys.argv[1])
