...
def load_document(text):
    """Read a project or legacy Case; reject duplicate keys at every depth."""
    def pairs(items):
        result = {}
        for key, value in items:
            ...
            result[key] = value
        return result
    return Project.from_dict(json.loads(text, object_pairs_hook=pairs))
...
def assemble_geometry(sections):
    ...
    for section in sections:
        ...
        if len(points) + len(local.profile)*count > 100000:
            raise ValueError('assembly exceeds 100000 editing vertices; reduce repetitions')
        for _ in range(count):
            ...
            arcs.extend({'end_index':base+i,'radius_m':r,'direction':d} for i,r,d in local.arcs)
            offset += local.length
    geometry = {'type': 'arc_profile' if arcs else 'stepped_profile', 'points_zr_m': points}
    if arcs:
        geometry.update(arcs=arcs, chord_tolerance_m=min(tolerances))
    return Case.from_dict({'schema_version':2,'geometry':geometry}).to_dict()['geometry']
...
class Project:
    ...
    def from_dict(cls, data):
        ...
        keys(data, ['project_version','case','sections','reflect_full','display_length_unit'],
             ['project_version','case'], 'project')
        ...
        if 'sections' in data:
            ...
            if expanded != case.to_dict()['geometry']:
                raise ValueError('case geometry differs from expanded sections; regenerate the case')
    ...
    def from_sections(cls, template, sections, **options):
        ...
        return cls.from_dict({'project_version':1,'case':raw,'sections':sections, **options})
    ...
    def to_dict(self):
        data = {'project_version':1,'case':self.case.to_dict(),
                'reflect_full':self.reflect_full,'display_length_unit':self.display_length_unit}
