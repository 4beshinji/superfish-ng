...
from .material_hphi_tracking_history import _current_groups, _native_part, _verify_link
from .material_hphi_identity_recovery import MaterialHphiIdentityRecoveryRequest, recover_material_hphi_modes


def verify_recovered_material_hphi_history_steps(directories, request):
    ...
    for index,path in enumerate(directories):
        ...
        if index in placements:
            recovery=MaterialHphiIdentityRecoveryRequest.from_dict(placements[index])
            ...
            native_request=MaterialHphiTrackingRequest.load(path/'tracking.json')
            event=recover_material_hphi_modes(read_hphi_run(anchor['location']),
    ...
    return dict(format='superfish_ng_material_hphi_tracking_history_result',history_version=2,
