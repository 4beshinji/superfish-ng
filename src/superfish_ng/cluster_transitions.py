...
def transition_partition(old,new,identities,left,right,minimum_link):
    ...
    for a,b in components:
        ...
        if one_to_many and len(previous)==len(current):
            ids=sorted(x for i in a for x in identities[i])
            old_result.append((previous,ids));new_result.append(current)
            events.append(dict(kind='MERGE' if len(a)>1 else 'SPLIT',previous_indices=[k+1 for k in previous],
    ...
    return [x[0] for x in old_result],new_result,[x[1] for x in old_result],dict(
        ...
        scope='links propose only one-to-many or many-to-one equal-dimension unions; full worst-principal-overlap and assignment tests still required; individual IDs are not recovered after a split')
