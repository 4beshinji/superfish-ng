...
"""Bracketed original material Hphi FEM tuning with explicit identity recovery."""
...
from .material_hphi_tune_trials import build_material_hphi_tune_trial,material_hphi_trial_comparison,_budget
from .material_hphi_tracking import track_material_hphi_modes
...
def validate_material_hphi_tune(request):
    ...
    names=('format','schema_version','project','parameter','shape_law','bounds','target_hz',
        ...
        'refinement_levels','max_triangles','max_dofs','mesh_frequency_tolerance_hz','max_sample_points','max_interface_tests','max_interface_pieces','identity_recovery')
    keys(request,names,names,'material Hphi tune')
    if (request['format']!='superfish_ng_material_hphi_tune' or type(request['schema_version']) is not int
            or request['schema_version']!=1 or request['parameter']!='deformation'):
        raise ValueError('expected material Hphi tune schema_version 1 with dimensionless deformation')
    project=HphiProject.from_dict(request['project'])
    if type(project.case) is not MaterialHphiCase:raise ValueError('material tune requires original MaterialHphiCase Project')
    law=MaterialHphiShapeLaw.from_dict(request['shape_law'])
    bounds=request['bounds']
    if type(bounds) is not list or len(bounds)!=2:raise ValueError('material tune bounds require two increasing positive values')
    for value in bounds:positive(value,'material tune bound')
    if not bounds[0]<bounds[1]:raise ValueError('material tune bounds must increase')
    ...
    for name in ('refinement_levels','max_triangles','max_dofs','max_sample_points','max_interface_tests','max_interface_pieces'):integer(request[name],name)
    ...
    partition=project.case.partition;levels=request['refinement_levels']
    for control in controls:
        if project.case.modes>min(control.max_gram_modes,256):
            raise ValueError('material tune spectrum exceeds max_gram_modes or scalar projection column budget 256')
        _budget(partition,project.case.element_order,levels,request['max_triangles'],request['max_dofs'],control.max_candidate_tests)
        _budget(partition,2,levels+1,control.max_overlay_triangles,control.max_dofs,control.max_candidate_tests)
        samples=len(list(triangle_quadrature(control.quadrature_order+10)))
        if samples*len(partition.mesh.triangles)*4**(levels+1)>request['max_sample_points']:
            raise ValueError('material tune final comparison exceeds max_sample_points')
    for value in bounds:law.apply(project,value,max_candidate_tests=request['controls']['max_candidate_tests'],
        max_overlay_triangles=request['max_triangles'],max_interface_tests=request['max_interface_tests'],
        max_interface_pieces=request['max_interface_pieces'])
    return project


def read_material_hphi_tune_request(path):
    request=parse_json(Path(path).read_text(encoding='utf-8'));validate_material_hphi_tune(request)
    return deepcopy(request)


def _trial(request,project,law,value,phase):
    return build_material_hphi_tune_trial(project,law,value,phase=phase,refinement_levels=request['refinement_levels'],
        max_triangles=request['max_triangles'],max_dofs=request['max_dofs'],max_candidate_tests=request['controls']['max_candidate_tests'],
        max_interface_tests=request['max_interface_tests'],max_interface_pieces=request['max_interface_pieces'])


def trial_material_hphi_project(request,value,phase):
    project=validate_material_hphi_tune(request);_json_types(value);positive(value,'material tune trial value')
    if not request['bounds'][0]<=value<=request['bounds'][1]:raise ValueError('material tune trial value is outside bounds')
    return _trial(request,project,MaterialHphiShapeLaw.from_dict(request['shape_law']),value,phase).project


def material_hphi_tune_decision(request,trials):
...
def _compare(request,previous,current,previous_solution,current_solution,ids,controls):
    declaration=material_hphi_trial_comparison(previous,current,previous_mode_ids=ids,
        ...
        max_interface_tests=request['max_interface_tests'],max_interface_pieces=request['max_interface_pieces'])
    return track_material_hphi_modes(previous_solution,current_solution,declaration)
...
def _recover_trial(request,trials,records,solutions,current,solution,trial,inherited):
    ...
    if inherited['status']!='PASS' or inherited['individual_ids_complete']:
        raise ValueError('material recovery requires a verified unresolved individual ID set')
    ...
    event=dict(format='superfish_ng_material_hphi_tune_identity_recovery',recovery_version=1,
        ...
        scope='earlier original material E/H reidentification inside inherited ID sets; anchor differs from search/refinement parent; no continuous-branch certificate')
...
def _assess_trial(request,trials,records,solutions,current,solution):
    decision=material_hphi_tune_decision(request,trials)
    if decision['status']!='PAUSED':raise ValueError('material tune contains trials after a terminal decision')
    trial=decision['next_trial']
    if not hasattr(solution,'case') or solution.case.to_dict()!=current.project.case.to_dict():
        raise ValueError('material tune solution differs from the declared trial Project')
    ...
    try:
        ...
        status='UNVERIFIED';frequency=None
...
def _result(request,trials,records,solutions):
    decision=material_hphi_tune_decision(request,trials)
    report=dict(format='superfish_ng_material_hphi_tune_result',schema_version=1,request=deepcopy(request),
        ...
        scope='original material FEM with individually confirmed E/H IDs and optional explicit earlier-anchor recovery; separate target/two-mesh frequency gates; no RF convergence or continuum error certificate')
    return MaterialHphiTuneRun(report,tuple(r.project for r in records),tuple(solutions),tuple(records))


def assess_material_hphi_tune(request,solutions):
    """Reconstruct every trial and decision from original FEMs, never saved reports."""
    request=deepcopy(request);project=validate_material_hphi_tune(request);law=MaterialHphiShapeLaw.from_dict(request['shape_law'])
    if type(solutions) not in (list,tuple) or len(solutions)>request['max_trials']+1:
        raise ValueError('material tune requires solutions within search plus final trial budget')
...
def run_material_hphi_tune(request,*,max_new_trials=None):
    ...
    while material_hphi_tune_decision(request,trials)['status']=='PAUSED':
        if max_new_trials is not None and len(trials)>=max_new_trials:break
        next_trial=material_hphi_tune_decision(request,trials)['next_trial']
        current=_trial(request,project,law,next_trial['value'],next_trial['phase'])
        solution=solve_material_hphi(current.project.case)
