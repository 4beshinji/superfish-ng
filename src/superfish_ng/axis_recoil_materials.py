...
"""Meridional recoil tensors with an explicit regular axisymmetric extension."""
...
class AxisRecoilPartition:
    ...
    def __post_init__(self):
        if type(self.mesh) is not AxisConnectedMesh:raise ValueError('axis recoil materials require a straight AxisConnectedMesh; planar, off-axis and curved domains are unsupported')
        ...
        checked=AxisMagneticPartition(self.mesh,[LinearMagneticMaterial(m.id,1.) for m in self.materials],
        ...
        contacts=np.array([bool(np.any(axis_cells[list(region.cell_indices)])) for region in self.regions])
        contacts.setflags(write=False);object.__setattr__(self,'region_axis_contact',contacts)
        materials={m.id:m for m in self.materials};mu=[];nu=[];remanent=[]
        for region,contact in zip(self.regions,contacts):
            material=materials[region.material]
            if contact and (region.orientation_rad!=0. or material.remanent_b_local_t[0]!=0.):
                raise ValueError('axis-touching recoil region '+region.id+' requires orientation_rad=0 and radial remanent B=0; explicitly exchange principal values or reverse the axial remanence to express equivalent aligned axes')
    ...
    def to_dict(self):
        return dict(format='superfish_ng_axis_recoil_partition',schema_version=1,coordinates='axisymmetric_rz',
            azimuthal_model='decoupled_mu_phi_equals_mu_rr_zero_remanent_phi',
            geometry=dict(type='explicit_straight_axis_connected',outer_rz_m=self.mesh.outer_rz_m.tolist(),
    ...
    def from_dict(cls,data):
        names=['format','schema_version','coordinates','azimuthal_model','geometry','materials','regions'];keys(data,names,names,'axis recoil partition')
        if data['format']!='superfish_ng_axis_recoil_partition' or type(data['schema_version']) is not int or data['schema_version']!=1 or data['coordinates']!='axisymmetric_rz':
            raise ValueError('expected superfish_ng_axis_recoil_partition version 1, axisymmetric_rz')
        if data['azimuthal_model']!='decoupled_mu_phi_equals_mu_rr_zero_remanent_phi':
            raise ValueError('axis recoil material requires no phi coupling, mu_phi=mu_rr and zero phi remanence')
        g=data['geometry'];names=['type','outer_rz_m','holes_rz_m','points_rz_m','triangles'];keys(g,names,names,'axis recoil geometry')
        if g['type']!='explicit_straight_axis_connected':raise ValueError('axis recoil partition requires straight axis-connected geometry')
