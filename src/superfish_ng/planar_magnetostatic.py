...
"""Planar Az magnetic source solve, original B/H and integral quantities per metre."""
...
from .planar_magnetostatic_fem import planar_magnetostatic_forms
...
class PlanarMagnetostaticCase:
    ...
    def __post_init__(self):
        if type(self.partition) is not PlanarMagneticPartition:raise ValueError('explicit planar magnetic partition required')
        partition=PlanarMagneticPartition.from_dict(self.partition.to_dict());object.__setattr__(self,'partition',partition)
    ...
    def to_dict(self):
        return dict(format='superfish_ng_planar_magnetostatic_case',schema_version=1,physics='linear_magnetostatic',
    ...
    def from_dict(cls,data):
        ...
        if (data['format']!='superfish_ng_planar_magnetostatic_case' or type(data['schema_version']) is not int
            or data['schema_version']!=1 or data['physics']!='linear_magnetostatic'):
            raise ValueError('expected superfish_ng_planar_magnetostatic_case version 1, linear_magnetostatic physics')
        if type(data['boundaries']) is not list:raise ValueError('magnetostatic boundaries must be a JSON list')
        return cls(PlanarMagneticPartition.from_dict(data['partition']),data['current_density_z_a_per_m2'],
...
@dataclass(eq=False)
class PlanarMagnetostaticSolution:
    case: PlanarMagnetostaticCase
    space: object
    stiffness: object
    ...
    def fields_in_cells(self,cell_indices,barycentric):
        ...
        magnetic=np.column_stack((gradient[:,1],-gradient[:,0]));intensity=self.case.partition.reluctivity_m_per_h[cells,None]*magnetic
    ...
    def probe_at(self,points_xy_m):
        ...
        return dict(points_xy_m=points.tolist(),cell_indices=cells.tolist(),barycentric=bary.tolist(),
            ...
            convention='static Az[Wb/m], B=(dAz/dy,-dAz/dx)[T], H=B/(mu0*mu_r(cell))[A/m]',
            interface_policy='lowest original cell index at shared edges; one-sided values, no averaging')


def solve_planar_magnetostatic(case):
    if type(case) is not PlanarMagnetostaticCase:raise ValueError('explicit PlanarMagnetostaticCase required')
    case=PlanarMagnetostaticCase.from_dict(case.to_dict())
    space,k,volume,report=planar_magnetostatic_forms(case.partition,dict(case.current_density_z_a_per_m2),case.element_order,quadrature_order=case.quadrature_order)
    ...
    solution=PlanarMagnetostaticSolution(case,space,k,volume,boundary_load,absolute_potential,potential,reference,free,MappingProxyType(fixed_boundaries),report,residual)
    planar_magnetostatic_quantities(solution)  # Check magnetic energy/current identities before returning.
    return solution



def planar_magnetostatic_quantities(solution):
    if type(solution) is not PlanarMagnetostaticSolution:raise ValueError('PlanarMagnetostaticSolution required')
    ...
    for bary,weight in triangle_quadrature(case.quadrature_order+4):
        ...
        cell_energy+=.5*p.reluctivity_m_per_h*(fields['Bx_T']**2+fields['By_T']**2)*measure
    energy=float(cell_energy.sum());region_energy=np.bincount(p.cell_region_indices,weights=cell_energy,minlength=len(p.regions))
    reaction=s.stiffness@s.az_relative_to_reference_wb_per_m-s.volume_load_a-s.boundary_load_a
    ...
    for index in range(len(p.mesh.boundary_edges)):
        ...
        line_integral=float(measure@(fields['Hx_A_per_m']*tangent[0]+fields['Hy_A_per_m']*tangent[1]))
        normal_flux=float(measure@(fields['Bx_T']*normal[0]+fields['By_T']*normal[1]));flux_scale+=float(measure@np.hypot(fields['Bx_T'],fields['By_T']))
        ...
        if boundary.kind=='fixed_az':fixed_original[boundary.id]-=line_integral
        else:specified+=boundary.value*float(measure.sum())
    current=float(sum(case.current_density_z_a_per_m2[r.id]*v for r,v in zip(p.regions,p.region_area_m2)))
    balance=sum(fixed_reaction.values())+current-specified
    absolute=sum(abs(v) for v in fixed_reaction.values())+sum(abs(case.current_density_z_a_per_m2[r.id]*v) for r,v in zip(p.regions,p.region_area_m2))+abs(specified)
    current_error=float(abs(balance)/absolute) if absolute else float(abs(balance))
    source_work=float(s.az_wb_per_m@s.volume_load_a);boundary_work=float(s.az_wb_per_m@s.boundary_load_a)
    fixed_work=sum(b.value*fixed_reaction[b.id] for b in case.boundaries if b.kind=='fixed_az');energy_scale=2*energy+abs(source_work)+abs(boundary_work)+abs(fixed_work)
    ...
    return dict(energy_j_per_m=energy,region_energy_j_per_m={r.id:float(v) for r,v in zip(p.regions,region_energy)},total_source_current_a=current,
        ...
        discrete_current_balance_a=balance,discrete_current_relative_error=current_error,source_work_j_per_m=source_work,boundary_load_work_j_per_m=boundary_work,
        fixed_boundary_work_j_per_m=fixed_work,discrete_energy_relative_error=energy_error,free_dof_relative_residual=s.relative_residual,reference_az_wb_per_m=s.reference_az_wb_per_m,
        energy_convention='static integral |B|^2/(2*mu0*mu_r) dxdy [J/m], no RF phasor',
        circulation_convention='counterclockwise boundary tangent; curl(H)_z=Jz; fixed-Az reaction is minus the H line integral',
        flux_convention='original B dot outward normal integrated along each boundary [Wb/m]; outward normal is to the right of the counterclockwise tangent',
