...
from .material_hphi_tracking_jobs import _snapshot, read_material_hphi_tracking


@dataclass(frozen=True)
class MaterialHphiTrackingHistoryRequest:
    step_count: int
    max_steps: int = 100
    ...
    def __post_init__(self):
        ...
        if self.recoveries is not None:
            from .material_hphi_identity_recovery import MaterialHphiIdentityRecoveryRequest
            ...
            for placement in self.recoveries:
                ...
                recovery=MaterialHphiIdentityRecoveryRequest.from_dict(placement['request'])
    ...
    def to_dict(self):
        data=dict(format='superfish_ng_material_hphi_tracking_history_request', history_version=1,
    ...
    def from_dict(cls, data):
        ...
        if data['format']!='superfish_ng_material_hphi_tracking_history_request':
            raise ValueError('expected superfish_ng_material_hphi_tracking_history_request')
...
def verify_material_hphi_history_steps(paths, request):
    ...
    """
    if not isinstance(request,MaterialHphiTrackingHistoryRequest):
        raise ValueError('expected MaterialHphiTrackingHistoryRequest')
    request=MaterialHphiTrackingHistoryRequest.from_dict(request.to_dict())
    ...
    if request.recoveries is not None:
        from .material_hphi_recovered_history import verify_recovered_material_hphi_history_steps
        return verify_recovered_material_hphi_history_steps(directories,request)
    ...
    for index,path in enumerate(directories):
        current=read_material_hphi_tracking(path)
        if verified:
    ...
    return dict(format='superfish_ng_material_hphi_tracking_history_result',history_version=1,
