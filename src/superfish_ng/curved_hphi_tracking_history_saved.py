...
from .curved_hphi_tracking_jobs import _load, _snapshot as pair_snapshot
from .curved_hphi_tracking_history import CurvedHphiTrackingHistoryRequest, verify_curved_hphi_history_steps

KIND='curved_hphi_tracking_history'
...
def _inputs(directory):
    ...
    request=CurvedHphiTrackingHistoryRequest.from_dict(_load(directory/'history.json'))
    ...
    if (sources['format']!='superfish_ng_curved_hphi_tracking_history_sources'
...
def verify_curved_hphi_history(directory, state, manifest):
    ...
    if state.get('status')!='complete' or state.get('kind')!=KIND or manifest.get('kind')!=KIND:
        raise ValueError('hphi history requires complete state and kind=curved_hphi_tracking_history')
    ...
    request=CurvedHphiTrackingHistoryRequest.from_dict(_load(directory/'history.json'))
    expected=verify_curved_hphi_history_steps(_paths(directory,request),request)
...
def read_curved_hphi_history(directory):
    directory=Path(directory)
    return verify_curved_hphi_history(directory,_load(directory/'job.json'),_load(directory/'manifest.json'))


def _prepare(paths, request, directory):
    """Copy verified pairs into a fresh history; never mutate a prior history."""
    implementation=_implementation_hashes()
    verified=verify_curved_hphi_history_steps(paths,request)
    if implementation!=_implementation_hashes():raise ValueError('hphi history implementation changed during source replay')
    request=CurvedHphiTrackingHistoryRequest.from_dict(request.to_dict());paths=[Path(p) for p in paths]
    sources=dict(format='superfish_ng_curved_hphi_tracking_history_sources',sources_version=1,
...
def execute_prepared_curved_hphi_history(directory):
    ...
    try:
        ...
        result=verify_curved_hphi_history_steps(_paths(directory,request),request)
        ...
        result=read_curved_hphi_history(directory)
        if _inputs(directory)!=expected or implementation!=_implementation_hashes():
...
def execute_curved_hphi_history(paths, request, directory):
    _prepare(paths,request,directory)
    return execute_prepared_curved_hphi_history(directory)


def start_curved_hphi_history(manager, paths, request):
    with manager.lock:
    ...
    try:
        with (directory/'log.txt').open('x') as log:
            manager.processes[identifier]=subprocess.Popen(
                [sys.executable,'-m','superfish_ng.curved_hphi_tracking_history_saved',str(directory)],
...
def start_extended_curved_hphi_history(manager, history, next_pair):
    """Freeze the validated ancestry before starting a new extension worker."""
    with manager.lock:
        if manager.closed:raise ValueError('job manager is closed')
        history=Path(history);before=history_snapshot(history);previous=read_curved_hphi_history(history)
        if not previous['can_extend']:raise ValueError(previous['stop_reason'])
        old=CurvedHphiTrackingHistoryRequest.from_dict(previous['request'])
        request=CurvedHphiTrackingHistoryRequest(old.step_count+1,old.max_steps,old.recoveries)
...
def is_curved_hphi_history(directory, state, manifest):
    ...
    for name in ('history.json','history-results.json'):
        ...
        if path.is_file():
            ...
            if isinstance(data,dict) and data.get('format') in (
                    'superfish_ng_curved_hphi_tracking_history_request','superfish_ng_curved_hphi_tracking_history_result'):
                return True
    return False


def extend_curved_hphi_history(history, next_pair, directory):
    """Create a new owned history after checking every existing ancestor."""
    history=Path(history);before=history_snapshot(history);previous=read_curved_hphi_history(history)
    if not previous['can_extend']:raise ValueError(previous['stop_reason'])
    old_request=CurvedHphiTrackingHistoryRequest.from_dict(previous['request'])
    request=CurvedHphiTrackingHistoryRequest(old_request.step_count+1,old_request.max_steps,old_request.recoveries)
    if before!=history_snapshot(history):raise ValueError('source hphi history changed before extension')
    target=Path(directory)
    result=execute_curved_hphi_history([*_paths(history,old_request),Path(next_pair)],request,target)
    try:
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('usage: python -m superfish_ng.curved_hphi_tracking_history_saved PREPARED_DIRECTORY')
    execute_prepared_curved_hphi_history(sys.argv[1])
