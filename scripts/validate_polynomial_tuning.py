...
"""Polynomial radius tuning with unit changes, Maxwell scaling and job restart."""
import argparse
import json
import math
import os
...
def main():
    ...
    for unit in ('m','1'):
        for scale in (1.,2.):
            request=json.loads((ROOT/'examples/tuning/pillbox_radius_squared.json').read_text())
            request['parameter_unit']=unit
            ...
            variable_scale=.1*scale if unit=='m' else 1.
            request['bounds']=[v*variable_scale for v in request['bounds']]
            request['parameter_tolerance']*=variable_scale
            for binding in request['bindings']:
                binding['coefficients']=[c*scale/variable_scale**k for k,c in enumerate(binding['coefficients'])]
            ...
            def finished(identifier):
                ...
                while time.monotonic()<deadline:
                    ...
                    time.sleep(.05)
                raise RuntimeError('polynomial tuning worker timeout')
            try:
    ...
    report=dict(passed=passed,rows=rows,similarity_relative_errors=scaling,unit_change_relative_errors=units,
        ...
        scope='native P2 TM010 cylinder radius tuning with two quadratic coordinate bindings, metres/dimensionless variables, JobManager restart/resume, Bessel frequency and f/RQ/G scale invariants; not arbitrary shape or RF convergence acceptance')
    (out/'validation.json').write_text(json.dumps(report,indent=2,allow_nan=False)+'\n');print(f'Polynomial tuning {"PASS" if passed else "FAIL"}: {out}');return 0 if passed else 1
