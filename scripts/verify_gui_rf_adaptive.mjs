...
try {
  ...
  if(built.schema_version!==5 || built.minimum_corner_angle_deg!==request.minimum_corner_angle_deg || 'minimum_angle_deg' in built || 'surface_refinement_policy' in built)throw Error('incorrect default version 5 request');
  ...
  await loadFile('#open',out+'/case.json');await wait('document.querySelector("#geometry-type").value==="curved_contour"');
  await ev('(()=>{const e=document.querySelector("#refine-version");e.value="5";e.dispatchEvent(new Event("change",{bubbles:true}));document.querySelector("#refine-surface-policy").value="uniform_when_rf_passes"})()');
  for(const [key,value] of [['frequency',.01],['rq',.1],['g',.1],['epk',1e-12],['bpk',.05]])await fill('#refine-'+key,value);
  await click('#refine-prepare');await wait('document.querySelector("#refine-request").value.includes("uniform_when_rf_passes")');
  await writeFile(out+'/surface-policy-request.json',await ev('document.querySelector("#refine-request").value'));
  await check('surface progression is an explicit version 5 form choice','!document.querySelector("#refine-surface-policy").disabled && JSON.parse(document.querySelector("#refine-request").value).surface_refinement_policy==="uniform_when_rf_passes"');
  await fill('#refine-limit','2');const surfaceFirst=await launch('#refine-start');await openJob(surfaceFirst,'PAUSED',2);
  await check('surface-only failure adopts a parent but never counts as confirmation','refinementResult.document.levels[1].accepted && !refinementResult.document.levels[1].confirmation_comparison.passed && refinementResult.document.levels[1].uniform_confirmations===0 && refinementResult.document.decision.parent_event_index===1 && refinementResult.document.decision.next_refinement_kind==="uniform_probe" && document.querySelector("#refine-gate-context").textContent.includes("確認未達・次の親として採用")');
  const surfaceRect=await ev('(()=>{const a=document.querySelector("#refine-status").getBoundingClientRect(),b=document.querySelector("#refine-levels").getBoundingClientRect();return {x:a.x+scrollX,y:a.y+scrollY,width:Math.max(a.width,b.width),height:b.bottom-a.top,scale:1}})()');
  const surfaceShot=await call('Page.captureScreenshot',{captureBeyondViewport:true,clip:surfaceRect},sessionId);await writeFile(out+'/rf-surface-progress.png',Buffer.from(surfaceShot.data,'base64'));
  await ev('refinementAction("replay-adaptive-refinement",{document:refinementResult.serialized})');
  await check('replay restores the explicit surface policy','document.querySelector("#refine-surface-policy").value==="uniform_when_rf_passes" && refinementResult.document.levels[1].uniform_confirmations===0');
  await fill('#refine-limit','1');const surfaceNext=await launch('#refine-resume');await openJob(surfaceNext,'PAUSED',3);
  await check('surface progression resumes uniformly from the adopted confirmation','refinementResult.document.levels[2].parent_event_index===1 && refinementResult.document.levels[2].triangles===16*refinementResult.document.levels[0].triangles && refinementResult.document.levels[2].uniform_confirmations===0');
