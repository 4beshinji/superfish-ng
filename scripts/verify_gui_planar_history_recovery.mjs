...
if (!args["--url"] || !args["--out"])
  throw Error(
    "Usage: node scripts/verify_gui_planar_history_recovery.mjs --url LAUNCH_URL --out NEW_DIRECTORY",
  );
...
try {
  ...
  await fill('#nx',6);await fill('#ny',6);await fill('#modes',3);
  ...
  await click('#history-result details summary');
  const recovery={format:'superfish_ng_planar_identity_recovery_request',recovery_version:1,anchor_snapshot_index:0,comparison:report.request};
  await fill('#history-recovery-request',JSON.stringify(recovery));
  await click('#history-recovery-save');
  if(!isDeepStrictEqual(JSON.parse(await download('planar-identity-recovery.json')),recovery))throw Error('recovery download differs');
  await fill('#history-recovery-request','null');await loadFile('#history-recovery-open',out+'/downloads/planar-identity-recovery.json');
  await wait('JSON.parse(document.querySelector("#history-recovery-request").value)?.anchor_snapshot_index===0');
  report.recovered_history=await execute('#history-recover','追跡履歴','selectedHistory');
  await check('explicit anchor recovers swapped IDs while raw inherited IDs remain null','currentHistory.result.individual_ids_complete && JSON.stringify(currentHistory.result.current_mode_ids)===JSON.stringify(["mode-2","mode-1"]) && currentHistory.result.steps[1].current_mode_ids.every(x=>x===null) && currentHistory.result.identity_recoveries[0].status==="PASS" && document.querySelector("#history-identities").textContent.includes("anchor_snapshot_index")');
  report.recovery_result=await ev('currentHistory.result');
  await writeFile(out+'/recovered-history-request.json',JSON.stringify(await ev('currentHistory.request')));
  await loadFile('#history-open',out+'/recovered-history-request.json');
  await wait('JSON.parse(document.querySelector("#history-recoveries").value).length===1');
  if(!isDeepStrictEqual(await ev('historyFromForm()'),await ev('currentHistory.request')))throw Error('recovered history request roundtrip differs');
  report.checks.push({operation:'recovered history conditions preserve explicit events',passed:true});
  await click('#history-use-groups');await wait('document.querySelector("#tracking-ids").value==="null" && JSON.parse(document.querySelector("#tracking-groups").value).length===2');
  await ev(`document.querySelector('#tracking-current').value=${JSON.stringify(report.current)}`);
  report.continuation=await execute('#tracking-start','モード追跡','selectedTracking');
  await check('next real pair inherits recovered individual IDs','currentTracking.result.individual_ids_complete && JSON.stringify(currentTracking.result.current_mode_ids)===JSON.stringify(["mode-2","mode-1"])');
  await ev(`document.querySelector('#history-next').value=${JSON.stringify(report.continuation)}`);
  report.final_history=await execute('#history-extend','追跡履歴','selectedHistory');
  await check('extended recovered history preserves the original event','currentHistory.result.steps.length===3 && currentHistory.result.identity_recoveries.length===1 && currentHistory.result.individual_ids_complete');
  await click('#history-source');await wait('selected!==null && project.case.geometry.width_m===.22');report.final_source=await ev('selected');
  await click('#plot');await wait('!document.querySelector("#image").hidden && document.querySelector("#image").naturalWidth>100',90000);
  report.checks.push({operation:'recovered final original field imports and renders',passed:true});
  await call('Page.navigate',{url:launch.origin+'/planar.html?job='+report.final_history},sessionId);await wait(`typeof selectedHistory!=='undefined' && selectedHistory===${JSON.stringify(report.final_history)}`,180000);
  await check('URL reload verifies recovered ancestry and final IDs','currentHistory.result.steps.length===3 && JSON.stringify(currentHistory.result.current_mode_ids)===JSON.stringify(["mode-2","mode-1"])');
  await ev('document.querySelector("#history-result").scrollIntoView({block:"start"})');const screenshot=await call('Page.captureScreenshot',{format:'png'},sessionId);await writeFile(out+'/history-recovery.png',Buffer.from(screenshot.data,'base64'));
