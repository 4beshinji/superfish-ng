...
"""Independent analytical and scaling checks of peak-confirmed adaptive stopping."""
import argparse
...
def main():
    ...
    for order in (1,2):
        series=[]
        for scale in (1.,2.):
            request=json.loads((ROOT/'examples/adaptive_refinement/pillbox_surface_confirmed.json').read_text())
            ...
            request.update(case=case.to_dict(),initial_ids=['fundamental'],max_levels=10,max_triangles=150000,relative_tolerances=dict(frequency_hz=1e-4,r_over_q_accelerator_ohm=.005,geometry_factor_ohm=.005))
            ...
            passed=(errors['frequency_hz']<1e-4 and all(errors[k]<.005 for k in ('r_over_q_accelerator_ohm','geometry_factor_ohm')) and all(errors[k]<.01 for k in ('epk_over_eacc','bpk_over_eacc_mt_per_mv_per_m'))
                ...
                and checkpoint['surface_status']=='TARGETS_MET' and checkpoint['status']=='TARGETS_MET' and report['status']=='TARGETS_MET' and report['two_uniform_steps_present'] and all(row['mode_index']==0 for row in report['rows']))
    ...
    result=dict(passed=passed,checks=checks,source_sha256=before,source_changed_during_run=not unchanged,
        scope='actual P1/P2 cylinder adaptive refinements, native replay, separate analytical f/RF/peak endpoint checks, and scaling; all five quantities and two final uniform refinements required for both orders; budget exhaustion is a failure; no general physical-error-bound acceptance')
    (out/'validation.json').write_text(json.dumps(result,indent=2,allow_nan=False)+'\n');print(f'Adaptive surface stopping {"PASS" if passed else "FAIL"}: {out}');return 0 if passed else 1
