...
if (!args["--url"] || !args["--out"])
  throw Error(
    "Usage: node scripts/verify_gui_planar_remesh_tracking.mjs --url LAUNCH_URL --out NEW_DIRECTORY",
  );
...
try {
  ...
  const makeProject=(n,flipped)=>{const p=structuredClone(template),points=[],triangles=[];p.case.name=flipped?'independent square mesh':'original square mesh';p.case.geometry.vertices_xy_m=[[0,0],[.2,0],[.2,.2],[0,.2]];
    for(let j=0;j<=n;j++)for(let i=0;i<=n;i++)points.push([.2*(i/n),.2*(j/n)]);
    for(let j=0;j<n;j++)for(let i=0;i<n;i++){const a=j*(n+1)+i,b=a+1,d=a+n+1,c=d+1;triangles.push(...(flipped?[[a,b,d],[b,c,d]]:[[a,b,c],[a,c,d]]));}
    p.case.mesh={points_xy_m:points,triangles,element_order:2};return p;};
  await writeFile(out+'/previous-project.json',JSON.stringify(makeProject(6,false)));await loadFile('#open',out+'/previous-project.json');await wait('project.case.name==="original square mesh"');
  report.previous=await execute('#start','','selected');await click('#tracking-select-previous');
  await writeFile(out+'/current-project.json',JSON.stringify(makeProject(8,true)));await loadFile('#open',out+'/current-project.json');await wait('project.case.name==="independent square mesh"');report.current=await execute('#start','','selected');
  await ev('document.querySelector("#tracking-mapping").value="polygon_same_domain";document.querySelector("#tracking-mapping").dispatchEvent(new Event("change"))');await click('#tracking-select-current');
  await fill('#tracking-previous-count',1);await fill('#tracking-current-count',1);await fill('#tracking-ids','["fundamental"]');await fill('#tracking-candidate-tests',123456);
  report.tracking=await execute('#tracking-start','モード追跡','selectedTracking');report.request=await ev('currentTracking.request');
  await check('independent meshes track the original electric field','currentTracking.result.status==="PASS" && currentTracking.result.result_version===4 && currentTracking.result.current_mode_ids[0]==="fundamental"');
  ...
  if(!isDeepStrictEqual(await ev('trackingFromForm()'),report.request))throw Error('request form differs');report.checks.push({operation:'remesh declaration and search budget roundtrip',passed:true});
  await click('#tracking-result-save');if(!isDeepStrictEqual(JSON.parse(await download('planar-tracking-results.json')),await ev('currentTracking.result')))throw Error('result download differs');report.checks.push({operation:'remesh full result download matches replay',passed:true});
  await click('#tracking-current-import');await wait(`selected!==${JSON.stringify(report.current)}`);report.imported=await ev('selected');await click('#plot');await wait('!document.querySelector("#image").hidden && document.querySelector("#image").naturalWidth>100',90000);report.checks.push({operation:'independent native is imported and plotted',passed:true});
  await click('#history-select-tracking');report.first_history=await execute('#history-start','追跡履歴','selectedHistory');
  await ev(`document.querySelector('#tracking-previous').value=${JSON.stringify(report.current)};document.querySelector('#tracking-current').value=${JSON.stringify(report.previous)}`);
  report.reverse=await execute('#tracking-start','モード追跡','selectedTracking');await check('reversed independent meshes preserve ID','currentTracking.result.status==="PASS" && currentTracking.result.current_mode_ids[0]==="fundamental"');
  ...
  await check('owned history replays both independent-mesh steps','currentHistory.result.can_extend && currentHistory.result.steps.length===2 && currentHistory.result.current_mode_ids[0]==="fundamental"');
  const before=await ev('JSON.stringify(trackingFromForm())');await writeFile(out+'/invalid.json',JSON.stringify({...report.request,mapping:{...report.request.mapping,scale:1}}));await loadFile('#tracking-open',out+'/invalid.json');await wait('!document.querySelector("#error").hidden');if(await ev('JSON.stringify(trackingFromForm())')!==before)throw Error('unsupported map changed inputs');report.checks.push({operation:'unsupported map fields preserve inputs',passed:true});
  ...
  await check('URL restores remesh budget and hides scale controls','trackingFromForm().mapping.max_candidate_tests===123456 && !document.querySelector("#tracking-remesh-options").hidden && document.querySelector("#tracking-polygon-options").hidden');
  await ev('document.querySelector("#tracking-result").scrollIntoView({block:"start"})');const screenshot=await call('Page.captureScreenshot',{format:'png'},sessionId);await writeFile(out+'/remesh-tracking.png',Buffer.from(screenshot.data,'base64'));
