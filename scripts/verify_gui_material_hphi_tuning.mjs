...
if (!args["--url"] || !args["--out"] || !args["--request"])
  throw Error(
    "Usage: node scripts/verify_gui_material_hphi_tuning.mjs --url LAUNCH_URL --out NEW_DIRECTORY --request REQUEST_JSON",
  );
...
try {
  ...
  await wait('typeof currentHphiTune !== "undefined" && $("jobs").dataset.signature');
  const request=JSON.parse(await readFile(args['--request'],'utf8'));
  if(request.format!=='superfish_ng_material_hphi_tune')throw Error('material request required');
  ...
  await fill('#hphi-tune-document',JSON.stringify(request));
  await click('#hphi-tune-apply');
  await wait('!$("hphi-tune-start").disabled');
  await check('complete material Project preview preserves display units',`$('hphi-tune-preview').textContent.includes('三角形') && $('unit').value===${JSON.stringify(request.project.display_length_unit)}`);
  await fill('#hphi-tune-new-trials','1');
  await click('#hphi-tune-start');
  await wait(`$('jobs').textContent.includes('complete / 周波数調整')`,resultTimeout);
  const firstId=await ev(`document.querySelector('#jobs .job').dataset.job`);
  await click(`[data-job="${firstId}"] > button`);
  await wait('currentHphiTune?.document.trials.length===1',resultTimeout);
  if(!isDeepStrictEqual(await ev('currentHphiTune.document.request'),request))throw Error('request changed');
  await check('PAUSED material worker appears with separate incomplete frequency gates',`currentHphiTune.document.status==='PAUSED' && $('hphi-tune-gates').textContent.includes('未実施')`);
  ...
  for(let n=0;n<2;n++){
    ...
    await wait(`$('hphi-tune-document').value.includes('superfish_ng_material_hphi_tune')`,resultTimeout);
    if(await ev('currentHphiTune.serialized')!==text)throw Error('checkpoint replay differs');
  }
  report.checks.push({operation:'same owned material checkpoint reopened twice',passed:true});
  await click('#hphi-tune-resume');
  await wait(`document.querySelectorAll('#jobs .job').length===2 && Array.from(document.querySelectorAll('#jobs .job strong')).every(e=>e.textContent.startsWith('complete'))`,resultTimeout);
  const resumedId=await ev(`document.querySelector('#jobs .job').dataset.job`);
  if(resumedId===firstId)throw Error('resume reused job');
  await click(`[data-job="${resumedId}"] > button`);
  await wait('currentHphiTune?.document.trials.length===2',resultTimeout);
  report.checks.push({operation:'resume creates separate material worker with inherited prefix',passed:true});
  const final=await ev('currentHphiTune.document');
  report.job_id=resumedId;
  ...
  await check('original target rank and Project display units retained; axial RF unavailable',`Number($('mode').value)===1 && $('unit').value===${JSON.stringify(request.project.display_length_unit)} && $('result').textContent.includes('N/A')`);
