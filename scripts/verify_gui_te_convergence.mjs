...
try {
  ...
  for (const [name,kind,expected] of [['mesh','mesh_convergence','PASS'],['fixed-curved','fixed_geometry_convergence','UNVERIFIED'],['geometry','geometry_convergence','FAIL']]) {
    ...
    await loadFile('#open-study',`out/te-convergence-study-kinds-20260909/${name}/study.json`);
  ...
  await check('TE convergence point imports and renders native field','currentResult.state.origin==="imported" && currentResult.result.modes[0].r_over_q_accelerator_ohm===null');
