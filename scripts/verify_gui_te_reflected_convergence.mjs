...
try {
  ...
  report.studies=[];
  for (const [name,kind,expected] of [['mesh','mesh_convergence','PASS'],['fixed-curved','fixed_geometry_convergence','UNVERIFIED'],['geometry','geometry_convergence','FAIL']]) {
    await ev('window.beforeStudyJobs=[...document.querySelectorAll("#jobs .job")].map(x=>x.dataset.job)');
    await loadFile('#open-study',`out/te-reflected-convergence-study-kinds-20260909/${name}/study.json`);
    await wait(`document.querySelector("#polarization").value==="te" && document.querySelector("#study-kind").value===${JSON.stringify(kind)}`);
    await click('#start-study');
    const selected='[...document.querySelectorAll("#jobs .job")].find(x=>!window.beforeStudyJobs.includes(x.dataset.job))';
    await wait(`(()=>{const x=${selected};return x && [...x.querySelectorAll('button')].some(b=>b.textContent==='結果を開く'&&!b.disabled)})()`,60000);
    const id=await ev(`(${selected}).dataset.job`);
    await click(`#jobs .job[data-job="${id}"] button`);
    await wait(`activeStudy?.physics==="axisymmetric_m0_te" && activeStudy.study.kind===${JSON.stringify(kind)}`);
    await check(`${name}: independent field/RF gates and N/A`, `activeStudy.numerical_status===${JSON.stringify(expected)} && document.querySelector('#study-report').textContent.includes('電場') && document.querySelector('#study-report').textContent.includes('磁場') && document.querySelector('#study-report').textContent.includes('RF') && document.querySelector('#study-report').textContent.includes('積分次数照合') && !document.querySelector('#study-report').textContent.includes('NaN') && [...document.querySelectorAll('#study-report table tr')].slice(1).every(r=>r.children[3].textContent.includes('N/A'))`);
    await check(`${name}: source-sector comparison remains explicit`, `activeStudy.study.project.reflect_full && activeStudy.comparisons.every(c=>c.reflection_comparison.domain==='reconstructed source half-domain') && document.querySelector('#study-report').textContent.includes('再構成・再検証した元半領域') && document.querySelector('#study-report th:nth-child(2)').textContent==='部分スペクトル内の番号' && document.querySelector('#study-report').textContent.includes('番号は全空洞の周波数順位ではありません')`);
    report.studies.push({name,id,result:await ev('activeStudy')});
    await ev('document.querySelectorAll("#study-report details").forEach(x=>x.open=true)');
    const rect=await ev('(()=>{const r=document.querySelector("#study-report").getBoundingClientRect();return {x:r.x+scrollX,y:r.y+scrollY,width:r.width,height:r.height,scale:1}})()');
  ...
  await click('#study-report table button');
  await wait('currentResult?.result.physics==="axisymmetric_m0_te" && !document.querySelector("#field-image").hidden && document.querySelector("#field-image").naturalWidth>0',60000);
  await check('reflected TE point imports and renders with source provenance','currentResult.state.origin==="imported" && currentResult.result.modes[0].r_over_q_accelerator_ohm===null && currentResult.result.reflection && currentResult.project.reflect_full');
  ...
  report.main_tree_implementation=true;console.log(JSON.stringify({passed:report.passed,checks:report.checks}));
