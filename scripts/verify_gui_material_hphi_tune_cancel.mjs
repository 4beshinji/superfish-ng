...
if (!args["--url"] || !args["--out"] || !args["--request"])
  throw Error(
    "Usage: node scripts/verify_gui_material_hphi_tune_cancel.mjs --url LAUNCH_URL --out NEW_DIRECTORY --request REQUEST_JSON",
  );
...
try {
  ...
  for(let n=0;n<2;n++){
    ...
    await wait(`JSON.parse($('hphi-tune-document').value).format==='superfish_ng_material_hphi_tune'`,resultTimeout);
  ...
  const hasAxis=Boolean(request.project.case.acceleration);
  await check('confirmed target opens actual native rank and declared axial RF',`Number($('mode').value)===${rank} && ${hasAxis ? "!$('result').textContent.includes('N/A')" : "$('result').textContent.includes('N/A')"}`);
  if(hasAxis){const rf=await ev(`result.modes[${rank-1}]`);if(!(rf.r_over_q_accelerator_ohm>0)||rf.r_over_q_accelerator_ohm!==2*rf.r_over_q_circuit_ohm||rf.accelerating_quantities_reason!==null)throw Error('both declared R/Q definitions were not preserved');}
  ...
  report.trial_runs=final.trial_runs;report.imported_job=await ev('selected');
