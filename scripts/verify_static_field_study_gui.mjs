...
if (!args["--url"] || !args["--out"] || !args["--cases"] || !args["--workspace"])
  throw Error(
    "Usage: node scripts/verify_static_field_study_gui.mjs --url-file LAUNCH_URL_FILE --cases CASES_JSON --workspace WORKSPACE --out NEW_DIRECTORY",
  );
...
try {
  ...
  const launch = new URL(args['--url']); launch.pathname = '/static-study.html';
  ...
  const ready = async () => await wait('typeof selected!=="undefined" && selected!==null && renderedStudy===selected && renderedPoint===selected+":"+selectedIndex && !document.querySelector("#result-view").hidden',900000);
  const loadStudy = async file => {
    ...
    await call('DOM.setFileInputFiles',{nodeId,files:[file]},sessionId);await wait('document.querySelector("#document").value.length>0',180000);
    if(!isDeepStrictEqual(JSON.parse(await ev('document.querySelector("#document").value')),JSON.parse(await readFile(file,'utf8'))))throw Error('file input changed Study or signed zero');
  };
  const solve = async file => {await loadStudy(file);const old=await ev('selected');await click('#solve');await wait(`selected!==${JSON.stringify(old)}`,180000);await ready();return await ev('selected');};
  await navigate(); report.records=[];
  for(const [index,value] of cases.entries()){
    const summary=JSON.parse(await readFile(value.expected_summary_file,'utf8')),study=summary.study;
    const views=JSON.parse(await readFile(value.expected_views_file,'utf8'));let job;
    if(restored){job=restored.records[index].id;await navigate(job);await ready();}
    else{
      await loadStudy(value.study_file);const alternate=study.project.display_length_unit==='m'?'mm':'m';
      for(const unit of [alternate,study.project.display_length_unit]){
        await ev(`document.querySelector('#length-unit').value=${JSON.stringify(unit)};document.querySelector('#input-status').textContent=''`);await click('#validate');await wait('document.querySelector("#input-status").textContent.length>0',180000);
        const expected={...study,project:{...study.project,display_length_unit:unit}};
        if(!isDeepStrictEqual(JSON.parse(await ev('document.querySelector("#document").value')),expected))throw Error('display unit switch changed Study SI Case or values');
      }
      if(index===0){
        await ev(`(()=>{const p=JSON.parse(document.querySelector('#document').value);p.project.case.name+=' GUI編集';document.querySelector('#document').value=jsonText(p);document.querySelector('#input-status').textContent=''})()`);
        await click('#validate');await wait('document.querySelector("#input-status").textContent.length>0');
        const expected={...study,project:{...study.project,case:{...study.project.case,name:study.project.case.name+' GUI編集'}}};
        if(!isDeepStrictEqual(JSON.parse(await ev('document.querySelector("#document").value')),expected))throw Error('Study editor changed unrelated fields');
        await fill('#document',await readFile(value.study_file,'utf8'));await click('#validate');
        report.checks.push({operation:'strict Study JSON editing preserves SI materials, initialization and values',passed:true});
      }
      await click('#save-study');const saved=await download('static-study.json');if(!saved.equals(await readFile(value.study_file)))throw Error('edited Study download bytes changed');await rename(out+'/downloads/static-study.json',out+'/downloads/'+index+'-edited-study.json');
      const old=await ev('selected');await click('#solve');await wait(`selected!==${JSON.stringify(old)}`,180000);await ready();job=await ev('selected');
    }
    if(!isDeepStrictEqual(JSON.parse(await ev('document.querySelector("#study-result-json").textContent')),summary))throw Error('displayed full Study outcome changed');
    if(!isDeepStrictEqual(JSON.parse(await ev('document.querySelector("#study-json").textContent')),study))throw Error('displayed input Study changed');
    if(await ev('document.querySelector("#study-status").dataset.allPointsSuccessful')!==String(summary.all_points_successful))throw Error('Study completion confused with point success');
    const options=await ev('Array.from(document.querySelector("#point-index").options).map(o=>o.value)');
    if(!isDeepStrictEqual(options,summary.points.map(p=>String(p.index))))throw Error('point selection changes input ordering');
    let downloads=0;const points=[];
    for(const file of ['study.json','study-results.json']){
      await click(`#study-downloads button[data-file="${file}"]`);const bytes=await download(file);
      if(!bytes.equals(await readFile(resolve(value.source_job,file)))||!bytes.equals(await readFile(resolve(args['--workspace'],job,file))))throw Error('Study download differs '+file);
      await rename(out+'/downloads/'+file,out+'/downloads/'+index+'-'+file);downloads++;
    }
    for(const [pointIndex,expected] of views.entries()){
      const project=expected.project,failed=expected.solver_status==='nonlinear_failed';
      await ev(`document.querySelector('#point-index').value=${JSON.stringify(String(pointIndex))};document.querySelector('#point-index').dispatchEvent(new Event('change'))`);await ready();
    ...
    if(await ev('document.querySelector("#field-view").hidden')!==failed)throw Error('failure plot visibility incorrect');
    ...
    const prefix=summary.points[pointIndex].directory;
    const files=['project.json',...(failed?['case.json','failure.json','manifest.json']:['case.json','fields.npz','manifest.json','mesh.npz','results.json']).map(n=>'solution/'+n)].map(n=>prefix+'/'+n);
    for(const file of files){await click(`#downloads button[data-file="${file}"]`);const name=file.replaceAll('/','-'),bytes=await download(name);if(!bytes.equals(await readFile(resolve(value.source_job,file)))||!bytes.equals(await readFile(resolve(args['--workspace'],job,file))))throw Error('native download differs '+file);await rename(out+'/downloads/'+name,out+'/downloads/'+index+'-'+name);}
      downloads+=files.length;points.push({index:pointIndex,failed,plots,downloads:files.length});
      if([0,24,66,75,76,77].includes(index) && (pointIndex===0 || failed)){
        await ev('document.querySelector("#study-status").scrollIntoView({block:"start"})');
        const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},sessionId);
        await writeFile(out+'/case-'+index+'-point-'+pointIndex+'.png',Buffer.from(shot.data,'base64'));
      }
    }
    report.records.push({name:value.name,id:job,points,downloads,study_download:!restored,all_original_cases_quantities_fields_units_and_failures_identical:true});
    report.checks.push({operation:(restored?'restart/replay/all points ':'input/edit/solve/all points ')+value.name,passed:true});console.log('DONE',index,value.name);
  }
  await check('HTTP rejects missing token',`(async()=>{const r=await fetch('/api',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'static-study-jobs'})});return r.status===403})()`);
  await check('HTTP rejects extra action fields',`(async()=>{const r=await fetch('/api',{method:'POST',headers:{'Content-Type':'application/json','X-NG-Token':token},body:JSON.stringify({action:'static-study-jobs',extra:1})});return r.status===400})()`);
  if(!restored){
    const original=await readFile(cases[0].study_file,'utf8'),duplicate=original.replace('"study_version": 1','"study_version": 1, "study_version": 1');
    ...
    report.checks.push({operation:'duplicate Study JSON keys reject through actual editor/solve action',passed:true});
    const extra=await solve(cases[0].study_file);report.changed_owned_job=extra;
    const changed=resolve(args['--workspace'],extra,'point-0001/solution/results.json');await writeFile(changed,Buffer.concat([await readFile(changed),Buffer.from('\n')]));
    await wait('!document.querySelector("#error").hidden && document.querySelector("#study-view").hidden && document.querySelector("#result-view").hidden && document.querySelector("#field-view").hidden');
    await check('changed unselected point removes current Study and rejects download',`(async()=>{try{await api('static-study-download',{id:selected,file:'study.json'},true);return false;}catch{return true;}})()`);
    await click(`.job[data-job="${report.records[0].id}"] button`);await ready();await check('valid Study selection clears previous changed-file error',`document.querySelector('#error').hidden`);
    const cancelStudy=resolve(args['--cases'],'../cancel-study.json');
    for(const operation of ['cancel','kill']){
      await loadStudy(cancelStudy);const old=await ev('selected');await click('#solve');await wait(`selected!==${JSON.stringify(old)}`,180000);const id=await ev('selected');
      await wait(`(async()=>{const j=(await api('static-study-jobs')).find(j=>j.id===${JSON.stringify(id)});return j?.status==='running'})()`,180000);
      if(operation==='cancel')await click(`.job[data-job="${id}"] button`);
      else{const directory=resolve(args['--workspace'],id),state=JSON.parse(await readFile(directory+'/job.json','utf8')),pid=Number((await readFile(directory+'/worker.claim','utf8')).trim());if(state.status!=='running'||!Number.isSafeInteger(pid)||pid<=1)throw Error('cannot identify live owned Study worker');process.kill(pid,'SIGKILL');}
      const expected=operation==='cancel'?'cancelled':'failed';await wait(`(async()=>{const j=(await api('static-study-jobs')).find(j=>j.id===${JSON.stringify(id)});return j?.status===${JSON.stringify(expected)}})()`,30000);
      await wait('document.querySelector("#study-view").hidden && document.querySelector("#result-view").hidden && document.querySelector("#field-view").hidden');report[operation+'_job']=id;
      report.checks.push({operation:'real static Study GUI worker '+operation+' leaves no completed Study or field',passed:true});
    }
  }
  for(const index of [0,66,75]){
    ...
    if(await ev('location.pathname')!=='/static-study.html'||await ev('selected')!==job)throw Error('RF history misroutes static Study');
    report.checks.push({operation:'main history opens static Study '+index+' in dedicated page',passed:true});
  }
  ...
  for(const [page,selector] of [['/planar.html','#width'],['/hphi.html','#inner-radius'],['/magnetic.html','#source-path'],['/static.html','#document']]){await call('Page.navigate',{url:launch.origin+page},sessionId);await wait(`document.querySelector(${JSON.stringify(selector)})`);await check('existing '+page+' retains static navigation',`document.querySelector('a[href="/static-study.html"]')!==null`);}
