...
"""Axis magnetostatic native/CLI replay of analytically validated source problems."""
import argparse,hashlib,json,os,subprocess,sys,time
from pathlib import Path
import numpy as np
ROOT=Path(__file__).resolve().parents[1];sys.path[:0]=[str(ROOT),str(ROOT/'src')]
from superfish_ng.axis_magnetostatic import AxisMagnetostaticCase,solve_axis_magnetostatic
...
def main():
    ...
    source_report=json.loads((reference/'report.json').read_text());assert source_report['status']=='PASS' and source_report['cases']==88 and source_report['uniform_cases']==32 and source_report['cylinder_cases']==32 and source_report['layered_cases']==24
    ...
    inventory=capabilities();assert json.loads(cli(['capabilities']))==inventory;description=inventory['axis_magnetostatic']
    ...
    for record in source_report['records']:
        ...
        if ((name.startswith('uniform-') and '-s0.5-mu1.0-' in name)
            or (name.startswith('cylinder-') and '-s0.5-mu1.0-' in name)
            or (name.startswith('layer-') and ('-p1-n16-' in name or '-p2-n8-' in name))):selected.append(record)
    assert len(selected)==24
    for record in selected:
        name=record['name'];input_path=reference/(name+'.json');case=AxisMagnetostaticCase.from_dict(json.loads(input_path.read_text()));s=solve_axis_magnetostatic(case);p=case.partition
        api=out/'api-native'/name;result=save_axis_magnetostatic_run(case,s,api);restored=read_axis_magnetostatic_run(api)
        assert axis_magnetostatic_result(restored)==result;np.testing.assert_array_equal(restored.aphi_over_r_t,s.aphi_over_r_t)
        assert result['quantities']==record['quantities'];assert result['physics']==case.to_dict()['physics']=='linear_magnetostatic'
        binary=out/'cli-native'/name;emitted=json.loads(cli(['solve-axis-magnetostatic',input_path,'--out',binary]));assert emitted==result
        assert json.loads(cli(['replay-axis-magnetostatic',binary]))==result;assert _snapshot(binary)==_snapshot(api)
        ...
        expected=s.probe_at(points);probe=export_axis_magnetostatic_probe(api,out/f'{name}.api-probe.json',points)
        for key,value in expected.items():assert probe[key]==value
        cli_probe=out/f'{name}.cli-probe.json';cli(['probe-axis-magnetostatic',binary,'--points',point_file,'--out',cli_probe]);assert json.loads(cli_probe.read_text())==probe
        ...
        if len(p.interface_edges):assert probe['cell_indices'][3]==int(min(p.interface_cells[0]))
    ...
    cli(['probe-axis-magnetostatic',binary,'--points',outside,'--out',out/'rejected-probe.json'],expected=2);assert not (out/'rejected-probe.json').exists()
    cli(['solve-axis-magnetostatic',reference/f'{records[0]["name"]}.json','--out',binary],expected=2)
    cli(['probe-axis-magnetostatic',binary,'--points',outside,'--mode','1','--out',out/'rejected-mode.json'],expected=2);assert not (out/'rejected-mode.json').exists()
    boolean=out/'boolean-points.json';boolean.write_text('[[false,0]]\n')
    cli(['probe-axis-magnetostatic',binary,'--points',boolean,'--out',out/'rejected-boolean.json'],expected=2);assert not (out/'rejected-boolean.json').exists()
    ...
    report=dict(status='PASS',cases=len(records),native_saves=2*len(records),records=records,cli_commands=commands,
        ...
        original_physics='unchanged axis magnetostatic solver/field/integral source and exact quantities from independent analytical validation',
        api_cli_native='five files byte-identical per case',api_cli_probe='full JSON identical, including one-sided a/Aphi/B/H including axis values, material metadata and source hashes',
