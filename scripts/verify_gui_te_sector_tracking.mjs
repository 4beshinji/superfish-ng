...
if (!args["--url"] || !args["--out"] || !args["--sources"])
  throw Error(
    "Usage: node scripts/verify_gui_te_tracking.mjs --url LAUNCH_URL --out NEW_DIRECTORY --sources CLUSTER_VALIDATION_DIRECTORY",
  );
...
try {
  ...
  const imported={},known=new Set(await ev('[...document.querySelector("#tracking-current").options].map(x=>x.value).filter(Boolean)'));
  ...
  await fill("#tracking-ids",'["r1z1","r2z1","r1z3","r2z3"]');await click("#tracking-retain");
  await click("#tracking-compare");await wait('trackingResult?.document.status==="PASS" && !trackingBusy');
  await check("merge is displayed as a subspace with unresolved individual IDs",'document.querySelector("#tracking-status").textContent.includes("個別IDは未確定") && document.querySelector("#tracking-matches tbody").rows.length===3 && trackingResult.document.tracking.cluster_transitions.events[0].kind==="MERGE"');
  await check("reflected TE sector and non-global ranks remain explicit",'trackingResult.document.tracking.physical_mapping.reflected_partial_spectrum && document.querySelector("#tracking-status").textContent.includes("番号は全空洞の周波数順位ではありません")');
  ...
  await check("split retains the ID set in the GUI history",'trackingResult.document.status==="PASS" && trackingResult.document.steps.at(-1).tracking.cluster_transitions.events[0].kind==="SPLIT" && trackingResult.document.current_mode_ids[2]===null');
  ...
  await check("modified history is rejected without replacing verified data",'trackingResult.document.current_identity_groups[0].ids[0]==="r1z1" && document.querySelector("#error").textContent.includes("replay")');
  ...
  report.imported=imported;report.verified_history=downloaded;
