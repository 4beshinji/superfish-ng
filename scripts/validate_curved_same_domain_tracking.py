...
"""Fixed quadratic boundaries: sphere reference, volume and Maxwell scaling."""
import argparse
...
def main():
    ...
    for kind in ('sphere','ellipsoid'):
        rows=[];overlaps=[];volumes=[];sphere_errors=[];counts=[];bounds=[]
        for scale in (1.,2.):
            ...
            base=Case.from_dict(data);names=[];quantities=[]
            for level in (0,1):
                case=replace(base,curved_refinement_levels=level);solution=solve(case);name=f'{kind}-{int(scale)}-{level}'
                save_run(case,solution,out/name);names.append(name);quantities.append(json.loads((out/name/'results.json').read_text())['modes'][0])
                if kind=='sphere':sphere_errors.append(float(abs(solution.frequencies_hz[0]/SphereTM(scale*.1).frequency_hz-1)))
            values=[]
            for order in (3,5):
                controls=dict(mapping='curved_same_domain',sample_order=order,minimum_overlap=.98,minimum_assignment_margin=.05,
                ...
                measured=m['axisymmetric_volumes_m3'];analytic=4*np.pi/3*(scale*.1)*radial**2
                volumes.append(dict(fixed_geometry_relative_difference=abs(measured[1]/measured[0]-1),analytical_relative_errors=[float(abs(v/analytic-1)) for v in measured]))
        ...
        passed=(max(similarity.values())<2e-9 and quadrature<1e-4 and scaled<2e-10
            and max(v['fixed_geometry_relative_difference'] for v in volumes)<2e-12
            and max(e for v in volumes for e in v['analytical_relative_errors'])<.002
            and all(b==4*a for a,b in counts) and (not sphere_errors or max(sphere_errors)<.002))
        checks[kind]=dict(passed=passed,similarity_relative_errors=similarity,overlaps=overlaps,quadrature_overlap_difference=quadrature,
            overlap_scale_difference=scaled,volumes=volumes,sphere_frequency_relative_errors=sphere_errors,triangle_counts=counts,boundary_checks=bounds)
    ...
    report=dict(passed=passed,checks=checks,source_sha256=before,source_changed_during_run=before!=hashes(),
        scope='native curved P2 fixed-geometry refinement, sphere and synthetic ellipsoid; whole quadratic boundary checks, sphere frequency, analytic volume and uniform Maxwell scaling; not equality of reprojected boundaries or physical RF convergence acceptance')
    (out/'validation.json').write_text(json.dumps(report,indent=2,allow_nan=False)+'\n');print(f'Curved same domain {"PASS" if passed else "FAIL"}: {out}');return 0 if passed else 1
