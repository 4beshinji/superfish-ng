...
"""Axis nonlinear B-H native/CLI comparison on independently checked Cases."""
...
from superfish_ng.axis_bh_saved import (save_axis_bh_run,read_axis_bh_run,axis_bh_result,export_axis_bh_probe,
    save_axis_bh_failure,read_axis_bh_failure,_snapshot,_failure_snapshot)
...
def main():
    ...
    inventory=capabilities();assert json.loads(cli(['capabilities']))==inventory;description=inventory['axis_bh']
    ...
    for record in source_report['records']:
        ...
        elif ((kind in ('uniform','exact_current') and '-s0.5-' in name) or (kind in ('layered','current') and '-n8-s0.5-' in name)
    ...
    for record in selected:
        name=record['name'];input_path=reference/(name+'.json');case=AxisBHCase.from_dict(json.loads(input_path.read_text()));s=solve_axis_bh(case);p=case.partition
        api=out/'api-native'/name;result=save_axis_bh_run(case,s,api);restored=read_axis_bh_run(api)
        assert axis_bh_result(restored)==result;np.testing.assert_array_equal(restored.aphi_over_r_t,s.aphi_over_r_t)
        ...
        binary=out/'cli-native'/name;assert json.loads(cli(['solve-axis-bh',input_path,'--out',binary]))==result
        assert json.loads(cli(['replay-axis-bh',binary]))==result;assert _snapshot(binary)==_snapshot(api)
        ...
        points=np.vstack((points,p.mesh.points_rz_m[p.mesh.axis_nodes[len(p.mesh.axis_nodes)//2]]))
        point_file=out/f'{name}.points.json';point_file.write_text(json.dumps(points.tolist())+'\n');expected=s.probe_at(points);probe=export_axis_bh_probe(api,out/f'{name}.api-probe.json',points)
        for key,value in expected.items():assert probe[key]==value
        cli_probe=out/f'{name}.cli-probe.json';cli(['probe-axis-bh',binary,'--points',point_file,'--out',cli_probe]);assert json.loads(cli_probe.read_text())==probe
        assert len(probe['fields'])==6 and 'phasor' not in probe['conventions'] and 'mu_r' not in probe
        assert probe['fields']['Aphi_Wb_per_m'][-1]==probe['fields']['Br_T'][-1]==probe['fields']['Hr_A_per_m'][-1]==0.
    ...
    for record in failures:
        name=record['name'];input_path=reference/(name+'.json');case=AxisBHCase.from_dict(json.loads(input_path.read_text()))
        ...
        assert failure.report==record['failure'] and failure.report['schema_version']==2 and failure.report['coefficient_field']=='aphi_over_r_t';api=out/'api-failure'/name;result=save_axis_bh_failure(case,failure,api)
        assert read_axis_bh_failure(api)==result;binary=out/'cli-failure'/name
        assert json.loads(cli(['solve-axis-bh',input_path,'--out',binary],expected=1))==result
        assert json.loads(cli(['replay-axis-bh',binary],expected=1))==result;assert _failure_snapshot(binary)==_failure_snapshot(api)
    ...
    cli(['probe-axis-bh',binary,'--points',outside,'--out',out/'rejected-probe.json'],expected=2);assert not (out/'rejected-probe.json').exists()
    cli(['solve-axis-bh',reference/f'{records[0]["name"]}.json','--out',binary],expected=2)
    cli(['probe-axis-bh',binary,'--points',outside,'--mode','1','--out',out/'rejected-mode.json'],expected=2);assert not (out/'rejected-mode.json').exists()
    boolean=out/'boolean-points.json';boolean.write_text('[[false,0]]\n');cli(['probe-axis-bh',binary,'--points',boolean,'--out',out/'rejected-boolean.json'],expected=2);assert not (out/'rejected-boolean.json').exists()
    failed=out/'cli-failure'/failure_records[0]['name'];cli(['probe-axis-bh',failed,'--points',outside,'--out',out/'rejected-failure-probe.json'],expected=2);assert not (out/'rejected-failure-probe.json').exists()
    cli(['solve-axis-bh',reference/f'{failure_records[0]["name"]}.json','--out',failed],expected=2)
    ...
    cli(['solve-axis-bh',invalid,'--out',out/'invalid-case-native'],expected=2);assert not (out/'invalid-case-native').exists()
    ...
    report=dict(status='PASS',cases=len(records),failure_cases=len(failure_records),native_saves=2*len(records),failure_saves=2*len(failure_records),records=records,failure_records=failure_records,cli_commands=commands,
        native_files_unchanged=len(native_hashes),reference_files_unchanged=len(originals),original_physics='unchanged axis nonlinear solver/fields/integrals and exact successful quantities or failure histories from independent validation',
