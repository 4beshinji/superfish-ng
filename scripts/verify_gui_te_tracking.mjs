...
try {
  ...
  await fill("#tracking-ids",'["r1z1","r1z2","r2z1","r1z3","r2z2","r1z4"]');await click("#tracking-retain");
  await click("#tracking-compare");await wait('trackingResult?.document.status==="PASS" && !trackingBusy');
  await check("merge is displayed as a subspace with unresolved individual IDs",'document.querySelector("#tracking-status").textContent.includes("個別IDは未確定") && document.querySelector("#tracking-matches tbody").rows.length===5 && trackingResult.document.tracking.cluster_transitions.events[0].kind==="MERGE"');
