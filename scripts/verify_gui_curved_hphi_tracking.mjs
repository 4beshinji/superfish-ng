...
if (!args["--url"] || !args["--out"] || !args["--request"])
  throw Error(
    "Usage: node scripts/verify_gui_curved_hphi_tracking.mjs --url LAUNCH_URL --out NEW_DIRECTORY --request REQUEST_JSON",
  );
...
try {
  ...
  if(!isDeepStrictEqual(await ev('trackingFromForm()'),request))throw Error('curved request changed');
  await check('complete quadratic comparison preview and curved native choices',`$('tracking-preview').textContent.includes('三角形') && Array.from($('tracking-previous').options).some(o=>o.value===${JSON.stringify(input.source_ids[0])})`);
  ...
  await check('curved E/H result displays identity and finite comparison diagnostics',`$('tracking-current-ids').textContent.includes('TEM') && $('tracking-resolution').textContent.includes('比較三角形')`);
  const historyRequest={format:'superfish_ng_curved_hphi_tracking_history_request',history_version:1,step_count:1,max_steps:3};
  ...
  await check('curved ordered history retains request, original IDs and step request',`currentHphiHistory.result.status==='PASS' && currentHphiHistory.result.current_mode_ids[0]==='TEM' && JSON.stringify(currentHphiHistory.step_requests[0])===JSON.stringify(currentTracking.request)`);
  ...
  await check('history opens original curved Project display unit and N/A axial RF',`$('unit').value==='m' && $('result').textContent.includes('N/A')`);
