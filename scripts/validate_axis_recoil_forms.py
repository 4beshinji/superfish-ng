...
"""Independent axis recoil tensor forms, remanent work and zero-H equilibrium."""
...
def make_partition(holes,pattern,scale,shift):
    ...
    angles=[0.,0.,0.] if pattern!=2 else [0.,.3,-.6];count=int(labels.max())+1
    ...
    return AxisRecoilPartition(mesh,materials,regions),{r.id:float(v) for r,v in zip(regions,current)},nu,h,br,current,labels
...
def main():
    ...
    polynomial_checks=zero_h=0;minimum_eigen_ratio=1.
    def error(a,b):
    ...
    for holes in (0,1,2):
        for order in (1,2):
            for pattern in (0,1,2):
                for scale in (.5,2.):
                    for shift in (0.,-.25):
                        name=f'axis-recoil-holes{holes}-p{order}-pattern{pattern}-scale{scale}-shift{shift}'
                        p,current,nu,h,br,densities,labels=make_partition(holes,pattern,scale,shift)
                        space,k,f,m,report=axis_recoil_forms(p,current,order)
                        ...
                        if scale==.5:pairs[key]=k.toarray(),f,m,constant
                            ...
                            old_k,old_f,old_m,old_c=pairs[key];differences=[error(k.toarray()/64,old_k),error(f/256,old_f),error(m/64,old_m),abs(constant/(64*old_c)-1)]
                        ...
                        if shift==0.:pairs[motion]=k.toarray(),f,m,constant
                            ...
                            assert max(differences)<1e-11;maxima['transform']=max(maxima['transform'],*differences)
                        permuted=AxisRecoilPartition(p.mesh,p.materials[::-1],p.regions[::-1]);_,pk,pf,pm,_=axis_recoil_forms(permuted,current,order)
                        ...
                        _,nk,nf,nm,_=axis_recoil_forms(AxisRecoilPartition.from_dict(raw),{name:-value for name,value in current.items()},order)
                        ...
                        _,sk,sf,sm,sq=axis_recoil_forms(AxisRecoilPartition.from_dict(raw),current,order)
                        ...
                        if pattern in (0,1):
                            ...
                            expected_constant=.5*sum(float(br[i]@h[i])*moment(0,0,i) for i in range(len(nu)))
                            ...
                            for a,b in polynomials:
                                c=radial**a*z**b
                                energy=sum(nu[i,1,1]*(a+2)**2*moment(2*a,2*b,i)+(nu[i,0,0]*b*b*moment(2*a+2,2*b-2,i)-2*nu[i,0,1]*b*(a+2)*moment(2*a+1,2*b-1,i) if b else 0.) for i in range(len(nu)))
                                work=sum(densities[i]*moment(a+1,b,i) for i in range(len(nu)))
                                rem_work=sum((a+2)*h[i,1]*moment(a,b,i)-(b*h[i,0]*moment(a+1,b-1,i) if b else 0.) for i in range(len(nu)))
                                differences=[abs(c@k@c/energy-1),abs(c@f-work)/max(np.linalg.norm(c)*np.linalg.norm(f),abs(work)),abs(c@m-rem_work)/max(np.linalg.norm(c)*np.linalg.norm(m),abs(rem_work))]
                                assert max(differences)<1e-11;maxima['polynomial']=max(maxima['polynomial'],*differences);polynomial_checks+=1
                        if pattern==0:
                            c=np.full(len(points),br[0,1]/2);equilibrium=error(k@c,m)
                        ...
                        assert AxisRecoilPartition.from_dict(json.loads(path.read_text())['partition']).to_dict()==p.to_dict()
    ...
    assert len(records)==72 and polynomial_checks==216 and zero_h==24 and fingerprints()==before
    ...
    result=dict(status='PASS',cases=len(records),partition_permutations=len(records),partition_roundtrips=len(preserved),positive_spectra=len(records),
        ...
        independent_polynomial_checks=polynomial_checks,maximum_relative_differences=maxima,minimum_positive_eigenvalue_ratio=minimum_eigen_ratio,
        interpretation='axis-connected a=Aphi/r[T], tensor K[m^4/H], separate current/remanence loads[A m^2], full 3D constitutive potentials[J]; regular axis material extension and uniform B=Brem zero-H equilibrium; no boundary solve or discretization-error acceptance',
