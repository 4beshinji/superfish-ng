...
try {
  ...
  await click('#study-points button[data-point="0"]');await wait('selected!==null && project.case.schema_version===2');report.previous=await ev('selected');
  await click('#tracking-select-previous');await wait(`document.querySelector('#tracking-previous').value===${JSON.stringify(report.previous)}`);
  await click('#study-points button[data-point="1"]');await wait(`selected!==${JSON.stringify(report.previous)} && project.case.schema_version===2`);report.scaled_source=await ev('selected');
  report.convergence=await execute('#convergence-start','細分診断','currentConvergence?.state?.id');
  await click('#convergence-points button[data-level="1"]');await wait(`selected!==${JSON.stringify(report.scaled_source)} && project.case.schema_version===2`);report.current=await ev('selected');
  await click('#tracking-select-current');await wait(`document.querySelector('#tracking-current').value===${JSON.stringify(report.current)}`);
  await fill('#tracking-scale',2);await fill('#tracking-current-refinements',1);await fill('#tracking-previous-count',1);await fill('#tracking-current-count',1);await fill('#tracking-ids','["fundamental"]');
  report.tracking=await execute('#tracking-start','モード追跡','selectedTracking');report.request=await ev('currentTracking.request');
  await check('polygon mapped worker follows fundamental','currentTracking.result.status==="PASS" && currentTracking.result.result_version===2 && currentTracking.result.current_mode_ids[0]==="fundamental"');
  await click('#tracking-save');const saved=await download('planar-tracking-request.json');if(!isDeepStrictEqual(JSON.parse(saved),report.request))throw Error('polygon request download differs');
  await fill('#tracking-scale',3);await loadFile('#tracking-open',out+'/downloads/planar-tracking-request.json');await wait('document.querySelector("#tracking-scale").value==="2"');
  await check('complete polygon mapping roundtrip','JSON.stringify(trackingFromForm())===JSON.stringify(currentTracking.request)');
  await click('#tracking-result-save');const resultFile=await download('planar-tracking-results.json');if(!isDeepStrictEqual(JSON.parse(resultFile),await ev('currentTracking.result')))throw Error('tracking result differs');report.checks.push({operation:'complete polygon tracking result download',passed:true});
  await click('#tracking-current-import');await wait(`selected!==${JSON.stringify(report.current)} && project.case.schema_version===2`);report.imported=await ev('selected');await click('#plot');await wait('!document.querySelector("#image").hidden && document.querySelector("#image").naturalWidth>100',90000);report.checks.push({operation:'copied polygon import and original field plot',passed:true});
  const before=await ev('JSON.stringify(trackingFromForm())');await writeFile(out+'/invalid.json',JSON.stringify({...report.request,mapping:{...report.request.mapping,rotation:.1}}));await loadFile('#tracking-open',out+'/invalid.json');await wait('!document.querySelector("#error").hidden');if(await ev('JSON.stringify(trackingFromForm())')!==before)throw Error('invalid mapping replaced inputs');report.checks.push({operation:'unsupported rotation preserves inputs',passed:true});
  await call('Page.navigate',{url:launch.origin+'/planar.html?job='+report.tracking},sessionId);await wait(`typeof selectedTracking!=='undefined' && selectedTracking===${JSON.stringify(report.tracking)}`,90000);
  await check('polygon URL restores result and mapping','currentTracking.result.status==="PASS" && trackingFromForm().mapping.scale===2 && !document.querySelector("#tracking-polygon-options").hidden');
  await ev('document.querySelector("#tracking-result").scrollIntoView({block:"start"})');const screenshot=await call('Page.captureScreenshot',{format:'png'},sessionId);await writeFile(out+'/polygon-tracking.png',Buffer.from(screenshot.data,'base64'));
