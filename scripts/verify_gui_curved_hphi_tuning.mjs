...
if (!args["--url"] || !args["--out"] || !args["--request"])
  throw Error(
    "Usage: node scripts/verify_gui_curved_hphi_tuning.mjs --url LAUNCH_URL --out NEW_DIRECTORY --request REQUEST_JSON",
  );
...
try {
  ...
  if(request.format!=='superfish_ng_curved_hphi_tune')throw Error('curved request required');
  ...
  await check('complete curved Project preview preserves display units',`$('hphi-tune-preview').textContent.includes('三角形') && $('unit').value===${JSON.stringify(request.project.display_length_unit)}`);
  ...
  await check('PAUSED curve worker appears with separate incomplete frequency gates',`currentHphiTune.document.status==='PAUSED' && $('hphi-tune-gates').textContent.includes('未実施')`);
  ...
  for(let n=0;n<2;n++){
    ...
    await wait(`$('hphi-tune-document').value.includes('superfish_ng_curved_hphi_tune')`,resultTimeout);
    if(await ev('currentHphiTune.serialized')!==text)throw Error('checkpoint replay differs');
  }
  report.checks.push({operation:'same owned curved checkpoint reopened twice',passed:true});
  ...
  report.checks.push({operation:'resume creates separate curve worker with inherited prefix',passed:true});
