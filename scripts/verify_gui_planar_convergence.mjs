...
try {
  ...
  const execute=async()=>{const previous=await ev('document.querySelector("#jobs .job")?.dataset.job');await click('#convergence-start');await wait(`document.querySelector('#jobs .job')?.dataset.job!==${JSON.stringify(previous)} && document.querySelector('#jobs .job strong').textContent.includes('細分診断') && document.querySelector('#jobs .job button')?.textContent==='結果を開く' && !document.querySelector('#jobs .job button').disabled`,180000);const id=await ev('document.querySelector("#jobs .job").dataset.job');await click('#jobs .job button');await wait(`typeof currentConvergence!=='undefined' && document.querySelector('#convergence-selection').textContent.includes(${JSON.stringify(id)})`,60000);return id;};
  await fill('#nx',4);await fill('#ny',3);await fill('#modes',3);
  report.rectangle_job=await execute();report.rectangle_request=await ev('currentConvergence.request');
  await check('three-level FEM diagnostic separates frequency E H RF','currentConvergence.result.status==="PASS" && Object.keys(currentConvergence.result.decisions[0].checks).length===4 && currentConvergence.result.surface_peak_accuracy==="not_checked"');
  await click('#convergence-save');const saved=await download('planar-convergence.json');if(!isDeepStrictEqual(JSON.parse(saved),report.rectangle_request))throw Error('request download differs');
  await fill('#convergence-ranks','[2]');await loadFile('#convergence-open',out+'/downloads/planar-convergence.json');await wait('document.querySelector("#convergence-ranks").value==="[1]"');
  await check('full request roundtrip','JSON.stringify(convergenceFromForm())===JSON.stringify(currentConvergence.request)');
  await click('#convergence-result-save');const diagnostic=await download('planar-convergence-results.json');if(!isDeepStrictEqual(JSON.parse(diagnostic),await ev('currentConvergence.result')))throw Error('diagnostic download differs');report.checks.push({operation:'complete diagnostic download matches displayed native replay',passed:true});
  await click('#convergence-points button[data-level="2"]');await wait('selected!==null && project.case.schema_version===2');report.imported_point=await ev('selected');
  await click('#plot');await wait('!document.querySelector("#image").hidden && document.querySelector("#image").naturalWidth>100',90000);
  await check('finest explicit mesh import and field plot','project.case.mesh.triangles.length===384');
  await ev('document.querySelector("#convergence-result").scrollIntoView({block:"start"})');const screenshot=await call('Page.captureScreenshot',{format:'png'},sessionId);await writeFile(out+'/convergence-results.png',Buffer.from(screenshot.data,'base64'));
  const invalid=out+'/invalid.json';await writeFile(invalid,JSON.stringify({...report.rectangle_request,levels:2}));const old=await ev('JSON.stringify(convergenceFromForm())');await loadFile('#convergence-open',invalid);await wait('!document.querySelector("#error").hidden');if(await ev('JSON.stringify(convergenceFromForm())')!==old)throw Error('invalid request replaced inputs');report.checks.push({operation:'invalid two-level request preserves input',passed:true});
  await click('#new');await wait('project.case.schema_version===1 && document.querySelector("#nx").value==="16"');await fill('#width',200);await fill('#height',200);await fill('#nx',4);await fill('#ny',4);await fill('#modes',3);report.square_job=await execute();
  await check('near-degenerate square is UNVERIFIED','currentConvergence.result.status==="UNVERIFIED" && !currentConvergence.result.decisions[0].correspondence_verified');
  await call('Page.navigate',{url:launch.origin+'/planar.html?job='+report.rectangle_job},sessionId);await wait(`typeof currentConvergence!=='undefined' && document.querySelector('#convergence-selection').textContent.includes(${JSON.stringify(report.rectangle_job)})`,60000);await check('persistent diagnostic URL restores request','currentConvergence.result.status==="PASS"');
