...
if (!args["--url"] || !args["--out"] || !args["--request"])
  throw Error(
    "Usage: node scripts/verify_gui_hphi_tune_recovery.mjs --url LAUNCH_URL --out NEW_DIRECTORY --request REQUEST_JSON",
  );
...
try {
  ...
  if(request.schema_version!==3)throw Error('recovery request version 3 required');
  ...
  for(let n=0;n<2;n++){
    ...
    await wait(`JSON.parse($('hphi-tune-document').value).schema_version===3`,resultTimeout);
