...
try {
  ...
  const expected=JSON.parse(await readFile('examples/planar/tuning.json','utf8'));
  await loadFile('#tune-open','examples/planar/tuning.json');await wait('project.case.mesh.nx===6 && $("tune-mode-id").value==="x"');
  ...
  await loadFile('#tune-open','examples/planar/tuning.json');await wait('JSON.parse($("tune-controls").value).minimum_overlap===.9');
  ...
  await check('resumed bisection and final refinement meet separate gates','currentTune.document.status==="TUNED" && currentTune.document.trials.length===4 && currentTune.document.decision.refined_target_met && currentTune.document.decision.mesh_difference_met && $("tune-resume").disabled && $("tune-decision").textContent.includes("粗細差")');
  await writeFile(out+'/final-checkpoint.json',await ev('currentTune.serialized'));
  await click('#tune-trials [data-tune-trial="4"]');await wait('selected!==null && $("mode").value==="1"');report.imported_job=await ev('selected');
  await check('target trial opens the verified native mode and per-length RF','result.case.geometry.width_m===.32 && $("quantities").textContent.includes("J/m") && $("na").textContent.includes("N/A")');
  ...
  const cancel=structuredClone(expected);cancel.frequency_tolerance_hz=1e-5;cancel.parameter_tolerance=1e-15;cancel.max_trials=60;
