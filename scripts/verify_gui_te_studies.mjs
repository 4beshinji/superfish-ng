...
try {
  ...
  await loadFile('#open-study',args['--study']);
  await wait('document.querySelector("#polarization").value==="te" && document.querySelector("#study-kind").value==="sweep"');
  await click('#start-study');
  await wait('document.querySelector("#jobs .job button")?.textContent==="結果を開く" && !document.querySelector("#jobs .job button").disabled',60000);
  await click('#jobs .job button');
  await wait('activeStudy?.physics==="axisymmetric_m0_te"');
  await check('TE sweep displays independent spectra and N/A for every R/Q','activeStudy.numerical_status==="UNVERIFIED" && document.querySelector("#study-report").textContent.includes("追跡は未実施") && [...document.querySelectorAll("#study-report table tr")].slice(1).every(row=>row.children[3].textContent.includes("N/A"))');
  report.study=await ev('activeStudy');
  ...
  const shot=await call('Page.captureScreenshot',{captureBeyondViewport:true,clip:rect},sessionId);await writeFile(out+'/study.png',Buffer.from(shot.data,'base64'));
  ...
  await check('TE Study point imports and renders the saved TE field','currentResult.state.origin==="imported" && currentResult.result.modes[0].r_over_q_accelerator_ohm===null');
