...
if (!args["--url"] || !args["--out"] || !args["--request"])
  throw Error(
    "Usage: node scripts/verify_gui_material_hphi_tune_recovery.mjs --url LAUNCH_URL --out NEW_DIRECTORY --request REQUEST_JSON",
  );
...
try {
  ...
  if(request.format!=='superfish_ng_material_hphi_tune'||request.schema_version!==1)throw Error('material recovery request required');
