...
"""Planar electrostatic native/CLI replay of analytically validated source problems."""
import argparse,hashlib,json,os,subprocess,sys,time
from pathlib import Path
import numpy as np
ROOT=Path(__file__).resolve().parents[1];sys.path[:0]=[str(ROOT),str(ROOT/'src')]
from superfish_ng.planar_electrostatic import PlanarElectrostaticCase,solve_planar_electrostatic
...
def main():
    ...
    source_report=json.loads((reference/'report.json').read_text());assert source_report['status']=='PASS' and source_report['cases']==88 and source_report['layered_cases']==32 and source_report['manufactured_cases']==32 and source_report['poisson_cases']==24
    ...
    inventory=capabilities();assert json.loads(cli(['capabilities']))==inventory;description=inventory['planar_electrostatic']
    ...
    for record in source_report['records']:
        name=record['name']
        if ((name.startswith('layer-') and '-eps1.0-' in name and name.endswith('-motion0'))
            or (name.startswith('manufactured-') and '-s0.5-a100.0' in name)
            or (name.startswith('poisson-') and ('-p1-n32-' in name or '-p2-n16-' in name))):selected.append(record)
    assert len(selected)==24
    for record in selected:
        name=record['name'];input_path=reference/(name+'.json');case=PlanarElectrostaticCase.from_dict(json.loads(input_path.read_text()));s=solve_planar_electrostatic(case);p=case.partition
        api=out/'api-native'/name;result=save_planar_electrostatic_run(case,s,api);restored=read_planar_electrostatic_run(api)
        assert planar_electrostatic_result(restored)==result;np.testing.assert_array_equal(restored.potential_relative_to_reference_v,s.potential_relative_to_reference_v)
        assert result['quantities']==record['quantities'];assert result['physics']==case.to_dict()['physics']=='linear_electrostatic'
        binary=out/'cli-native'/name;emitted=json.loads(cli(['solve-planar-electrostatic',input_path,'--out',binary]));assert emitted==result
        assert json.loads(cli(['replay-planar-electrostatic',binary]))==result;assert _snapshot(binary)==_snapshot(api)
        ...
        expected=s.probe_at(points);probe=export_planar_electrostatic_probe(api,out/f'{name}.api-probe.json',points)
        for key,value in expected.items():assert probe[key]==value
        cli_probe=out/f'{name}.cli-probe.json';cli(['probe-planar-electrostatic',binary,'--points',point_file,'--out',cli_probe]);assert json.loads(cli_probe.read_text())==probe
    ...
    cli(['probe-planar-electrostatic',binary,'--points',outside,'--out',out/'rejected-probe.json'],expected=2);assert not (out/'rejected-probe.json').exists()
    cli(['solve-planar-electrostatic',reference/f'{records[0]["name"]}.json','--out',binary],expected=2)
    cli(['probe-planar-electrostatic',binary,'--points',outside,'--mode','1','--out',out/'rejected-mode.json'],expected=2);assert not (out/'rejected-mode.json').exists()
    boolean=out/'boolean-points.json';boolean.write_text('[[false,0]]\n')
    cli(['probe-planar-electrostatic',binary,'--points',boolean,'--out',out/'rejected-boolean.json'],expected=2);assert not (out/'rejected-boolean.json').exists()
    ...
    report=dict(status='PASS',cases=len(records),native_saves=2*len(records),records=records,cli_commands=commands,
        ...
        original_physics='unchanged planar electrostatic solver/field/integral source and exact quantities from independent analytical validation',
