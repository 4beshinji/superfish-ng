...
if (!args["--url"] || !args["--out"])
  throw Error(
    "Usage: node scripts/verify_gui_planar_tracking_history.mjs --url LAUNCH_URL --out NEW_DIRECTORY",
  );
...
try {
  ...
  await fill('#history-step-ids',JSON.stringify([report.merged]));
  report.first_history=await execute('#history-start','追跡履歴','selectedHistory');
  await check('one-step history fully replays a degenerate ID set','currentHistory.result.can_extend && !currentHistory.result.individual_ids_complete && currentHistory.result.steps.length===1');
  await click('#history-save');const historyRequest=JSON.parse(await download('planar-history-request.json'));
  if(!isDeepStrictEqual(historyRequest,await ev('historyFromForm()')))throw Error('history request download differs');
  await fill('#history-budget',7);await loadFile('#history-open',out+'/downloads/planar-history-request.json');await wait('document.querySelector("#history-budget").value==="100" && document.querySelector("#history-step-ids").value===JSON.stringify([""])');
  await fill('#history-step-ids',JSON.stringify([report.merged]));await check('history conditions roundtrip with explicit local step selection','historyFromForm().step_count===1 && historyFromForm().max_steps===100');
  await ev(`document.querySelector('#history-next').value=${JSON.stringify(report.split)}`);
  report.extended_history=await execute('#history-extend','追跡履歴','selectedHistory');
  await check('extension preserves the ID set through splitting','currentHistory.result.can_extend && !currentHistory.result.individual_ids_complete && currentHistory.result.steps.length===2 && currentHistory.result.current_identity_groups[0].ids.length===2');
  await click('#history-result-save');if(!isDeepStrictEqual(JSON.parse(await download('planar-history-results.json')),await ev('currentHistory.result')))throw Error('history result download differs');report.checks.push({operation:'full history download matches saved replay',passed:true});
  await ev(`document.querySelector('#history-next').value=${JSON.stringify(report.crossing)}`);await click('#history-extend');await wait('!document.querySelector("#error").hidden');
  await check('PASS pair with wrong native ancestry cannot extend history',`selectedHistory===${JSON.stringify(report.extended_history)} && document.querySelector('#error').textContent.includes('native')`);
  await click('#history-source');await wait(`selected!==${JSON.stringify(report.copied_source)} && project.case.geometry.width_m===.22`);report.history_source=await ev('selected');await click('#plot');await wait('!document.querySelector("#image").hidden && document.querySelector("#image").naturalWidth>100',90000);report.checks.push({operation:'history owns an importable and plotable final spectrum',passed:true});
  await click('#history-use-groups');await wait('document.querySelector("#tracking-ids").value==="null" && document.querySelector("#tracking-current").value===""');report.history_carried=await ev('document.querySelector("#tracking-previous").value');
  await ev(`document.querySelector('#tracking-current').value=${JSON.stringify(report.current)}`);await fill('#tracking-current-count',1);
  report.unverified_pair=await execute('#tracking-start','モード追跡','selectedTracking');await check('incomplete destination band remains UNVERIFIED','currentTracking.result.status==="UNVERIFIED"');
  await ev(`document.querySelector('#history-next').value=${JSON.stringify(report.unverified_pair)}`);report.stopped_history=await execute('#history-extend','追跡履歴','selectedHistory');
  await check('UNVERIFIED terminal step stops extension and ID carry','!currentHistory.result.can_extend && currentHistory.result.current_identity_groups===null && currentHistory.result.steps.length===3 && document.querySelector("#history-extend").disabled && document.querySelector("#history-use-groups").disabled');
  await fill('#history-step-ids',JSON.stringify([report.merged]));await fill('#history-budget',1);report.budget_history=await execute('#history-start','追跡履歴','selectedHistory');
  await check('step budget stops extension while retaining a verified ID set','currentHistory.result.status==="PASS" && !currentHistory.result.can_extend && currentHistory.result.stop_reason.includes("max_steps") && currentHistory.result.current_identity_groups.length===1 && document.querySelector("#history-extend").disabled && !document.querySelector("#history-use-groups").disabled');
  const old=await ev('JSON.stringify(historyFromForm())');await writeFile(out+'/invalid-history.json',JSON.stringify({...historyRequest,history_version:2}));await loadFile('#history-open',out+'/invalid-history.json');await wait('!document.querySelector("#error").hidden');if(await ev('JSON.stringify(historyFromForm())')!==old)throw Error('invalid history replaced conditions');report.checks.push({operation:'invalid history request preserves input',passed:true});
  await call('Page.navigate',{url:launch.origin+'/planar.html?job='+report.stopped_history},sessionId);await wait(`typeof selectedHistory!=='undefined' && selectedHistory===${JSON.stringify(report.stopped_history)}`,90000);
  await check('history URL restores the fully replayed stopped chain','currentHistory.result.status==="UNVERIFIED" && currentHistory.result.steps.length===3');
  await ev('document.querySelector("#history-result").scrollIntoView({block:"start"})');const screenshot=await call('Page.captureScreenshot',{format:'png'},sessionId);await writeFile(out+'/history-results.png',Buffer.from(screenshot.data,'base64'));
