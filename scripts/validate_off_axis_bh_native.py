...
"""Off-axis nonlinear B-H native/CLI comparison on independently checked Cases."""
...
from superfish_ng.off_axis_bh_saved import (save_off_axis_bh_run,read_off_axis_bh_run,off_axis_bh_result,export_off_axis_bh_probe,
    save_off_axis_bh_failure,read_off_axis_bh_failure,_snapshot,_failure_snapshot)
...
def main():
    ...
    inventory=capabilities();assert json.loads(cli(['capabilities']))==inventory;description=inventory['off_axis_bh']
    ...
    for record in source_report['records']:
        ...
        elif ((kind=='radial_field' and '-s0.5-' in name) or (kind in ('layered','current') and '-n8-s0.5-' in name)
            or (kind=='linear' and '-s0.5-sign1.0' in name) or (kind=='zero' and '-holes1-shift-0.25' in name)):selected.append(record)

    assert len(selected)==24 and len(failures)==8
    for record in selected:
        name=record['name'];input_path=reference/(name+'.json');case=OffAxisBHCase.from_dict(json.loads(input_path.read_text()));s=solve_off_axis_bh(case);p=case.partition
        api=out/'api-native'/name;result=save_off_axis_bh_run(case,s,api);restored=read_off_axis_bh_run(api)
        assert off_axis_bh_result(restored)==result;np.testing.assert_array_equal(restored.psi_relative_to_reference_wb,s.psi_relative_to_reference_wb);np.testing.assert_array_equal(restored.psi_wb,s.psi_wb);assert restored.reference_psi_wb==s.reference_psi_wb
        ...
        binary=out/'cli-native'/name;assert json.loads(cli(['solve-off-axis-bh',input_path,'--out',binary]))==result
        assert json.loads(cli(['replay-off-axis-bh',binary]))==result;assert _snapshot(binary)==_snapshot(api)
        ...
        points=np.vstack((points,p.mesh.points_rz_m[0]))
        point_file=out/f'{name}.points.json';point_file.write_text(json.dumps(points.tolist())+'\n');expected=s.probe_at(points);probe=export_off_axis_bh_probe(api,out/f'{name}.api-probe.json',points)
        for key,value in expected.items():assert probe[key]==value
        cli_probe=out/f'{name}.cli-probe.json';cli(['probe-off-axis-bh',binary,'--points',point_file,'--out',cli_probe]);assert json.loads(cli_probe.read_text())==probe
        assert len(probe['fields'])==6 and 'phasor' not in probe['conventions'] and 'mu_r' not in probe
        psi=np.asarray(probe['fields']['psi_Wb']);aphi=np.asarray(probe['fields']['Aphi_Wb_per_m']);np.testing.assert_allclose(aphi,psi/points[:,0],rtol=1e-13,atol=0.)
        if record['kind']=='zero':
            for field in ('Br_T','Bz_T','Hr_A_per_m','Hz_A_per_m'):np.testing.assert_array_equal(probe['fields'][field],0.)
        b=np.column_stack((probe['fields']['Br_T'],probe['fields']['Bz_T']));h=np.column_stack((probe['fields']['Hr_A_per_m'],probe['fields']['Hz_A_per_m']))
    ...
    for record in failures:
        name=record['name'];input_path=reference/(name+'.json');case=OffAxisBHCase.from_dict(json.loads(input_path.read_text()))
        ...
        assert failure.report==record['failure'] and failure.report['schema_version']==2 and failure.report['coefficient_field']=='psi_relative_to_reference_wb';api=out/'api-failure'/name;result=save_off_axis_bh_failure(case,failure,api)
        assert read_off_axis_bh_failure(api)==result;binary=out/'cli-failure'/name
        assert json.loads(cli(['solve-off-axis-bh',input_path,'--out',binary],expected=1))==result
        assert json.loads(cli(['replay-off-axis-bh',binary],expected=1))==result;assert _failure_snapshot(binary)==_failure_snapshot(api)
        ...
        failure_records.append(dict(name=name,reason=result['reason'],native_files_identical=True,last_valid_is_null=result['last_valid_coefficients'] is None));print('DONE',name,flush=True)
    binary=out/'cli-native'/records[0]['name'];outside=out/'outside-points.json';outside.write_text('[[100,100]]\n')
    cli(['probe-off-axis-bh',binary,'--points',outside,'--out',out/'rejected-probe.json'],expected=2);assert not (out/'rejected-probe.json').exists()
    cli(['solve-off-axis-bh',reference/f'{records[0]["name"]}.json','--out',binary],expected=2)
    cli(['probe-off-axis-bh',binary,'--points',outside,'--mode','1','--out',out/'rejected-mode.json'],expected=2);assert not (out/'rejected-mode.json').exists()
    boolean=out/'boolean-points.json';boolean.write_text('[[false,0]]\n');cli(['probe-off-axis-bh',binary,'--points',boolean,'--out',out/'rejected-boolean.json'],expected=2);assert not (out/'rejected-boolean.json').exists()
    failed=out/'cli-failure'/failure_records[0]['name'];cli(['probe-off-axis-bh',failed,'--points',outside,'--out',out/'rejected-failure-probe.json'],expected=2);assert not (out/'rejected-failure-probe.json').exists()
    cli(['solve-off-axis-bh',reference/f'{failure_records[0]["name"]}.json','--out',failed],expected=2)
    ...
    cli(['solve-off-axis-bh',invalid,'--out',out/'invalid-case-native'],expected=2);assert not (out/'invalid-case-native').exists()
    ...
    report=dict(status='PASS',cases=len(records),failure_cases=len(failure_records),native_saves=2*len(records),failure_saves=2*len(failure_records),records=records,failure_records=failure_records,cli_commands=commands,
        native_files_unchanged=len(native_hashes),reference_files_unchanged=len(originals),original_physics='unchanged off-axis nonlinear solver/fields/integrals and exact successful quantities or failure histories from independent validation',
