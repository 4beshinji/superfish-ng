...
2026-09-14追補：[曲線アフィンStudy](CURVED_AFFINE_STUDY.md)で、元Projectと参照分割を保つ履歴付き形状掃引へ接続した。一般形状/初期再メッシュの掃引と自動番号対応は残る。以下は各段階の履歴。

2026-09-14追補：[局所分割の固定](FROZEN_CURVED_REFINEMENT.md)で、各marked段階に任意のsplit_pattern版1を追加。
元番号付きメッシュ・分割辺・遷移対角線を保存して、宣言アフィン形状変更後も同じ親子関係を再構築する。
未指定の旧入力は従来の幾何依存選択を維持する。一般Studyの形状/初期メッシュ掃引は未対応。以下は各追加時点の記録。
