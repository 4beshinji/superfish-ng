...
| K26 | 非線形BH・材料曲線 | H | 部分 | [単調H(B)表](BH_CURVE.md)、[平面P1解/保存](PLANAR_BH_NATIVE.md)、[軸接続P1解/保存](AXIS_BH_NATIVE.md)と[軸非接続P1非線形磁場の保存・失敗再現・CLI](OFF_AXIS_BH_NATIVE.md)を限定受入。解析/独立1次元BVP比較済み。対象版モデルと原要件の受入照合は未完 | B / S04 |
