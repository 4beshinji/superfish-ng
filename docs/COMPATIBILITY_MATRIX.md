...
更新: 2026-09-08、D01対応保存・再検証CLIの追加後（直前基準 `a26280d`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
| K17 | 形状掃引・モード同定 | U | 部分：独立掃引と条件付き同定、重み付き部分空間追跡・明示円筒写像・2時点保存/再検証CLI | 一般写像、合流/分裂、連続履歴と製品接続 | A / D01 |
...
一般形状の写像・合流/分裂・連続履歴・GUI統合は未完。2時点対応の保存・再検証CLIは実装済みで、K07/K17の全対応ではない。
