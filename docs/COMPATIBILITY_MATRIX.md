...
更新: 2026-09-08、D01完了Studyの追跡GUI追加後（直前基準 `406e636`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
合流/分裂は明示policyで集合継続に対応。一般形状の写像・多対多/個別枝回復は未完。完了Studyの保存場追跡はCLI/API/GUIへ接続。追跡付き自動実行と再開・適応的点追加・tuneは未完。対応比較・履歴のGUI操作は追加済み。2時点対応の保存・再検証CLIと個別ID/部分空間ID集合の履歴/再開は実装済みで、K07/K17の全対応ではない。
