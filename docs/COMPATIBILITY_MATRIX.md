...
更新: 2026-09-08、D01明示メッシュ対応の追加後（直前基準 `3e0d6a3`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
| K17 | 形状掃引・モード同定 | U | 部分：独立掃引と条件付き同定、重み付き部分空間追跡・明示円筒/profile写像・明示メッシュ対応・2時点保存/再検証CLI・個別ID/部分空間ID集合の履歴/再開 | 一般写像、合流/分裂、製品接続 | A / D01 |
...
一般形状の写像・合流/分裂・GUI統合は未完。2時点対応の保存・再検証CLIと個別ID/部分空間ID集合の履歴/再開は実装済みで、K07/K17の全対応ではない。
