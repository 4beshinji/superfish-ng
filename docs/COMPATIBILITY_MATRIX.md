...
更新: 2026-09-08、D01アフィン再メッシュのGUI接続後（直前基準 `a9f7324`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
| K17 | 形状掃引・モード同定 | U | 部分：独立掃引と条件付き同定、重み付き部分空間追跡・明示円筒/profile写像・明示メッシュ対応・2時点保存/再検証CLI・個別ID/部分空間ID集合の履歴/再開・合流/分裂の集合継続・同一領域/アフィン変形の独立再メッシュ比較 | 非アフィン一般写像、個別枝回復、製品接続 | A / D01 |
