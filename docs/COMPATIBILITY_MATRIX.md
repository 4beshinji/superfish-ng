...
| K04 | 内導体・複数境界成分 | U（旧版対象実行） | 部分：[閉同軸円筒](COAXIAL_RF.md)の内導体・外導体・短絡端板 | 一般の内導体と複数の断面境界成分は未対応。領域位相・境界契約と対応する物理空間の拡張を継続 | B / P03 |
...
| K11 | 同軸共振器/TEM系 | U（旧版対象実行） | 部分：[閉同軸円筒Hφ族](COAXIAL_RF.md)のP1/P2 FEM・静的零空間除外・TEM/TM場/RF・専用native/CLI | 両尺度・両次数16FEM/native、全内外導体/端板損失まで限定受入。一般内導体・複数断面境界、Project/Job/GUI、旧版照合は未完 | B / P03 |
