...
| K02 | 楕円・双曲線・接線接続 | D（NT=2楕円/3双曲線）、U（接線操作） | 部分：native楕円/双曲線・接続検査・弦GUI、[曲線P2/保存/描画/固定幾何Study・曲線鏡映・楕円/双曲線GUI](CURVED_ELEMENTS.md) | 接線構築・曲線ピークの物理収束・曲線鏡映GUI・二次幾何近似/総合収束、旧曲線パラメータと対象版での実行検証 | A / C00,G03 |
...
| K06 | 高次要素・誤差推定・適応細分 | U | 部分：P1と実長細分、[P2場/RF/保存・表示](HIGH_ORDER_FIELDS.md) | 精度到達手段として高次場/適応を開発。旧アルゴリズムの再現要件ではない | A / N01,N02,N04 |
...
| K15 | 表面ピークE/B | L | 部分：P1/P2片側推定、角部診断 | 固定丸め半径で収束、境界評価位置/規約の一致 | A / N03 |
