...
| K18 | 周波数調整・制約付き最適化 | U | 部分：[追跡付き1変数tune・連動profile/曲線アフィン座標・再開・細メッシュ判定API/CLI・JobManager・GUI](TUNING.md)、[RF設計制約評価・2変数探索API/CLI](RF_OPTIMIZATION.md) | 非アフィン曲線/任意関数連動・一般制約付き探索/GUI・旧tuner照合 | A / D02,D03 |
