...
| K06 | 高次要素・誤差推定・適応細分 | U | 部分：P1と実長細分 | 精度到達手段として高次場/適応を開発。旧アルゴリズムの再現要件ではない | A / N01,N02,N04 |
...
| K19 | 旧幾何/計算入力デッキ | L（比較用AF等の生成だけ） | 未：汎用旧入力読込なし | 文法、既定値、単位、領域、未対応指定の拒否 | A / C00,C02 |
...
| K23 | 完了管理・再現・配布 | U | 部分：管理ジョブとローカル配布 | 直接保存、外部メッシュ/高次/追加物理との一貫性、他環境 | A / O01,O02,V02 |
