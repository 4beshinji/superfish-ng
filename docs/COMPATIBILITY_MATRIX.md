...
| K25 | 平面/軸対称線形静磁場 | H | 部分 | [平面mu_r/Jz/Azの実FEM・元B/H/磁束](PLANAR_MAGNETOSTATIC_SOLVE.md)と[保存/CLI](PLANAR_MAGNETOSTATIC_NATIVE.md)、[軸接続の正則弱形式](AXIS_MAGNETOSTATIC_FORMS.md)を限定受入。[軸接続の実FEM・元6場/J/Wb](AXIS_MAGNETOSTATIC_SOLVE.md)も限定受入。[軸保存/CLI](AXIS_MAGNETOSTATIC_NATIVE.md)も限定受入。軸非接続・外部境界診断は未完 | B / S02 |
