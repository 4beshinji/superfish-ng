...
更新: 2026-09-08、版4線分フィレット統合後（直前基準 `35380e3`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
| K02 | 楕円・双曲線・接線接続 | D（NT=2楕円/3双曲線）、U（接線操作） | 部分：native楕円/双曲線・接続検査・共通接線・有限弧区間判定/位置誤差上界付き接続・固定直線と弧の接続・線分間フィレットと保存・CLI/GUI、[曲線P2/保存/描画/幾何・固定FEM系列・曲線鏡映GUI・楕円/双曲線GUI](CURVED_ELEMENTS.md) | 円錐曲線弧を含むフィレット、曲線ピークの物理収束・全要件照合、旧曲線パラメータと対象版での実行検証 | A / C00,G03 |
