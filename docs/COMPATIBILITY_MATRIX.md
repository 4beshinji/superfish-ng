...
| K12 | 複数RF材料領域 | D（MAT、MT EPSILON/MU） | 部分 | [領域/界面とHφ K/M基盤](MATERIAL_HPHI_FORMS.md)を限定受入。[専用材料Case・固有解/場/エネルギー/RF](MATERIAL_HPHI_RF.md)も限定受入。材料保存・操作・損失モデル・対象版での材料入力/実行検証は未完 | B / P04 |
