...
更新: 2026-09-08、有限弧接線API追加後（直前基準 `e68002f`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
| K02 | 楕円・双曲線・接線接続 | D（NT=2楕円/3双曲線）、U（接線操作） | 部分：native楕円/双曲線・接続検査・共通接線・有限弧数値判定/明示選択G1接続API、[曲線P2/保存/描画/幾何・固定FEM系列・曲線鏡映GUI・楕円/双曲線GUI](CURVED_ELEMENTS.md) | 弧端/接点区間の認証、接線要求/選択保存・Case/CLI/GUI・閉輪郭検証の統合、曲線ピークの物理収束・全要件照合、旧曲線パラメータと対象版での実行検証 | A / C00,G03 |
