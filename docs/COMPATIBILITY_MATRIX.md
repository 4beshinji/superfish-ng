...
更新: 2026-09-08、D01幾何掃引の適応二分API/CLI追加後（直前基準 `710cdd8`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
合流/分裂は明示policyで集合継続に対応。一般形状の写像・多対多/個別枝回復は未完。完了Studyの保存場追跡はCLI/API/GUIへ接続。追跡付き逐次実行とチェックポイント再開をAPI/CLIへ接続。共通JobManagerの別プロセス実行・中止・再開を追加。ブラウザーの開始/中止/結果/再開も接続。幾何掃引の適応二分API/CLIを追加。適応実行の再開/JobManager/GUI、一般追跡・tuneは未完。対応比較・履歴のGUI操作は追加済み。2時点対応の保存・再検証CLIと個別ID/部分空間ID集合の履歴/再開は実装済みで、K07/K17の全対応ではない。
