...
更新: 2026-09-08、版6直線・弧フィレット統合後（直前基準 `cf3a380`）。最新の親課題状態は [計画冒頭](COMPATIBILITY_PLAN.md)。
...
| K02 | 楕円・双曲線・接線接続 | D（NT=2楕円/3双曲線）、U（接線操作） | 部分：native楕円/双曲線・接続検査・共通接線・有限弧区間判定/位置誤差上界付き接続・固定直線と弧の接続・線分間/有限弧間/直線と弧のフィレットと保存・CLI/GUI、[曲線P2/保存/描画/幾何・固定FEM系列・曲線鏡映GUI・楕円/双曲線GUI](CURVED_ELEMENTS.md) | 弧端/退化の追加分類、曲線ピークの物理収束・全要件照合、旧曲線パラメータと対象版での実行検証 | A / C00,G03 |
