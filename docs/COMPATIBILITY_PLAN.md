...
2026-09-16 JST：本書の残要件を[実行用の90作業カード](development-plan/README.md)へ分解した。現在の受入範囲を維持し、対象版未確認による条件付き追加も別管理する。本書は親要件、リンク先は実行順とコミット境界を示す。
...
2026-09-14 JST：[A01/P0-04の候補比較とADR-018](A01_BACKEND_COMPARISON.md)を受入。
固定cavsim2dの明示Profileと自作NGを、閉PECの2形状・両側3水準・3回の36実FEMで比較した。
解析五量・内部場・双方収束・実DOF/時間と出典を保存し、公開端条件/表示/依存の実問題も記録する。
製品組込みを見送り、候補は独立参照用途に限定。親33=9受入/17進行/6他未受入/1範囲外、全計画goal ACTIVE。
以下の日付付き件数は各段階の履歴であり、現在の親課題集計が優先する。

2026-09-14 JST：N04/D01の[異なる履歴の共通比較分割](CURVED_COMPARISON_OVERLAY.md)を限定受入。
対応する初期P2分割からの別々の局所細分を交差分割し、全要素被覆と可変体積・元FEM場による保存追跡/CLI/GUIを確認した。
一般の初期再メッシュ間対応・連続物理枝・一般精度/効率は残る。親33=8受入/17進行/7他未受入/1範囲外、全計画goal ACTIVEを維持する。
...
親33=8受入/17進行/7他未受入/1範囲外、全計画goal ACTIVEを保持する。以下は各段階の履歴。
...
一般の楕円/双曲線対の必要条件、次数56以下の整数Sturm根分離、元符号/有限元弧/元カスプを確認。
関連35テスト、独立336点の行列式と6条件32候補が合格。候補を交点と認定せず、相手側の実元点・符号・有限弧は未接続。
現在の公開診断11/構築1〜9/GUIへは新経路を接続していない。新フィレット構築・物理ピーク収束とG03全体/C00.V/V02は残る。
親33=8/17/7/1を維持。以下は各段階の履歴である。

2026-09-14 JST：[主軸表現が異なる同一円錐曲線のオフセット](REPARAMETERIZED_CONIC_OFFSETS.md)を限定受入。
診断11で厳密支持/距離等号、有限周期/枝、重なり/共有端点/全反射元点対を扱う。
関連103テスト、独立1,984条件435反射元点対、実Chrome90チェック34取得と全17保存バイト一致が合格。
旧保存診断1〜10/構築1〜9を保持。交差の診断完了で未確認構築の適用を許可しない。
異なる支持・距離の両非零オフセット、異なる物理枝の一般交差、新フィレット構築、物理ピーク収束は残る。
G03全体/C00.V/V02と全計画は未完、親33=8/17/7/1を維持。以下は各段階の履歴である。

2026-09-14 JST：[元の非円円錐曲線と法線オフセットの全交点](CONIC_IMPLICIT_OFFSET.md)を限定受入。
一方の距離0の二次形式代入と全根/元符号/有限枝/退化を診断10へ接続した。
関連97テスト、独立80条件200元点対、既存構築GUI96チェック38取得が合格。旧保存診断1〜9/構築1〜9を保持。
両非零の一般処理と新候補構築、同一非円支持の一部、物理ピーク収束・G03全体/C00.V/V02は残る。
親33=8/17/7/1を維持。以下は各段階の履歴である。

2026-09-14 JST：[円同士の全元点対からのフィレット構築](CIRCULAR_FILLET.md)を限定受入。
構築版9で有限交点/接触と孤立共有端点を候補へ接続し、無限元点対と零長を区別した。
関連62テストの最終合格、独立336条件264元点対と実FEM尺度則、Chrome186チェック86取得を確認。
旧構築1〜8/診断1〜9を保持。非円弧同士の一般交点、物理ピーク収束、G03全体/C00.V/V02は残る。
親33=8/17/7/1を維持。以下は各段階の履歴である。

2026-09-14 JST：[円・非円円錐曲線の全元点対からのフィレット構築](CIRCLE_CONIC_FILLET.md)を限定受入。
構築版8で元点対/順序、有限端点、同一中心/厳密零長と出力誤差を候補/Caseへ接続した。
関連63テスト（62+追加1）、独立44条件100元点対、実FEM尺度則、Chrome154チェック64取得が合格。
旧構築1〜7/診断1〜9を保持。円同士の全根からの構築・非円弧同士の一般交点、G03全体/C00.V/V02は残る。
親33=8/17/7/1を維持。以下は過去の各段階の履歴である。

2026-09-14 JST：[円と非円円錐曲線オフセットの全交点](CIRCLE_CONIC_CROSSINGS.md)を限定受入。
新版9と旧診断1〜8/構築1〜7を分け、全元点/中心数・有限所属・退化を確認した。
関連87件の最終合格、独立53条件、Chrome110チェック44取得が合格。
円/非円フィレットへの接続、非円弧同士の一般交点、G03全体/C00.V/V02は残り、親33=8/17/7/1を維持。

2026-09-14 JST：[全根列挙からの直線・円錐曲線フィレット構築](ALGEBRAIC_CONIC_FILLET.md)を限定受入。
構築版7で端点/カスプ/同一中心の全元点対と認証付き順序を新候補へ接続した。
関連71テスト、非円36/円24独立条件、FEM尺度則、Chrome138チェック54取得が合格。
曲線同士の一般交点、G03全体とC00.V/V02は残り、親33=8/17/7/1を維持する。

2026-09-14 JST：[直線と非円円錐曲線オフセットの全交点](LINE_NONCIRCULAR_CROSSINGS.md)を限定受入。
接触/カスプを含む全根・偽解除外・同一中心・有限所属を関連83項目/独立115条件/Chrome140チェックで確認。
新規版8、旧版1〜7を保持。新候補構築・曲線同士の一般交点等は残り、親33=8/17/7/1を維持。

2026-09-14 JST：[直線と非円楕円・双曲線オフセットの接触](LINE_NONCIRCULAR_OFFSET_CONTACTS.md)を限定受入。
全域0/1点と接触証拠のみを分離し、関連65テスト・独立1,944条件・Chrome118チェック64取得が合格。
新規版7、旧版1〜6を保持。一般非円交点・新候補構築等は残り、親33=8/17/7/1を維持。

2026-09-14 JST：[一般二進回転/直線長を保持する円オフセット](ALGEBRAIC_CIRCULAR_OFFSETS.md)を限定受入。
関連58テスト・独立1,944条件・Chrome初回/再起動62チェック32取得が合格。
新規版6、旧保存版1〜5を保持。非円楕円/双曲線の一般弧端/重解等は残り、親33=8/17/7/1を維持。

2026-09-14 JST：[有理支持の異なる円・直線の有限交点診断](FINITE_CIRCULAR_CROSSINGS.md)を限定受入。
関連49テスト、独立4,896条件、Chrome初回/再起動48チェック24取得が合格。
旧保存版1〜4と新版5を分離する。G03と全計画は未完、親33=8/17/7/1を維持。
...
2026-09-12：軸接続の[幾何/K/M基盤](AXIS_CONNECTED_MESH.md)を42e4d38、[FEM/RF/native/CLI](AXIS_HPHI_RF.md)をd2ca604として限定受入。
標準1038件（1035合格/3skip）・独立16FEM/native・12幾何/72モード・凹形4FEM/native・主8unit/6CLIを確認した。
[Project/表示/独立Study接続](AXIS_HPHI_WORKSPACE.md)は別候補で検証中。親33課題の状態数は変更しない。
...
## 現在の親課題集計 — 2026-09-14、A01の候補比較受入後
...
| 状態 | 件数 | 親課題と範囲 |
|---|---:|---|
| .S/.I/.V受入済み | 10 | C01、C02（初期AF部分集合）、O01（ローカル保存）、R01（native規約）、N01/N02（直線P2）、G01/G02（軸接続単一輪郭・自動メッシュ）、A01（[固定版候補の役割・精度/DOF/時間比較とADR-018](A01_BACKEND_COMPARISON.md)）、D01（[元要件・一般写像・個別ID回復・全利用先の受入](D01_ACCEPTANCE.md)） |
| 調査・実装進行中、親課題全体は未受入 | 16 | S05（[原要件照合](S05_ACCEPTANCE.md)の専用API/CLI/GUI範囲を限定受入。C00.Vの対象版/必須集合は未確認）、S04（[原要件照合](S04_ACCEPTANCE.md)を完了。平面/軸接続/軸非接続P1の専用解/元場/失敗保存/CLI・解析/BVPを限定受入。C00.Vの対象版/材料モデルは未確認）、 S03（平面/軸接続の反跳FEM/保存と[軸非接続の反跳材料の保存・再構築・CLI](OFF_AXIS_RECOIL_NATIVE.md)を限定受入。仕事相殺検査修正も受入。C00.Vの対象版/材料モデル確認は未完）、 S02（[平面mu_r/Jz/Azの実FEM・元B/H/磁束](PLANAR_MAGNETOSTATIC_SOLVE.md)と[保存/CLI](PLANAR_MAGNETOSTATIC_NATIVE.md)、[軸接続の正則弱形式](AXIS_MAGNETOSTATIC_FORMS.md)を限定受入。[軸接続の実FEM・元6場/J/Wb](AXIS_MAGNETOSTATIC_SOLVE.md)も限定受入。[軸保存/CLI](AXIS_MAGNETOSTATIC_NATIVE.md)と[固定電流の境界距離/細分系列](MAGNETOSTATIC_BOUNDARY_STUDY.md)も限定受入。[軸非接続のpsi弱形式](OFF_AXIS_MAGNETOSTATIC_FORMS.md)も限定受入。[軸非接続の解/元6場/J/Wb](OFF_AXIS_MAGNETOSTATIC_SOLVE.md)も限定受入。[軸非接続の保存/CLI](OFF_AXIS_MAGNETOSTATIC_NATIVE.md)も限定受入。C00.Vは未完（[S02照合](S02_ACCEPTANCE.md)））、 S01（[軸対称誘電体/電荷/K/荷重](ELECTROSTATIC_FORMS.md)を限定受入。[固定電極/Poisson解・元E/D・電荷/容量](ELECTROSTATIC_SOLVE.md)も限定受入。[専用native/CLI](ELECTROSTATIC_NATIVE.md)も限定受入。[平面の誘電体/K/荷重基盤](PLANAR_ELECTROSTATIC_FORMS.md)も限定受入。[静的Project入力](STATIC_FIELD_PROJECT.md)、[実worker/CLI](STATIC_FIELD_JOBS.md)、[静的GUI](STATIC_FIELD_GUI.md)、[Study入力](STATIC_FIELD_STUDY_INPUT.md)は専用範囲を限定受入。[Study実worker/CLI](STATIC_FIELD_STUDY_JOBS.md)を限定受入。[Study GUI](STATIC_FIELD_STUDY_GUI.md)も専用範囲を限定受入。[O02原要件照合](O02_ACCEPTANCE.md)の専用範囲は完了。対象版確認は未完。[平面Poisson解・元E/D・J/m/C/m/F/m](PLANAR_ELECTROSTATIC_SOLVE.md)も限定受入。[平面専用native/CLI](PLANAR_ELECTROSTATIC_NATIVE.md)も限定受入。[固定電荷の外部境界/細分144例](ELECTROSTATIC_BOUNDARY_STUDY.md)を限定受入。[原要件と残件](S01_ACCEPTANCE.md)：C00の対象版照合・純Neumann等は未完）、 P04（[線形RF材料の領域/界面・K/M基盤](MATERIAL_HPHI_FORMS.md)と[専用Case/固有解・場/RF](MATERIAL_HPHI_RF.md)、[native/CLI](MATERIAL_HPHI_NATIVE.md)、[Project/GUI/独立Study](MATERIAL_HPHI_WORKSPACE.md)を限定受入。材料比較/追跡・損失モデル・旧版照合は未完）、P03（[閉同軸円筒](COAXIAL_RF.md)、[正半径の一般断面・複数穴](HPHI_MESH_RF.md)、[軸接続のPEC穴付き断面](AXIS_HPHI_RF.md)のHφ FEM・各空間の零モード処理・全壁RF・native/CLIを限定受入。[軸接続のProject/GUI/独立Study](AXIS_HPHI_WORKSPACE.md)も限定受入。[明示メッシュ列の細分差診断](HPHI_CONVERGENCE.md)、[同領域のE/H部分空間対応](HPHI_SAME_DOMAIN_TRACKING.md)、[所有保存/worker/CLI](HPHI_TRACKING_JOBS.md)、[追跡GUI](GUI_HPHI_TRACKING.md)、[所有履歴](HPHI_TRACKING_HISTORY.md)、[穴付き二次幾何](CURVED_MERIDIONAL_GEOMETRY.md)と[Hφ K/M](CURVED_HPHI_FORMS.md)、[明示曲線の固有解/場/RF API](CURVED_HPHI_RF.md)と[専用native/CLI](CURVED_HPHI_NATIVE.md)、[Project/表示/独立Study](CURVED_HPHI_WORKSPACE.md)も限定受入。曲線の比較/追跡・一般の形状写像・旧版照合等は未完）、 P02（[矩形P1/P2と専用入力・保存/CLI](PLANAR_RF.md)を限定受入。単純多角形版2/CLIと[専用Project/Job](PLANAR_JOBS.md)を限定受入、[平面表示/GUI](GUI_PLANAR.md)を限定受入、[独立Study](PLANAR_STUDY.md)、[同形状細分診断](PLANAR_CONVERGENCE.md)、[矩形追跡](PLANAR_TRACKING.md)、[多角形一様尺度追跡](PLANAR_POLYGON_TRACKING.md)を限定受入。[保存履歴](PLANAR_TRACKING_HISTORY.md)、[宣言相似写像](PLANAR_SIMILARITY_TRACKING.md)、[同じ多角形の独立再メッシュ](PLANAR_REMESH_TRACKING.md)、[相似変換＋独立内部メッシュの合成](PLANAR_SIMILARITY_REMESH_TRACKING.md)、[可逆アフィン＋独立内部メッシュの合成](PLANAR_AFFINE_REMESH_TRACKING.md)、[厳密アフィン＋境界/内部の独立メッシュ](PLANAR_EXACT_AFFINE_TRACKING.md)も限定受入。一般の丸めた境界・非線形変形・曲線断面等は未完）、P01（直線P1/P2と曲線P2のTE固有値・場・RF・専用native/CLI）、 D03（保存RF制約評価・曲線2変数探索の実FEM/予算/最終細分/保存再開API・CLI・JobManager・GUI、[固定局所履歴の要求版2](RF_OPTIMIZATION_HISTORY.md)と[多変数曲線寸法の版3](RF_OPTIMIZATION_GEOMETRY.md)を部分実装）、O02（外部mesh単体GUI読込/解除・通常Job/保存・直線P1/P2および曲線固定Study、TE通常Project/Job/GUIを部分実装、[静的Project入力](STATIC_FIELD_PROJECT.md)を限定受入。[静的worker/CLI](STATIC_FIELD_JOBS.md)を限定受入。[静的GUI](STATIC_FIELD_GUI.md)を限定受入。[静的Study入力](STATIC_FIELD_STUDY_INPUT.md)を限定受入。[Study実worker/CLI](STATIC_FIELD_STUDY_JOBS.md)を限定受入。[Study GUI](STATIC_FIELD_STUDY_GUI.md)を限定受入。[O02原要件照合](O02_ACCEPTANCE.md)の専用範囲は完了。対象版/必須集合C00.Vは未確認）、C00（対象版/必須集合未確定）、G03（曲線FEM等と[同一支持円の有限弧/同じ楕円・双曲線の自己接点](G03_FINITE_OFFSETS_CHECKPOINT.md)を限定受入。異なる支持曲線の一般重解/全弧端・退化候補構築・物理ピーク収束・全要件照合が残る）、D02（追跡付き1変数tuneと再開・細分判定API/CLI・JobManager・GUI、[多項式曲線寸法の非アフィン版5](CURVED_HARMONIC_TUNING.md)と[明示回復方針の外側版6](TUNING_IDENTITY_RECOVERY.md)を部分実装）、N03（通常RFの上下界評価API/保存/CLI/GUI、固定曲線幾何の追跡済み表面収束評価API/CLI/GUI、直線P1/P2の連続離散ピーク囲い込み・元多角形角診断・表面収束評価API/保存/CLI/GUI）、N04（曲線適応版4の五量停止/保存再開API/CLI/JobManager、曲線親子質量内積追跡、曲線残差指標、曲線局所細分の基盤APIとCase履歴/native保存/CLI、適合細分・係数移送・残差指標、追跡付きf/RQ/G適応停止・保存再開API/CLI/JobManager/GUI。版2全域確認で円筒P1のRF改善検査もPASS。版3のピーク比停止はAPI/CLI/JobManagerへ追加。一般形状/効率は未受入） |
| 親課題として未受入 | 6 | C03/C04、V01/V02、L01/L02。既存プローブ/GUI/掃引や個別統合はあるが、各親課題の完了ではない |
...
P02は[平面問題の契約と隔離候補](PLANAR_RF_PLAN.md)を先に確認し、P01の完了は必須としない。P1/P2の解析周波数/場/壁積分、TE零空間、正方形縮退、二尺度を主ツリーで確認し、専用PlanarCase/保存/CLIを接続した。標準813件と既存TM/TE回帰まで限定受入した。単純多角形版2・CLIは標準835件と独立物理まで限定受入。専用Project/Jobは標準849件と独立worker/取込/再起動まで限定受入。表示・GUIは標準856件・独立場/ブラウザー/保存まで限定受入。独立Studyは標準865件・実worker/GUI/独立則/保存まで限定受入。[細分診断](PLANAR_CONVERGENCE.md)、矩形/尺度/相似/同一領域再メッシュの追跡と保存履歴、[相似変換＋独立内部メッシュの合成](PLANAR_SIMILARITY_REMESH_TRACKING.md)も接続済み。各仕様の受入条件を参照。境界密度独立の一般形状変更・曲線断面等は残る。
