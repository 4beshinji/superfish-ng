...
2026-09-14：[分裂後の個別ID回復](MODE_IDENTITY_RECOVERY.md)を単独履歴API/CLI/GUIへ追加。
以前の確認済み個別場との一意対応と継承ID集合の整合を条件に、履歴版3で元比較を保持して回復根拠を記録する。
関連65unit・独立9保存解・Chrome17項目と旧履歴2項目が合格。Study/tuneへの回復方針の接続と一般写像全体の受入は残る。
以下の「個別枝回復は未実装」は直前監査時の状態であり、本追補が単独履歴の経路を更新する。

2026-09-14：[元要件と現行残件の照合](D01_CURRENT_AUDIT.md)で、56選択検査と6実FEMを確認した。
多対多のID集合継承は実装済み。分裂後の個別枝回復は未実装で、親D01は進行中を維持する。
以下の未接続/残件表記は各段階の履歴。後半の追補と上記監査を優先する。

2026-09-14追補：[曲線比較メッシュの版2入力](CURVED_PIECEWISE_REMESH_TRACKING.md)をpiecewise_remeshへ追加。
各側の元弦メッシュ/独立細分履歴から二次空間を再構築し、曲線境界全体を照合する。
FEM側の接続/要素数は独立にでき、可変の物理体積を保持する。既存の直線版1と保存/CLI/GUI経路は保持。
以下の直線専用という記述は初回実装時の履歴であり、現在の曲線受入条件は専用文書を参照する。
...
D02の[限定tune API/CLI](TUNING.md)へ保存場追跡を接続した。全個別IDの確認が取れない
試行は周波数を渡さず停止する。以下の古い段階別記録のtune未接続記述に優先する。
...
直線幾何では、新メッシュの座標を逆変換し、旧メッシュとの境界全辺被覆・タグ一致をsame_domain方式で
...
二次曲線幾何は下記条件で対応。任意非アフィン写像と連続枝の同定は未対応。
...
同じ `affine_remesh` controlsで、両側がnative曲線P2の保存結果の場合にも対応する。
同じ番号のnative primitiveと、その0〜1パラメータを対応付ける契約を用いる。
primitive数・順序・パラメータ対応が必要で、対応は推定しない。解析曲線宣言そのものは
異なってよいが、新側の二次近似境界を逆変換した後、共通パラメータ区間のBernstein係数を
旧側と照合する。係数差は区間全体の位置差を上から抑える。タグと全区間被覆も検査する。
直線/曲線の混在、PEC/axis以外、誤写像、再投影による二次境界変更は拒否する。
同じ曲線でも分割方法・パラメータ付けを変えたものを自動対応させる仕様ではない。

両側の曲線要素積分点を実物理座標で相手側へ写し、保存した実FEM場を評価する。
旧側の測度は `r dr dz`、新側は実測度を `a²c` で割った旧領域の測度として半分ずつ用い、
新側の場は `Hphi/a` を比較する。境界全体の照合結果と、実領域/基準領域の体積を保存する。
元のFEM行列・固有値・係数は変更せず、曲線場を直線メッシュへ置き換えない。
固定二次幾何を細分した独立接続は比較可能。標本次数を変えた確認は引き続き必要。
GUIの同じアフィン選択・係数入力・逆変換・保存/履歴操作からも利用できる。

`validate_curved_affine_remesh_tracking.py` は球形と合成楕円体を半径1.2倍・軸0.8倍し、
固定境界細分後の実固有モードを比較する。尺度1/2のf/RQ/G相似則、元球形の解析周波数、
楕円体体積と変換体積比、次数3/5、逆方向・保存再検証を独立に確認する。
せん断は既知多項式場の別テストで検証し、曲線せん断形状の固有モード受入とは区別する。
相似則や追跡PASSは、変形前後で周波数/RFが等しいことや物理RF収束を意味しない。
...
native保存解を渡す。GUIでは「比較用メッシュによる局所変形」を選び、2要素のJSON配列を入力する。
保存文書から復元でき、旧・新の交換は入力欄だけを更新する。保存済み履歴は維持する。
二次曲線幾何と対応の自動推定は未対応。
...
## 同じ二次曲線境界を持つ再メッシュ比較（2026-09-08）

`mapping="curved_same_domain"` は、両側がnative曲線P2保存解で、同じ曲線宣言・
パラメータ付けを持ち、表現している二次曲線境界が一致する場合に使う。
追加の対応入力は不要。sample_orderは2〜32、両メッシュ合計262144標本以下。
旧same_domainやpaired_meshの意味は変更しない。

各native曲線の境界パラメータ区間を共通区間へ分け、旧/新の二次Bézier制御点を比較する。
制御点差の最大ノルムは、Bernstein基底の非負性と和が1であることから区間全体の位置差上界になる。
端点が一致しても中点の異なる辺は拒否する。数値判定には座標スケール×512εの丸め幅を用い、
パラメータ被覆の丸め幅、共通区間数、最大係数距離と判定基準を保存診断へ残す。
この浮動小数点の一致検査を、解析曲線との厳密な等価性証明と呼ばない。

固定曲線の制限として細分したメッシュを受理する。解析曲線へ再投影して境界の二次近似が
変わった場合は、元の解析曲線宣言が同じでも拒否する。両側とも閉PEC/axis境界が必要。
境界を保った任意の曲線P2接続を扱えるが、現時点の実FEM受入例は固定幾何の一様4分割。

両メッシュの各曲線三角形で正の物理体積測度r detJ wを生成し、半分ずつ合わせて比較する。
自分側の場は元の要素で直接評価し、相手側は同じ物理座標で曲線逆写像により評価する。
領域外や未確認の逆写像を補完しない。標本次数を変えて対応を確認し、対応PASSをFEM/RF収束としない。
積分体積は同じ離散領域の体積であり、解析楕円体等との差とは別に記録する。

API `curved_same_domain_tracking.track_curved_same_domain_modes`、保存/履歴CLIとStudy controls JSONで指定可能。
GUIでは「同じ二次曲線境界の領域（異なるメッシュ）」を選択し、保存文書からも復元する。
比較診断に物理写像と境界係数照合の証拠を表示する。曲線境界を変形・再近似する一般写像、異なるnative曲線パラメータ付けの
自動対応は未対応。個別枝の連続同定もこの受入には含めない。

追加6検査は境界細分一致、同じ場の延長で重なり1、端点のみ一致する不正辺と再投影差の拒否、
実FEMの往復対応/保存再検証、物理/入力拒否を検査する。
`validate_curved_same_domain_tracking.py` は球形と合成楕円体で固定曲線細分、次数3/5、
球形解析周波数、離散体積一致/解析体積差、Maxwell相似則f/RQ/Gを検査する。

## 明示された親子細分の質量内積比較（2026-09-08）

nested_affineをAPI/保存比較/履歴CLIへ追加。[仕様と内積不変量](NESTED_AFFINE_TRACKING.md)。
同じCase/次数の明示した要素選択から全細分メッシュを再構築できる場合に限定する。
独立再メッシュ用のsame_domainや既存保存文書の判定は変更しない。GUIの方式選択は未接続。
