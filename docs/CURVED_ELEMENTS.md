...
2026-09-09追記: [曲線適応版4](CURVED_ADAPTIVE_REFINEMENT.md)で残差選択・追跡・高次積分比較・五量停止・保存再開をAPI/CLI/JobManagerへ接続。滑らかな閉PEC曲線を対象とし、GUI・一般精度/効率は残る。

2026-09-08追記: [曲線親子空間の質量内積追跡](NESTED_CURVED_TRACKING.md)をAPI/保存/CLIへ接続。局所/全域の履歴拡張、対称条件・鏡映の宣言偶奇を扱う。曲線適応停止と一般精度/効率は残る。
...
2026-09-08、製品基準 `e68002f`: 共有二次幾何・全体行列/solve・場/RFを通常Caseへ接続済み。
v3 curved_contourでmesh.geometry_order=2とsolver.element_order=2を指定する。
物理座標プローブ・保存/再読込・描画・固定幾何Study・鏡映・GUIにも対応する。
連続離散表面極値の囲い込みと元解析曲線の角診断をRF/保存/GUIへ接続した。
球形の独立参照、楕円/双曲線の幾何近似3段階と各固定FEM比較を部分受入。
追加系列のPASSは確定済みで、初回FAILを保持する。

G03全体は未受入。共通接線は円錐曲線全体の四次式・根分離・接点再構成まで実装。
有限弧の区間/枝/向き制限、切詰め・G1接続、候補選択とCase/CLI/GUIへの統合が残る。
仕様と証拠は [TANGENT_CONSTRUCTION.md](TANGENT_CONSTRUCTION.md)。
物理ピーク収束・旧曲線入力互換・全要件照合も残る。離散極値の計算保証を物理誤差保証としない。
以下の日付付き実施記録と初期実装順序は履歴であり、この到達点を現在の状態とする。
...
以下の製品経路は接続済み。現在の残件は冒頭と最新引継ぎを参照。
