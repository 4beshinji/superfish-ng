...
2026-09-14追記: [共通履歴を一度だけ再構築](NESTED_RECONSTRUCTION_REUSE.md)するよう整理した。
元メッシュから旧履歴と追加履歴を構築して両側を照合する。strict解析・幾何・係数・偶奇の検査を保持し、
3形状の転送/報告は完全一致。公開追跡全体で約1.73〜1.76倍を観測した。保存値を信頼するキャッシュは追加しない。
...
元メッシュからnative履歴を再構築して両側の幾何配列・軸/本質拘束を照合する。後の履歴は前の履歴を真に延長し、
