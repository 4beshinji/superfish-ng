...
2026-09-14：曲線primitiveの数値項目を多項式で連動させる[版5の非アフィン調整](CURVED_HARMONIC_TUNING.md)を追加。
固定履歴・実試行からの比較写像・採用親の最終細分、CLI/GUIの保存再開を接続する。以下の版1〜4の契約は維持する。
...
一次式の連動座標は下記のschema_version=2、多項式は
[schema_version=3](POLYNOMIAL_TUNING.md)を使う。
...
一般写像・追跡の枝回復、曲線・組立・非多項式連動、細分失敗後の自動再探索、
...
[GUI_ACCEPTANCE.md](GUI_ACCEPTANCE.md)のD02記録を参照。一般曲線/非多項式連動の調整と
...
版2は一般曲線、組立、非線形な結合関数、複数独立変数の最適化ではない。
後続版3で多項式の非線形連動を追加した。連動指定を変えた
