...
2026-09-09: [版5非球形・両尺度検証](CURVED_RF_NONSPHERE_COMPARISON.md)の再現スクリプトを追加。既存の自作FEM版4の受入済みreportをhashと一致検査付きで再利用し、参照場を今回再計算したとは扱わない。初回12回は両尺度LEVEL_LIMITを保持。48回上限の実計算を継続中。新規外部資料・依存・Wine実行はない。
