...
目的関数一つと複数のRF制約を評価する。探索器・最適解の判定はまだない。
...
書込は新規ファイルだけ。CLI/JobManager/GUI、変数探索・全試行記録・予算/停止/再開、
初期/最終の比較の自動実行は未接続。D03親課題は進行中で未受入。
...
D03の次工程では、同じ曲線Projectを元にした半径/軸方向の複数設計変数と
...
全FEM呼出しを含む予算、途中保存と再検証を仕様化してから実装する。
