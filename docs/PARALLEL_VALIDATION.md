...
通常開発の実行範囲は[検証手順と棚卸し](TESTING.md)に従う。以下は全件実行が必要な時の方法と過去の実測であり、各変更での全件実行を要求しない。対象ファイルだけなら`python scripts/run_tests.py --pattern 'test_axis_bh*.py' --out out/tests-axis-bh-new`、seed数値検証だけならvalidateへ`--skip-tests`を付ける。
