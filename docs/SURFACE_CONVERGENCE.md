...
2026-09-14 JST：[固定履歴の後への全域細分](RF_OPTIMIZATION_HISTORY.md)を版2で評価する。
同じ局所履歴の接頭部分・元Case/メッシュを保ち、末尾uniform回数だけを増やす系列を受理する。
保存した接頭部分を元nativeから再構築し、GUIでも段数の意味を表示する。旧版1は維持する。

2026-09-14 JST：[G03現行照合](G03_CURRENT_AUDIT.md)で、合成楕円の幾何3水準それぞれの固定FEM3水準を専用検証した。幾何間の表面五量差は別報告とし、本APIの固定二次境界契約とgeometry_approximation_assessed=Falseは保持する。
...
- 元Caseと元メッシュを保持し、`curved_refinement_levels` のみを厳密に増やす。
  順序付き履歴を使う版2では、同じ固定接頭部分の後のuniform回数を厳密に増やす。
