...
後に初期同一性の検証式が同一区間の幅を差へ数える誤りを発見した。
対応端点差へ修正したが、従来方針のLEVEL_LIMITという未達自体は変わらない。
尺度1/2の時間は124.582/124.566秒。全イベント構造一致、Maxwell五量差の最大5.089e-13。
未達を成功へ変更せず、初回validationと実出力を保持した。

## 48回上限の結果

許容差・要素上限を維持し、実solve回数の予算だけを増やしたが、途中で中断した。
[明示した表面細分方針](CURVED_RF_SURFACE_POLICY.md)が元の12回予算で両尺度を完走したため、
任意の従来方針48回拡大をSIGINTで終了130。直前観測4,745秒、検証済みcheckpoint27、
solve完了ログ28イベント。尺度2は未実行で、従来方針の完走時間とは扱わない。
interruption.jsonと全native出力を保持した。主ツリーのソース固定を解除し、検証済み変更を統合した。
