...
以下の範囲で限定受入。矩形の宣言写像（追跡版1）と明示多角形の原点一様尺度（追跡版2）を、所有する保存場の連続性で結ぶ。各段階の数値的対応を全再生し、履歴のPASSを連続した物理経路の同一性や誤差上界とはしない。
...
全コピー・全再検証の費用は段階数と場データ量に増加する。任意多角形変形・回転・連続経路の追跡証明・真の物理誤差上界は対象外。親P02と開発計画全体は未完了。
