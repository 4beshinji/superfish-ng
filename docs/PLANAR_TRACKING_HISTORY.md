...
以下の範囲で限定受入。矩形の宣言写像（追跡版1）、明示多角形の原点一様尺度（追跡版2）、[宣言相似写像（追跡版3）](PLANAR_SIMILARITY_TRACKING.md)、[同一領域再メッシュ（追跡版4）](PLANAR_REMESH_TRACKING.md)を、所有する保存場の連続性で結ぶ。各段階の数値的対応を全再生し、履歴のPASSを連続した物理経路の同一性や誤差上界とはしない。
