...
2026-09-14 JST 追補：[両非零の全元候補射影](GENERAL_CONIC_OFFSET_PROJECTION.md)を一般交点処理の基盤として追加した。
必要条件の全元候補と相手側の認証済み交点は区別する。相手実元点・距離符号・有限弧の回復は次段階。
現在の診断11・構築1〜9と、以下の既受入規則は保持する。

2026-09-14 JST 追補：[主軸表現が異なる同一円錐曲線](REPARAMETERIZED_CONIC_OFFSETS.md)の等距離オフセットを診断11へ追加した。
楕円の軸交換・半回転、双曲線の同じ物理枝への表現変更を厳密に照合し、有限重なり/共有端点/反射元点対を分類する。
旧保存診断1〜10と構築1〜9の規則を保持する。異なる支持・距離や異なる物理枝の一般交差、新候補構築は残件。
以下は各段階の履歴で、現行状態はこの追補とリンク先を優先する。
