2026-09-18 JST：H09の直線一般写像を追跡・調整へ接続した。`hphi_geometry_mapping.mapped_pullback_factors`が標本ごとの`(r_current/r_previous*det(A))**-1/2`を返し、`hphi_field_overlap.hphi_mapped_field_grams`が宣言写像の厳密overlayで前E/Hを引き戻す。前後自己グラムは元FEM`_source_grams`へ再現し（最大1.9e-13）、一様尺度はH02経路と正規化overlap一致。`track_hphi_mapped_modes`を公開入口に追加し版1`same_vacuum`は不変。調整は`coaxial_dimensions`（m単位の内外半径/長さ）と`general_piecewise_affine`（radial/axial無次元倍率）を要求・試行生成・`_assess_trial`比較・checkpoint scope・replay/CLI/worker/GUI transportへ接続し、GUI表示単位をmapping種別へ合わせた。比較23件66.326秒、`test_hphi_tuning` 10件119.685秒、worker 4件62.163秒、GUI調整2件54.775秒、追跡履歴4件・GUI追跡2件PASS、終了0。合成同軸TEMは`f=c/(2L)`へTUNED、穴付き軸接続の単軸変形はID確認後のみ周波数評価、保存checkpoint replayが要求・試行を完全再現、正逆写像・番号置換・同物理形状の別分割を確認。詳細は[HPHI_TUNING.md](HPHI_TUNING.md)。次はH10の個別ID回復。H01〜H09完了、90カード中10完了。

2026-09-18 JST：H08の同軸寸法・直線一般写像契約を`hphi_geometry_mapping.py`へ追加した。同軸の内外半径/長さの非相似写像と、軸/穴を保持する向き保存アフィン写像を有理行列式で拒否検査し、`map_mesh`/`hphi_geometry_overlay`は境界/穴の完全被覆を既存の厳密同領域overlayで確認する。`test_hphi_geometry_mapping` 7件と既存候補の計34件PASS、14.091秒、終了0。同軸寸法の解析面積/体積と独立積分、一様尺度のH02一致、逆写像往復、穴消失/未被覆/反転/軸移動の拒否を確認した。詳細は[HPHI_TUNING.md](HPHI_TUNING.md)。次はH09の追跡・調整接続。H01〜H08完了、90カード中9完了。
