...
次の限定課題はO02の[静的Study入力と尺度変換](STATIC_FIELD_STUDY_INPUT_PLAN.md)。11形式に対する厳密な幾何/励起尺度変換と独立な比例/尺度不変量を先に検証し、[Study全体](STATIC_FIELD_STUDY_PLAN.md)の実FEM各条件保存とAPI/CLI/worker/GUIへ続ける。
