...
確認日: 2026-09-08。弧端・退化診断追加後（直前製品基準 `f6c7bec`）。コードと最新の個別受入記録を照合した。
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 397件中395合格・NGSolve参照環境専用2件skip。弧端・退化診断追加後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | 弧端・退化診断追加後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-g03-offset-degeneracy-20260908、[引継ぎ](CODEX_HANDOFF.md) |
