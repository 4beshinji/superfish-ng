...
確認日: 2026-09-08。N04適応計算JobManager接続後（直前製品基準 `d6064fb`）。コードと最新の個別受入記録を照合した。
...
| メッシュ・FEM | タグ付きJSON、品質条件付き自動生成、P1/P2、二次曲線写像、固定幾何細分、選択直線要素の適合細分と係数移送 | 品質未達は拒否。二次境界は元の解析曲線の近似。残差指標/対象選択APIあり。f/RQ/G停止・保存再開API/CLI/JobManagerあり。物理誤差上界・表面量停止・曲線局所細分は未実装 | [GENERAL_MESH.md](GENERAL_MESH.md)、[HIGH_ORDER_FIELDS.md](HIGH_ORDER_FIELDS.md)、[CURVED_ELEMENTS.md](CURVED_ELEMENTS.md)、[MARKED_REFINEMENT.md](MARKED_REFINEMENT.md)、[RESIDUAL_INDICATOR.md](RESIDUAL_INDICATOR.md)、[ADAPTIVE_REFINEMENT.md](ADAPTIVE_REFINEMENT.md) |
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 596件中594合格・NGSolve参照環境専用2件skip。N04適応計算JobManager接続後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | N04適応計算JobManager接続後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-n04-adaptive-jobs-20260908、[引継ぎ](CODEX_HANDOFF.md) |
| N04適応JobManager | 版1/版2×尺度1/2の部分実行・再開・管理器再生成、解析P2円筒f/RQ/Gと相似則PASS | out/n04-adaptive-jobs-initial-20260908、[仕様](ADAPTIVE_REFINEMENT_JOBS.md) |
