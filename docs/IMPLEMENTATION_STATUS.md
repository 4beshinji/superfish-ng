...
現行の受入範囲・担当カード・証拠の現存/欠落は[B01対応台帳](development-plan/B01-ledger.md)を参照する。以下の日付付き記録は履歴である。

2026-09-16 JST：[Hφ調整所有保存・CLI再開](HPHI_TUNING.md)をH05として受入。
要求と全試行Project/nativeを新規出力へ所有保存し、solverを呼ばない再生、PAUSED履歴の新規出力へのコピー再開、
要求/係数/native/順序/親の改変拒否を接続した。専用5件、H05指定の既存回帰/H04依存15件、専用validator 9実FEMがPASS。
H06 worker、H07 GUI、一般写像・曲線・材料/D03は残る。親33=11/15/6/1、全計画goal ACTIVE。
...
2026-09-14：[分裂後個別ID回復](MODE_IDENTITY_RECOVERY.md)を単独保存履歴API/CLI/GUIへ追加。
以前の個別場と既存ID集合を照合し、履歴版3の回復イベントを保存・再検証・継続できる。旧版1/2は維持する。
関連65unit・9保存解の解析/尺度則・Chrome17項目と旧履歴2項目が合格。Study/逐次・適応Study/tuneへの回復接続は残る。
親D01/D02/D03全体は未受入、親33=9/17/6/1、goal ACTIVE。以下の未実装表記は監査時の記録。

2026-09-14：[D01現状監査](D01_CURRENT_AUDIT.md)で56選択検査と6実FEMがPASS。多対多のID集合継承は実装済み。
分裂後の個別枝回復は未実装で、D01/D02/D03親の受入は保留。親33=9/17/6/1を維持する。

2026-09-14：[単位付き多変数曲線寸法のRF探索版3](RF_OPTIMIZATION_GEOMETRY.md)をAPI/CLI/JobManager/GUIへ接続。
既存の実FEMと3水準評価を用い、形状法則・実試行からの比較写像・各変数の表示を追加する。版1/2の2倍率は維持する。
関連29unitの分割証拠、独立36FEM、Chrome新版24/旧版13/追加入力11項目が合格。初期粗系列の不採用と最終の制約充足を区別した。
D03全体と全計画は未完。親33=9/17/6/1を保持する。

2026-09-14：[非アフィン曲線tune版5](CURVED_HARMONIC_TUNING.md)をAPI/CLI/JobManager/GUIへ接続。
既存の形状法則・調和変位・固定履歴を使い、比較分割と最終FEM分割を分けて実場を追跡する。
一般関数・境界分割変更・他物理の調整とD02全体は未受入。親33=9/17/6/1を維持する。

2026-09-14 JST：D03/P2-02の[固定局所履歴付きRF探索](RF_OPTIMIZATION_HISTORY.md)を追加。
要求版2で元メッシュ/固定分割を保ち、各形状の履歴末尾へ追加全域細分してRF設計条件を評価する。
表面評価版2は共通接頭部分と追加段数を再構築。版1の保存・厳密入力・五量許容差は保持する。
関連29unitと専用24実FEMの解析五量/Green/尺度則・保存再生はPASS。GUIの最終証拠は専用文書を参照。
親33=9受入/17進行/6他未受入/1範囲外は維持し、D03全体と全計画goalは未完。

2026-09-14 JST：[A01/P0-04の候補比較](A01_BACKEND_COMPARISON.md)を受入。
固定cavsim2dの明示Profileと自作NGを、閉PECの2形状/両側3水準/3回の36実FEMで比較した。
円筒解析五量・全幾何/エネルギー・内部E/H・双方収束・実DOF/時間と全出典SHAを保存。11unitは分割合格。
公開端条件・DOF表示・任意依存の問題も記録し、ADR-018で製品組込みを見送った。
親33=9受入/17進行/6他未受入/1範囲外。FEM本体不変、全suite/seed/Hosted CIは今回未実行。全計画goalはACTIVE。
以下の日付付き「P0-04未着手」は過去の記録であり、この受入結果を優先する。

2026-09-14 JST：[両非零オフセットの全元候補射影](GENERAL_CONIC_OFFSET_PROJECTION.md)を専用範囲で受入。
円錐曲線/円の行列式から全元候補を作り、整数係数の根分離と元符号/有限弧/カスプを検査する。
関連35テスト13.067秒、独立336点/6条件32候補128.253841秒がPASS。最大正規化行列式差1.148e-133以下。
旧有理方式は最初の次数56例が171.83秒で未完のため意図的に停止。同じ例の最終整数版は0.330499秒/8根/91箱。
一般回転の両チャート/元所属と独立参照は113.287427秒を要した。全入力の高速性や係数演算時間上限は主張しない。
初回の予算不足テストは有理根で完了したため無理数の根を持つ例へ修正。製品はその修正で不変。
相手元点/符号/有限相手弧と新構築は未接続。公開診断11/構築/GUIは不変で、新ブラウザー/全suite/seed/Hosted CIは未実行。
許容差/benchmarks不変、親33=8/17/7/1・全計画は継続中。以下は各段階の履歴である。

2026-09-14 JST：[主軸表現が異なる同一円錐曲線のオフセット](REPARAMETERIZED_CONIC_OFFSETS.md)を限定受入。
新診断11、旧保存診断1〜10/構築1〜9を保持。関連103テスト57.033秒、独立1,984条件8.755431秒がPASS。
元座標系の435反射元点対と608無限対条件を確認。実Chrome初回46/実再起動44チェック、各17取得が合格・バイト一致。
初回の回転.3+πの係数不一致というテスト前提を修正し、一致する.3と不一致の.7を両方確認した。製品はこの修正で不変。
製品301ファイルは検証を通して不変、全suite/seed/Hosted CIは未実行、許容差/benchmarks不変。
異なる支持・距離と異なる物理枝の一般交差、新候補構築等の残件を保持し、親33=8/17/7/1・全計画は継続中。
以下は各段階の履歴である。

2026-09-14 JST：[元の非円円錐曲線と法線オフセットの全交点](CONIC_IMPLICIT_OFFSET.md)を限定受入。
一方の距離0の組合せを新診断10へ追加し、旧保存診断1〜9/構築1〜9を保持した。
関連97テスト56.631秒、独立80条件200元点対96.727825秒、既存GUI96チェック38取得が合格。
初回の偽解存在のテスト期待を適切な外側targetへ修正。製品はこの修正で不変。
製品source300ファイルは独立validator/両ブラウザーを通して不変。全suite/seed/Hosted CIは実行せず、許容差/benchmarks不変。
両非零オフセットなどの残件を保持し、親33=8/17/7/1・全計画は継続中。以下は各段階の履歴である。

2026-09-14 JST：[円同士の全元点対からのフィレット構築](CIRCULAR_FILLET.md)を限定受入。
構築版9、旧構築1〜8/診断1〜9を保持。関連62テストは61合格/旧期待1失敗の後、修正した1件が合格。
独立336条件264元点対と実FEMの一括validator25.241157秒、Chrome186チェック86取得が合格。
テスト側の二進回転ノルムの仮定/旧診断版期待と編集時のインデント誤りの修正を記録。製品コードはこれらで不変。
source299ファイルは独立validator/初回/再起動で不変。全suite/seed/Hosted CIは実行せず、許容差/benchmarks不変。
非円弧同士の一般交点などは残る。親33=8/17/7/1・全計画は継続中。以下は各段階の履歴である。

2026-09-14 JST：[円・非円円錐曲線の全元点対からのフィレット構築](CIRCLE_CONIC_FILLET.md)を限定受入。
構築版8を追加し、旧構築1〜7/診断1〜9を保持。関連63テスト（62+追加1）、
独立44条件100元点対と実FEM尺度則の一括validator61.535295秒、Chrome154チェック64取得が合格。
CLIテスト入力と負支持半径の独立参照の修正を記録した。これらの修正で製品コードを変更していない。
製品source298ファイルはvalidator/初回/再起動で不変。全suite/seed/Hosted CIは実行せず、許容差/benchmarks不変。
円同士の全根からの構築・非円弧同士の一般交点などは残る。親33=8/17/7/1・全計画は継続中。
以下は過去の各段階の履歴である。

2026-09-14 JST：[円と非円円錐曲線オフセットの全交点](CIRCLE_CONIC_CROSSINGS.md)を限定受入。
新規診断9、旧診断1〜8/構築1〜7を保持。関連87件の最終合格、独立53条件、Chrome110チェック44取得。
離隔した潰れ円のメタデータ修正、旧公開版/対応範囲テストの修正と限定再検査を記録した。
FEM不変。全suite/seed/Hosted CI再実行ではない。親33=8/17/7/1・全計画は継続中。

2026-09-14 JST：[全根列挙からの直線・円錐曲線フィレット構築](ALGEBRAIC_CONIC_FILLET.md)を限定受入。
構築版7、旧構築1〜6/診断1〜8の規則を保持。関連71テスト（70+追加1）、非円36/円24独立条件、
新構築CaseのFEM尺度則とChrome138チェック54取得が合格。参照側の途中失敗と限定再実行も記録した。
全suite/seed/Hosted CI再実行ではない。親33=8/17/7/1・全計画は継続中。
...
| 専用Hφの操作 | 真空直線/曲線Case/native、Project/worker/取込・取消・再起動、元場表示、全E/H/BのSI CSV、独立Study。元E/H比較・細分差診断・部分空間追跡、所有履歴/過去anchor回復、調整CLI/worker。曲線は完全二次領域と元nativeを使用 | 曲線追跡/調整GUIはH13-e。調整回復の所有操作はH13-dで受入済み。有限比較空間の診断は連続誤差上界・連続経路ID・表面ピーク精度を保証しない。材料は別の専用範囲 | [曲線追跡](CURVED_HPHI_TRACKING.md)、[所有曲線履歴](CURVED_HPHI_HISTORY.md)、[調整所有操作監査](CURVED_HPHI_TUNING_RECOVERY_SAVED.md)、[元場GUI](CURVED_HPHI_WORKSPACE.md) |
