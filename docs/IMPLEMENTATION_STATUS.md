...
確認日: 2026-09-08。N03表面収束評価のGUI接続後（直前製品基準 `21fb6a0`）。コードと最新の個別受入記録を照合した。
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 567件中565合格・NGSolve参照環境専用2件skip。N03表面収束評価GUI接続後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | N03表面収束評価GUI接続後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-n03-surface-gui-20260908、[引継ぎ](CODEX_HANDOFF.md) |
