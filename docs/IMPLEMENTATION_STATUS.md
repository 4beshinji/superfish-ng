...
確認日: 2026-09-08。D01標本部分空間追跡の追加後（直前製品基準 `66a6518`）。コードと最新の個別受入記録を照合した。
...
| 条件群・鏡映 | 掃引、同一形状細分比較、条件付きバンド同定、曲線の幾何/FEM別Study・鏡映 | D01は重み付き標本部分空間・円筒写像による追跡を追加。一般形状/合流・分裂/連続履歴、tune、制約付き最適化は未実装 | studies.py、symmetry.py、curved_reflection.py |
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 421件中419合格・NGSolve参照環境専用2件skip。D01標本部分空間追跡の追加後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | D01標本部分空間追跡の追加後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-d01-mode-tracking-20260908、[引継ぎ](CODEX_HANDOFF.md) |
