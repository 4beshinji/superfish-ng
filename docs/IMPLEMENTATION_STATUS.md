...
確認日: 2026-09-08。N03直線表面評価GUI接続後（直前製品基準 `b648932`）。コードと最新の個別受入記録を照合した。
...
| 表面収束評価 | 固定曲線P2の3水準・追跡ID・ピーク上下界によるf/RQ/G/ピーク比判定API/CLI/GUI、元多角形角診断・追跡済み直線P1/P2の表面評価API/保存/CLI/GUI | 幾何近似誤差/一般形状/通常RF画面統合/適応停止統合は残件。物理誤差上界ではない | [SURFACE_CONVERGENCE.md](SURFACE_CONVERGENCE.md) |
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 613件中611合格・NGSolve参照環境専用2件skip。N03直線表面評価GUI接続後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | N03直線表面評価GUI接続後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-n03-affine-surface-gui-20260908、[引継ぎ](CODEX_HANDOFF.md) |
