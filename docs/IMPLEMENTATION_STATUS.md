...
確認日: 2026-09-08。D02連動座標の周波数調整追加後（直前製品基準 `c5233d9`）。コードと最新の個別受入記録を照合した。
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 558件中556合格・NGSolve参照環境専用2件skip。D02連動座標の周波数調整追加後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | D02連動座標の周波数調整追加後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-d02-coupled-tuning-20260908、[引継ぎ](CODEX_HANDOFF.md) |
