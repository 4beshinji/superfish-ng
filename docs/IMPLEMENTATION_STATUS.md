...
確認日: 2026-09-08。N03固定幾何の表面収束評価追加後（直前製品基準 `904c72b`）。コードと最新の個別受入記録を照合した。
...
8件が記載範囲で.S/.I/.V受入済み。G03・D01・D02・N03は部分実装・部分検証、C00は調査継続中。
...
| 表面収束評価 | 固定曲線P2の3水準・追跡ID・ピーク上下界によるf/RQ/G/ピーク比判定API/CLI | 幾何近似誤差/一般形状/直線要素/GUIは残件。物理誤差上界ではない | [SURFACE_CONVERGENCE.md](SURFACE_CONVERGENCE.md) |
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 567件中565合格・NGSolve参照環境専用2件skip。N03固定幾何の表面収束評価追加後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | N03固定幾何の表面収束評価追加後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-n03-surface-convergence-20260908、[引継ぎ](CODEX_HANDOFF.md) |
