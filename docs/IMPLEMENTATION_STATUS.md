...
確認日: 2026-09-08。D02追跡付き1変数tune初期実装後（直前製品基準 `64a086f`）。コードと最新の個別受入記録を照合した。
...
| 周波数調整 | 明示円筒/profile写像の1変数二分探索・停止・再開・最終細メッシュ判定API/CLI | 全個別ID確認が前提。二水準差は誤差上界でない。一般変数/GUI/JobManager/最適化は未実装 | [TUNING.md](TUNING.md) |
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 542件中540合格・NGSolve参照環境専用2件skip。D02追跡付き1変数tune初期実装後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | D02追跡付き1変数tune初期実装後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-d02-tuning-final-20260908、[引継ぎ](CODEX_HANDOFF.md) |
...
| 周波数調整 | 追加11検査PASS。円筒TM011の順位3→2、二分探索16回＋細分1回、独立解析周波数/相似則・CLI停止再開/再検証PASS。二つの細分判定のFAIL保持 | [TUNING.md](TUNING.md)、out/d02-tuning-accepted-20260908 |
