...
確認日: 2026-09-08。D02周波数調整のGUI接続後（直前製品基準 `0b40a5e`）。コードと最新の個別受入記録を照合した。
...
8件が記載範囲で.S/.I/.V受入済み。G03・D01・D02は部分実装・部分検証、C00は調査継続中。
...
| 種別 | 状態・範囲 | 記録 |
|---|---|---|
| 標準unittest | 552件中550合格・NGSolve参照環境専用2件skip。D02周波数調整のGUI接続後に標準validate内で再実行 | `.venv/bin/python -m unittest discover -s tests -v`、OPENBLAS_NUM_THREADS=1 |
| 標準数値回帰 | D02周波数調整のGUI接続後PASS。seed周波数差ゼロ、RF/エネルギー差最大8.882e-16。FEM変更なし | out/validation-d02-tuning-gui-final-20260908、[引継ぎ](CODEX_HANDOFF.md) |
...
| 周波数調整 | tune核11検査・JobManager接続7検査・GUI接続3検査PASS。円筒TM011の順位3→2、二分探索16回＋細分1回、独立解析周波数/相似則・CLI停止再開/再検証PASS。実workerの停止/再開・管理器再起動・改変拒否と独立計算もPASS。GUI13操作と既存46操作PASS。二つの細分判定のFAIL保持 | [TUNING.md](TUNING.md)、out/d02-tuning-gui-final-20260908 |
