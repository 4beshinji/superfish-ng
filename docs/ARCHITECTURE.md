...
## ADR-018: A01の候補比較を隔離した参照環境で行う

2026-09-14、A01/P0-04。cavsim2d 0.1.0のcommit
`48741ff46ca44463281a5ab5945328615881b502`を比較対象に固定する。
目的は同じ閉PEC空洞でのf/RF誤差対自由度/時間と、入出力・依存・規約の適合性の測定。
NGSolve/Gmsh等はこの測定用の`/tmp`仮想環境だけへ導入し、製品依存には追加しない。
既存P0-01は独立Hphi弱形式の比較であり、cavsim2d自身の評価を代用していない。
公式MITライセンスと依存宣言を確認し、固定版の現代的なPythonコードだけを選択取得する。
同梱ABCI/TopDrawer等の実行形式、旧SUPERFISH/POISSONソース・バイナリは取得しない。
固有モード経路だけを実行し、参照が不成功なら実際の失敗と未測定項目を残す。
測定結果と原要件照合は[A01_BACKEND_COMPARISON.md](A01_BACKEND_COMPARISON.md)。36実FEMで両側収束、
円筒解析五量、円錐台のRF/内部場が合格。候補の表示DOFは複合空間全体を含まないため実保存係数数を比較した。
公開Pillboxのee指定でPMCが残る問題と、core importで任意IPythonを要求する問題も記録した。
今回の円筒では自作NGの解析誤差が小さく、一般速度優位は得られていない。
**製品組込みは見送り、NGを維持する。候補は明示Profileで検証した独立参照用途に限定する。**
参照実行をNGの製品solveへ組み込まない。A01/P0-04の比較・判断は受入、他課題の受入を意味しない。
