...
H10-aの数値回復核を受入済み。親H10の所有履歴event・後続継承（H10-b）と調整の検索/最終細分（H10-c）は未接続で、親課題は未完了。
...
この核単独ではindexと実際の過去の所有snapshotとの結び付けを証明しない。それはH10-bの履歴所有側の必須責務である。現在の専用履歴版1は暗黙の集合→個別ID変更を引き続き拒否する。既存の調整・CLI・worker・GUIはこの核をまだ呼び出さない。真空直線の軸接続u/正半径qに限定し、曲線・材料の回復を受入済みとは呼ばない。
