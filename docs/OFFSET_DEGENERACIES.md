...
2026-09-14 JST：[元の非円円錐曲線と法線オフセットの全交点](CONIC_IMPLICIT_OFFSET.md)を追加した。
一方の距離0の一般非円対を新診断版10で扱い、旧保存構築診断1〜9は元規則で再検証する。
構築文書は引き続き版1〜9。両非零の一般交点や新フィレット構築への代用にはしない。
以下の日付付き記録は各段階の履歴である。

2026-09-14 JST：[円同士の全元点対構築9](CIRCULAR_FILLET.md)を接続した。
保存構築5〜9の再現と診断を扱う。新規診断は引き続き版9で、構築とは別の版番号空間である。
以下の日付付き記録は各段階の履歴である。

2026-09-14 JST：[円と非円弧の全交点診断9](CIRCLE_CONIC_CROSSINGS.md)と
[全元点対からの構築8](CIRCLE_CONIC_FILLET.md)を接続した。
構築版5/6/7/8の保存再現と診断表示を扱う。構築と診断は別の版番号空間で、旧版は元規則を保持する。
以下の日付付き記録は各段階の履歴である。
...
`construction_diagnostics` は版5/6/7/8/9の構築を再実行・照合した後、保存された交点探索証明の
...
新規診断文書はschema_version=10 / document_type=construction_offset_diagnosisで、
...
[円と非円楕円/双曲線の全交点](CIRCLE_CONIC_CROSSINGS.md)は版9の対象。旧版8も当時の規則を保持する。
[非円曲線対で一方の距離が0の全交点](CONIC_IMPLICIT_OFFSET.md)は版10の対象。旧版9も当時の規則を保持する。
その他の未確認診断も、構築の合格を取り消す根拠にはしない。

GUIは版5/6/7/8/9構築の候補表示・選択後検査・再読込時に同じ診断を別欄へ表示する。
...
診断対象は2026-09-14の版10まで拡張した。構築は別の版番号空間で、
[版7](ALGEBRAIC_CONIC_FILLET.md)は直線/円錐曲線、[版8](CIRCLE_CONIC_FILLET.md)は円/非円弧の全元点対から新候補を作る。
[構築版9](CIRCULAR_FILLET.md)では円同士の全元点対と孤立した共有端点を候補へ接続する。
非円弧同士の一般交点/重解等は残る。
