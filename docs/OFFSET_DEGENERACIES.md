...
2026-09-14 JST：[主軸表現が異なる同一円錐曲線](REPARAMETERIZED_CONIC_OFFSETS.md)の等距離オフセットを診断11へ追加。
零・非零距離の有限重なり/共有端点/反射元点対を扱い、旧保存診断1〜10の規則と構築1〜9を保持する。
異なる支持・距離や異なる物理枝の一般交差、新フィレット構築は残件。以下は各段階の履歴である。
...
新規診断文書はschema_version=11 / document_type=construction_offset_diagnosisで、
...
[異なる主軸表現の同一非円曲線オフセット](REPARAMETERIZED_CONIC_OFFSETS.md)は版11の対象。旧版10も当時の規則を保持する。
...
診断対象は2026-09-14の版11まで拡張した。構築は別の版番号空間で、
