...
2026-09-08、版5有限弧フィレット統合時。G03は**部分受入**。
...
| 3. 元曲線/弦許容差のCase・保存・GUI | test_curved_case.py、test_curved_saved.pyの保存往復/改変拒否、GUI_ACCEPTANCE.mdの曲線入力/保存操作 | 通常経路を限定受入。版5で有限弧同士のフィレットも接続。線分と弧のフィレット操作は未接続 |
...
| 円錐曲線弧を含むフィレット | normal_offsets.pyで法線方向の中心軌跡/導関数を有理数区間で囲み、正則/反転/潰れ/未確認を全有限区間に保持。offset_intersections.pyの孤立交点認証からconic_fillet.pyで接点/切詰め/位置上界付き構築へ接続。版5保存/Case/CLI/GUIとtest_conic_fillet.py、Chrome8操作PASS | 有限弧同士を部分受入。線分と弧、弧端/重解/無限解の追加分類と各形状の物理精度収束は未完 |
...
版5GUIはout/gui-conic-fillet-browser-20260908/report.jsonのChrome8操作で新規確認した。
既存版1〜4の記録も保持する。標準回帰の最新件数は
[実装状況](IMPLEMENTATION_STATUS.md)と[引継ぎ](CODEX_HANDOFF.md)に保持する。

版5で根箱からの有限接点と有向フィレットを製品操作へ接続。次は線分と弧のフィレット、弧端/退化の追加分類。
