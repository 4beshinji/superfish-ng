...
2026-09-14追補：[NT=2/3の版別参照と次の受入条件](C00_CONIC_INPUT_RESEARCH.md)を整理した。
1987年公式マニュアルの入力表と対象7.17の仕様を分離し、未確認の省略値・枝・端点処理を推定しない。
今回parser/API/CLI/保存形式は無変更。以下のNT=1/4/5限定仕様とNT=2/3の拒否を保持する。
