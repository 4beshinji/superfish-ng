...
[軸接続P1の非線形B-H弱形式](AXIS_BH_FORMS.md)。軸接続P1のa=Aphi/r[T]から元B=(−r a_z,2a+r a_r)、H=h(|B|)B/|B|、実接線を評価する。dV=2pi*r dr dzでg[A m²]、K[m⁴/H]、電流荷重[A m²]とU/U*[J]を積分する。a·g=U+U*、sum(g)=∫2Hz dVを検査し、全軸DOFを保持する。一定aは一様軸Bでありgaugeではない。q/q+4の積分差は残差/場精度と別の診断として保持し、元P1場の頂点範囲も検査する。境界付き解は後続工程。
