[曲線Hφのnative細分](CURVED_HPHI_REFINEMENT.md)は元P2写像の参照4分割であり、解析曲線へ再投影しない。旧比較版1の厳密係数一致を維持し、明示版2だけbinary64係数差を有理数で測定してBernstein上限を外向き表示する。局所要素幅に対して未分解なら停止する。M/K転送不変量やこの丸め上限を固有解・物理誤差上界とは扱わない。

[曲線Hφ三水準診断](CURVED_HPHI_CONVERGENCE.md)は同一の完全P2領域で、元E/H直接差・f・RF・全参照境界損失・複素軸電圧を別判定する。正スペクトルguard不足と近縮退は未確認に保つ。幾何の解析曲線への誤差、連続問題の誤差上界、表面peak精度、永続IDの保証にはしない。

[曲線HφのE/H Gram](CURVED_HPHI_FIELDS.md)は元nativeの正周波数スペクトルと全spaceを再検証し、両二次領域の実3D体積でE/Hを別々に積分する。crossは固定円筒成分と体積密度平方根を使う単位的L2比較で、非一様写像の固有解生成ではない。元K/M Gram再現と求積差を確認しても、guard・モードID・物理精度の保証にはしない。

[曲線Hφ比較領域](CURVED_HPHI_COMPARISON.md)は全二次参照写像へのnative要素の厳密制限と、有理数の共通分割を検証する。全穴・軸役割を保持し、頂点一致を二次領域一致とは扱わない。GEOMETRY_VERIFIEDは幾何だけの判定で、場IDや物理精度の保証ではない。

2026-09-21：[Hφ調整版3の明示ID回復](HPHI_TUNING_IDENTITY_RECOVERY.md)は、確認済み継承部分空間の個別IDが未解決の場合だけ、宣言した過去の確認済み試行と実E/Hを比較する。全個別対応と継承集合の一致を満たすまで周波数を評価しない。検索/最終細分の比較親とanchorを分け、元場・raw対応・RFを保持する。真の縮退やguard不足を回復要求で迂回しない。

2026-09-21：[Hφ調整版2](HPHI_SHAPE_TUNING.md)は同軸3寸法と明示頂点変位を扱う。元Projectから生成した各形状の全域/軸/穴を検証し、実FEMのE/H個別IDが確認されてから周波数を評価する。非一様変形に解析周波数倍率を適用しない。目標差・粗細差と、独立参照/全native保存の証拠を区別する。

2026-09-21：[非一様直線Hφ比較](HPHI_MAPPED_TRACKING.md)は固定円筒成分を体積倍率の平方根で補正する比較規約を追加する。元E/H自己内積と正逆比較を保持し、Maxwell場の変形則・固有解生成・連続枝の保証とは区別する。有限比較空間と静的核は各元物理領域で扱い、周波数倍率を推測しない。
...
[tuneの個別ID回復](TUNING_IDENTITY_RECOVERY.md)はPASSの部分空間に対してのみ、以前の保存場との全個別対応と継承集合整合を検査する。
回復失敗は周波数評価nullで停止し、元のUNVERIFIED比較を回復で迂回しない。回復した試行も参照可能で、細分親と参照元の写像を分ける。
Bessel解析順位交換/縮退拒否とMaxwellの場/RF尺度則を確認。native場・周波数順位・二つの周波数ゲートは維持する。

[適応Studyの個別ID回復](ADAPTIVE_STUDY_IDENTITY_RECOVERY.md)は元目標の実形状と採用済み場を比較する。
挿入中点による履歴位置の変化を解決し、指定目標で一意対応と継承集合整合が確認できた場合だけ採用する。
回復失敗では直前の採用履歴を保持し、隣接比較PASSと回復UNVERIFIEDを別に保存して停止する。
解析縮退後の回復と後半の明示近接集合を区別し、周波数・RF・場の尺度則を独立確認した。連続枝の保証ではない。

[逐次Studyの個別ID回復](TRACKED_STUDY_IDENTITY_RECOVERY.md)は完了Studyと同じ指定検査・実点写像を使用する。
回復前後のチェックポイントから元場と全履歴を再検証し、個別IDを後続点へ継承する。未確認なら次のFEMを実行しない。
元スペクトル・規格化・両R/Qを含むRF量・周波数順位は保持し、指定二時点の再同定を連続枝の証明とは扱わない。

[完了Studyの回復](STUDY_IDENTITY_RECOVERY.md)は宣言した点とanchorの実際の形状から比較写像を導出する。
元の隣接比較を保持し、回復したID集合を後続比較へ渡す。失敗後は後続点を未追跡とし、独立スペクトルや収束状態を変更しない。

[個別モードIDの回復](MODE_IDENTITY_RECOVERY.md)は、以前に個別IDが確認された保存場との重み付き比較を使用する。
全個別対応の確認と、各継承ID集合内での候補集合一致を要求する。曖昧・縮退・集合境界違反は元集合を保持してUNVERIFIEDとなる。
回復後のIDは以後の比較へ渡すが、元の固有値順・FEM場・RF量は変更しない。指定二時点の再同定であり、縮退を通る連続枝の証明ではない。

[RF探索の幾何変数版3](RF_OPTIMIZATION_GEOMETRY.md)は各曲線数値項目を単項式の和で指定する。
変数のSI単位と係数の積の単位を明示し、元Projectから調和変位を解く。軸区間[0,L]・固定境界分割・全marked選択を保持する。
試行間比較は元の全履歴、各候補の細分比較は同一二次領域を用いる。目的関数・制約/五量収束・最終別3水準の判定は変更しない。
