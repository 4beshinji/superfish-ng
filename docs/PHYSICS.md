...
2026-09-18 JST：[同軸寸法と直線一般形状の写像契約](HPHI_TUNING.md)（H08）を追加した。
`hphi_geometry_mapping.py`は(r,z)の向き保存アフィン写像を有理行列式で検査し、反転・退化・
軸接続の軸移動（r方向のせん断/移動、軸方向シフト）を拒否する。同軸寸法`coaxial_dimension_mapping`は
内外半径/長さを独立に写像する。写像後の前メッシュは既存の厳密同領域`meridional_overlap`で
現メッシュと比較し、境界/穴の完全被覆と正Jacobianを要求する。体積/面積は元三角形の独立積分と
解析式`(b'-a')L'`・`π(b'²-a'²)L'`で一致する。FEM・求積・規格化・物理許容差・周波数/場/RFの
尺度則は変更しない。場比較・追跡・調整への接続はH09。
