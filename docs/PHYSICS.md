...
製品経路の離散空間で `u` は連続P1、従って軸上でも有限。
開発用P2固有値空間も同じ弱形式を使用する（[N01仕様](QUADRATIC_ELEMENTS.md)）。
P2の場/RF/保存統合はN02の受入まで未対応。`Hφ=0` は自動的に満たされる。
